...
```text
[read-only|review-and-fix] [guard=git|snapshot] [rounds=<n>] [resume]
```
...
- Persistent review-and-fix 需要 target-local/project-local state；resume 不依赖 chat history。代码 routes 的 state key 可以从 route、project root、base/scope 和 guard 语义派生，但必须可恢复、可校验、可拒绝过期状态。代码 routes 的 `resume` 必须由用户显式传入；未传 `resume` 的相同 invocation 不得静默继续旧 state，如果 persistent state 已存在，应报告 state 已存在并提示用户使用 `resume`、清理旧 state 或启动新的有效流程。`resume` 必须使用 route、project root、base/scope、guard、roundLimit 和 file-set identity 校验现有 state；不匹配时必须拒绝继续并报告 stale target context。
- No-state read-only 路径不能创建 `.drfx/targets/` 状态，只能在内存中持有必要 token 或 guard 信息。
...
```text
review-fix-pr base=<branch> [read-only|review-and-fix] [guard=git|snapshot] [rounds=<n>] [resume]
```
...
11. `resume` 必须显式传入。`review-fix-pr base=<branch> resume` 只能在 stored route、project root、base、merge-base/current HEAD、guard、roundLimit 和 PR file-set identity 仍匹配时继续；不匹配时必须拒绝并报告 stale target context。未传 `resume` 时不得自动接续旧 PR state。
...
- 用户级 `~/.drfx/rules/PR.md` 和项目级 `.drfx/rules/PR.md` 会被读取并合并到 review context。
...
```text
review-fix-code [scope=<path>...] [read-only|review-and-fix] [guard=git|snapshot] [rounds=<n>] [resume]
```
...
3. route 默认审查当前 project root 下的全局代码。它必须排除 `.git`、`.drfx`、`node_modules`、构建产物、依赖缓存、临时文件和其他明显非源码目录。
...
11. `resume` 必须显式传入。`review-fix-code [scope=<path>...] resume` 只能在 stored route、project root、normalized scopes/exclusions、guard、roundLimit 和 CODE file-set identity 仍匹配时继续；不匹配时必须拒绝并报告 stale target context。未传 `resume` 时不得自动接续旧 CODE state。
...
- 未传 `scope` 时审查 project root 下代码，并排除 `.git`、`.drfx`、依赖目录、构建产物和临时文件。
...
- 用户级 `~/.drfx/rules/CODE.md` 和项目级 `.drfx/rules/CODE.md` 会被读取并合并到 review context。
...
- 两个 code review routes 的 `resume` 必须显式传入；未传 `resume` 的新 invocation 不得静默继续旧 state。
- `review-fix-pr base=<branch> resume` 和 `review-fix-code [scope=<path>...] resume` 必须通过 route、project root、base/scope、guard、roundLimit 和 file-set identity 校验后才继续；不匹配时拒绝并报告 stale target context。
