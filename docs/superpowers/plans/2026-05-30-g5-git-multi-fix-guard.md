...
- **Post-review hardening (2026-05-30):** Two P2 review findings fixed with tests:
  - `abort-fix` on a legacy git fix state (no `snapshotPath` in the baseline) now **falls through to the original no-restore abort** (writes the receipt, releases the lock) instead of failing with `rollback-unavailable` and stranding the lock. Gated on `rollbackAnchor.snapshotPath` presence; snapshot mode unchanged. Test: `guard=git abort-fix without a snapshot anchor still completes (legacy fix state)`.
  - `checkGitRollbackAnchor` `priorFix` mode relaxes only the **worktree**-clean rule, not index-clean: a **staged** target change is still rejected (`assertPreFixFingerprint` doesn't validate the index). Test: `checkGitRollbackAnchor: priorFix rejects a staged target change`.
