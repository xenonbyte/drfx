...
- `units.json` — `{reviewMode, unitByteBudget, units:[{unit_id, member_count, member_bytes, member_digest, files:[...], suggestedRefs:[{path, contentId}], oversize_file?:true}], crosscuttingBackstops:[fixed list], projectReviewFingerprint}`. `projectReviewFingerprint` = ordered sha256 of all `member_digest`. `unit_id` = `unit-NNN`.
