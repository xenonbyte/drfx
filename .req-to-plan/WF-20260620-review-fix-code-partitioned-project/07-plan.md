...
r2p_version: 2
r2p_status: approved
r2p_created_at: 2026-06-20T02:22:06.309078+00:00
r2p_updated_at: 2026-06-20T02:33:54.913601+00:00
---

# Plan

## Tasks
### PLAN-TASK-001 Deterministic project-review core module
Spec References: SPEC-BEHAVIOR-002, SPEC-BEHAVIOR-004, SPEC-BEHAVIOR-005, SPEC-DATA-001
Change Type: create
TDD Applicable: yes
Files:
...
// Normative fixed backstop list (was only in prose; inlined per SPEC nit).
const CROSSCUTTING_BACKSTOPS = ['security-redaction', 'state-machine-invariant',
  'install-uninstall-fs-safety', 'cli-parser-template-consistency',
  'cross-platform-symlink', 'tests-fixtures', 'public-contract-backcompat'];
function partitionInventory(inventory, { unitByteBudget = MAX_UNIT_BYTES } = {}) {
  // directory natural order; never split a file; size > unitByteBudget => single-member oversize_file:true
}
function suggestRefsFor(unitFiles, inRootSet) { /* deterministic require()/import regex -> in-root {path,contentId} */ }
...
function aggregate(summaries, findings) { /* dedup (location,category); coverage proof; force high re-read; earned-PASS gate */ }
module.exports = { partitionInventory, suggestRefsFor, reviewCacheKey, aggregate,
  CROSSCUTTING_BACKSTOPS, MAX_UNIT_BYTES, CONTRACT_READ_BUDGET };
```
Steps:
- [ ] partitionInventory: directory natural-order bin-pack, member_bytes ≤ MAX_UNIT_BYTES, never split a file; a file larger than MAX_UNIT_BYTES becomes a single-member `oversize_file:true` unit whose body is never loaded.
- [ ] inventory line carries `{path,size,ext,contentId,unit_id}` — this intentionally extends SPEC-DATA-001's `{path,size,ext,contentId}` line with `unit_id` per the SPEC review's nit (c) (restore from DESIGN §5; without it the file→unit mapping lives only in units.json).
- [ ] reviewCacheKey hashes member_digest ++ merged-rules fingerprint ++ ordered suggestedRefs ++ ordered extraReads; the merged-rules fingerprint is `sha256(mergedRules.text)` (concrete input, since no existing rules fingerprint exists).
- [ ] aggregate emits the coverage proof (discovered / body-reviewed / extra-read / skipped+reason / high-risk-units-fully-reviewed / residual risk) and the earned-PASS gate; PASS only when every unit `coverage_risk=none`. The CLI enforces only structural preconditions; the emergent-property derivability judgment stays the model's.
Verification: npm run syntaxcheck; npm test (new test/project-review.test.js: determinism, never-split, oversize unit, reviewCacheKey invalidation on contract change).

### PLAN-TASK-002 Whole-tree inventory in target-context
Spec References: SPEC-BEHAVIOR-001, SPEC-DATA-001
Change Type: modify
TDD Applicable: yes
Files:
...
Skeleton:
```js
function streamingContentId(absPath) { /* createReadStream + sha256, same namespace as hashFileContent */ }
function resolveCodeInventory({ cwd, scopes, commandLog }) {
  const files = []; // reuse walkDirectory(rootRealPath, '', files, null, ignoreContext) -> uncapped
  const inventory = files.map((p) => ({ path: p, size: statSize(p), ext: extOf(p), contentId: streamingContentId(p) }));
  const projectReviewFingerprint = sha256(stableJson(inventory.map((f) => ({ path: f.path, contentId: f.contentId })).sort(byPath)));
  return { inventory, projectReviewFingerprint };
}
```
Steps:
- [ ] Add resolveCodeInventory reusing the existing uncapped `walkDirectory(..., null, ...)` path (no cap counting), honoring .drfxignore / version-ignore / scope-wins exactly as today.
- [ ] Use streaming sha256 for contentId to avoid single-file OOM, keeping the identical sha256 identity namespace (zero fingerprint migration).
- [ ] Compute projectReviewFingerprint as the ordered sha256 of member content ids.
Verification: npm run syntaxcheck; npm test (target-context.test.js: inventory not cap-truncated, ignore sources still apply, fingerprint stable/ordered).

### PLAN-TASK-003 Over-cap entry + checkpoint state
Spec References: SPEC-BEHAVIOR-001, SPEC-BEHAVIOR-006, SPEC-DATA-002
Change Type: modify
TDD Applicable: yes
Files:
...
if (wholeRootOverCap) {
  if (persistent) {
    writeCheckpointManifest({ status: 'checkpoint', statusReason: 'checkpoint-requested', currentPhase: 'review' });
    return { status: 'partitioned-review', reviewMode: 'partitioned', targetStateDir,
      reviewPlanPath: 'project-review/units.json', unitCount, nextAction };
  }
  return noStatePartitionPlan(); // read-only --no-state: never writes .drfx/targets/
}
```
Steps:
- [ ] Whole-root over MAX_WHOLE_ROOT_BYTES/FILES returns `partitioned-review` instead of `file-set-too-large`; under cap and scoped runs unchanged.
- [ ] Persistent path writes a checkpoint manifest under the existing CODE target-key and writes project-review/ plan files; one-shot read-only --no-state returns a no-state plan and never writes `.drfx/targets/`.
- [ ] Mark the new `Status: checkpoint` + `Current phase: review` combination legal for partition re-entry (new wiring, since checkpoint is not in ACTIVE_STATUSES).
Verification: npm run syntaxcheck; npm test (target-state.test.js: partitioned-review JSON, checkpoint reset/archive/resume/stale-fingerprint, no-state writes nothing).

### PLAN-TASK-004 Partitioned rubric + route text sync
...
- lib/generator.js
- templates/fragments/route-contract.code.claude.md
- templates/fragments/route-contract.code.codex.md
- templates/fragments/route-contract.code.gemini.md
Skeleton:
```js
// generator embeds the partitioned section; fixtures regenerate from templates.
// code.md partitioned section asserts: unit PASS != project PASS; the four disciplines
// (redaction-at-write-boundary, identity-field-coverage, allowlist-only-git, status/phase legality);
// a cross-unit finding must name the specific dependency edge / caller path.
```
Steps:
- [ ] Add the partitioned section + four locally-checkable disciplines to shared/rubrics/code.md.
- [ ] Document the three independently-mergeable phases in the route contract text (SCOPE-IN-008) and re-sync generated CODE routes + fixtures via the generator.
Verification: npm run syntaxcheck; npm test (shared-assets.test.js: route text + fixtures carry the partitioned section consistently).

### PLAN-TASK-005 Bounded unit context + merged-rules fingerprint
...
function mergedRulesFingerprint(mergedRules) {
  return crypto.createHash('sha256').update(String(mergedRules.text || '')).digest('hex');
}
function buildFileSetContextPack(opts) {
  // when reviewMode==='partitioned': include only this unit's files + merged rules
  // + suggestedRefs injected as readOnly references; add reviewMode/unit_id.
}
```
Steps:
- [ ] Support a unit subset: a partitioned context contains only that unit's files plus its suggestedRefs as readOnly references; assert no out-of-set leakage.
- [ ] Add mergedRulesFingerprint = sha256(mergedRules.text) for the reviewCacheKey.
Verification: npm run syntaxcheck; npm test (context-pack/unit-review tests: bounded context, no out-of-set files, fingerprint changes when rules change).

### PLAN-TASK-006 Unit-review lifecycle module
Spec References: SPEC-CLI-001, SPEC-BEHAVIOR-003, SPEC-BEHAVIOR-006, SPEC-BEHAVIOR-007
Change Type: create
TDD Applicable: yes
Files:
...
Skeleton:
```js
function unitContext({ unitId }) { /* oversize_file -> metadata-only + nextAction 'record oversize coverage blocker' */ }
function recordUnitReview({ unitId, payload }) { /* writes findings/<id>.json + summaries/<id>.json; cache skip iff reviewCacheKey + extraReads unchanged */ }
function nextUnit(units, summaries) { /* validate projectReviewFingerprint; drift -> stale/blocked; else next unit lacking a valid summary */ }
function unitsToReReview(changedFiles, units, summaries) { /* changed + suggestedRefs-hit + extraReads-hit units */ }
```
Steps:
- [ ] Implement unit dispatch + resume keyed on projectReviewFingerprint (drift reuses existing stale-state/blocked); oversize units return metadata-only context and a forced coverage blocker.
- [ ] record-review writes the coverage receipt; reuse a prior summary only when reviewCacheKey and every extraReads {path,contentId} are unchanged.
- [ ] After a fix round expose unitsToReReview = changed units ∪ suggestedRefs-hit ∪ extraReads-hit, for re-aggregation.
Verification: npm run syntaxcheck; npm test (workflow-fileset-lifecycle.test.js: bounded context, resume continues from next unverified unit, contract edit forces dependent re-review).

### PLAN-TASK-007 CLI subcommands
...
Skeleton:
```js
// lib/workflow/index.js is where the real allowlist + dispatch live:
// - add 'aggregate-review' to WORKFLOW_SUBCOMMANDS (~:42)
// - dispatch switch (~:700) routes 'context'/'record-review' carrying --phase unit-review|crosscutting
//   to the new unit-review module, and 'aggregate-review' to project-review.aggregate
// bin/drfx.js forwards argv to runWorkflowCommand unchanged.
// context review-fix-code <mode> --phase unit-review|crosscutting --unit <id> --json
// record-review review-fix-code <mode> --phase unit-review --unit <id> --result-stdin --json
// aggregate-review <targetStateDir> --json
```
Steps:
- [ ] Extend lib/workflow/index.js: add `aggregate-review` to WORKFLOW_SUBCOMMANDS and the dispatch switch; route `context`/`record-review` carrying `--phase unit-review|crosscutting --unit <id>` to the new unit-review module; leave non-partitioned dispatch unchanged.
- [ ] bin/drfx.js forwards the new argv shape to runWorkflowCommand; all additive, gated behind partitioned mode.
Verification: npm run syntaxcheck; npm test (cli.test.js: each subcommand parses and dispatches; non-partitioned paths unchanged).

### PLAN-TASK-008 unit-review-report parser + reason enum (copy 2)
Spec References: SPEC-DATA-002, SPEC-CONFIG-001
Change Type: modify
TDD Applicable: yes
Files:
...
Skeleton:
```js
const STATUS_REASONS = new Set([/* ...existing..., */ 'coverage-incomplete']);
function parseUnitReviewReport(text) {
  // additive; coverage_risk restricted to the enum {'none','high'}
}
// readSemanticPayload dispatch gains 'unit-review-report'; reviewer-pass-fail/fix-report untouched.
```
Steps:
- [ ] Add the additive parseUnitReviewReport with coverage_risk fixed to none|high; register it in readSemanticPayload; leave reviewer-pass-fail (parseFinalResponseBlock) and fix-report (parseFixReport) untouched.
- [ ] Add `coverage-incomplete` to this file's STATUS_REASONS copy.
Verification: npm run syntaxcheck; npm test (parser tests: valid/invalid unit-review-report, coverage_risk enum rejects other values).

### PLAN-TASK-009 Reason enum (copy 1) + parity test
...
Skeleton:
```js
const STATUS_REASONS = Object.freeze([/* ...existing..., */ 'coverage-incomplete']);
```
Steps:
- [ ] Add `coverage-incomplete` to the workflow-state STATUS_REASONS copy.
- [ ] Add a constants-level parity test (workflow-state-v2.test.js / target-state.test.js) asserting the workflow-state and semantic-parsers STATUS_REASONS copies are equal — NOT in the route-text suite.
Verification: npm run syntaxcheck; npm test (workflow-state-v2.test.js parity assertion green).

### PLAN-TASK-010 Finalize amendment for coverage-incomplete
Spec References: SPEC-BEHAVIOR-008, SPEC-CONFIG-001
Change Type: modify
TDD Applicable: yes
Files:
...
- templates/fragments/route-contract.code.codex.md
- templates/fragments/route-contract.code.gemini.md
- test/fixtures/generated/**/review-fix-code.*
- test/fixtures/embedded/**/review-fix-code.*
Skeleton:
```js
if (state && state.persistent && finalResponse.finalStatus === 'stopped-with-deferrals') {
  const reason = finalResponse.statusReason;
  if (!['deferred-findings', 'round-limit', 'coverage-incomplete'].includes(reason)) fail('ERR_FINAL_DEFERRED_STATUS_REASON', ...);
  const deferredIssueIds = normalizeIssueIds(state.deferredBlockingIssueIds || []);
  if (reason !== 'coverage-incomplete' && deferredIssueIds.length === 0) fail('ERR_FINAL_DEFERRED_FINDINGS_EMPTY', ...);
  // coverage-incomplete requires a Deferrals/blockers owner + next action, no reviewer issue id.
}
```
Steps:
- [ ] Amend the validateReadOnly stopped-with-deferrals branch to admit coverage-incomplete and allow an empty deferred-reviewer-id set for that reason (gate on a coverage-deferral owner + next action).
- [ ] file-set-finalize routes through the same validateFinalResponse, so confirm the file-set path accepts it; update the shared/core.md route contract enum.
- [ ] Regenerate/sync generated and embedded CODE route text so `coverage-incomplete` is consistent across `shared/core.md`, CODE route templates, generated fixtures, and embedded skill fixtures.
- [ ] Add/update `test/shared-assets.test.js` coverage for that route-text and fixture consistency; keep the two `STATUS_REASONS` copy parity assertion in the constants-level test from PLAN-TASK-009.
Verification: npm run syntaxcheck; npm test (finalize tests: coverage-incomplete finalizes on document + file-set paths; PASS carrying that reason is rejected; shared-assets.test.js: shared/core.md, generated CODE routes, embedded CODE skill text, and fixtures carry coverage-incomplete consistently).

### PLAN-TASK-011 Fix integration (counting frozen)
...
Skeleton:
```js
// After a fix round: re-review unitsToReReview(changedFiles), then re-aggregate.
// fixAttemptCount stays per-file-set; MAX_FIX_ATTEMPTS = 5 unchanged; rounds= unchanged.
```
Steps:
- [ ] Wire post-fix re-review of affected units + re-aggregate into the existing triage→fix→diff-review→full-re-review loop, fixes bounded to the in-set union via the existing buildFileSetFixerGuard.
- [ ] Leave fixAttemptCount, MAX_FIX_ATTEMPTS, and rounds= counting untouched.
Verification: npm run syntaxcheck; npm test (workflow-fileset-lifecycle.test.js: fix stays in-set, affected units re-reviewed, counting unchanged, gate-satisfied => PASS).

## Trace
| This ID | Upstream | Status |
|---|---|---|
| PLAN-TASK-001 | SPEC-BEHAVIOR-002, SPEC-BEHAVIOR-004, SPEC-BEHAVIOR-005, SPEC-DATA-001 | Phase 1+3 |
| PLAN-TASK-002 | SPEC-BEHAVIOR-001, SPEC-DATA-001 | Phase 1 |
| PLAN-TASK-003 | SPEC-BEHAVIOR-001, SPEC-BEHAVIOR-006, SPEC-DATA-002 | Phase 1 |
| PLAN-TASK-004 | SPEC-BEHAVIOR-003 | Phase 1 |
| PLAN-TASK-005 | SPEC-BEHAVIOR-003, SPEC-BEHAVIOR-004 | Phase 2 |
| PLAN-TASK-006 | SPEC-CLI-001, SPEC-BEHAVIOR-003, SPEC-BEHAVIOR-006, SPEC-BEHAVIOR-007 | Phase 2 |
| PLAN-TASK-007 | SPEC-CLI-001 | Phase 2+3 |
| PLAN-TASK-008 | SPEC-DATA-002, SPEC-CONFIG-001 | Phase 2+3 |
| PLAN-TASK-009 | SPEC-CONFIG-001 | Phase 3 |
| PLAN-TASK-010 | SPEC-BEHAVIOR-008, SPEC-CONFIG-001 | Phase 3 |
| PLAN-TASK-011 | SPEC-BEHAVIOR-007 | Phase 3 |
