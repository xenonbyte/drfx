...
Spec References: SPEC-BEHAVIOR-002, SPEC-BEHAVIOR-003, SPEC-BEHAVIOR-006
Change Type: modify
TDD Applicable: yes
Files:
- lib/generator.js
...
- [ ] Add Codex shared measurement for route bytes, embedded shared bytes, copied shared bytes, duplicate bytes, and the 16 KiB plus 12% de-duplication gate.
- [ ] Implement Codex shared de-duplication only if the gate and fail-closed tests pass; otherwise record the measured no-op outcome. This closes SCOPE-IN-002, SCOPE-IN-003, and SCOPE-IN-004.
- [ ] Refresh all affected generated and embedded fixtures by hand, not only the representative files listed above.
...
- [ ] Add negative tests for missing baseline, unparseable baseline, reference mutation, non-target mutation, target-only guard unavailable, and lock reacquisition failure.
...
| PLAN-TASK-003 | SPEC-BEHAVIOR-002 [ADDRESSED], SPEC-BEHAVIOR-003 [ADDRESSED], SPEC-BEHAVIOR-006 [ADDRESSED], SCOPE-IN-002, SCOPE-IN-003, SCOPE-IN-004 | [ADDRESSED] |
