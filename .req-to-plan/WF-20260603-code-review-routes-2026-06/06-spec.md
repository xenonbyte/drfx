...
r2p_version: 2
r2p_status: approved
r2p_created_at: 2026-06-02T18:39:14.792893+00:00
...
After implementation, `@xenonbyte/document-review-fix` must expose six review-fix routes from one shared route model: four existing document routes plus new `review-fix-pr` and `review-fix-code`. Document routes keep current document semantics and add `rounds=<n>`. Code routes use file-set target contexts, route-specific validation, PR/CODE rubrics, external PR/CODE rules, file-set guards, explicit `resume`, and platform-specific generated entries. All routes default to `review-and-fix` and `guard=git`; `guard=snapshot` is explicit-only; `read-only` never fixes, creates automatic-fix state, or claims workflow PASS. Claude Code and Codex can perform automatic repair where workflow hard constraints pass; Gemini remains advisory-only and must not claim automatic fixes or workflow PASS for code review routes. Code routes must not call or wrap platform-native `/review`.
...
| SPEC-FR-005 | PR and CODE routes must include built-in rubrics and merge external user/project rules without allowing external rules to relax workflow hard constraints. PR rubric categories are `correctness`, `regression`, `safety`, `tests`, `contracts`, `maintainability`, and `platform`. CODE rubric categories are `correctness`, `architecture`, `state-and-io`, `safety`, `tests`, `contracts`, `maintainability`, and `platform`. | DES-SPEC-007 | [ADDRESSED] |
...
| SPEC-FR-009 | All six routes must default omitted mode to `review-and-fix` and omitted guard to `guard=git`; `guard=snapshot` must only be used when the user explicitly selects it. `read-only` is a single complete review that may report clean/findings/blocked/unsupported style read-only outcomes, but must not write files, run automatic fixes, create automatic-fix state, or claim workflow PASS. | Requirement Brief common constraints, RISK-SPEC-003, RISK-SPEC-004 | [ADDRESSED] |
| SPEC-FR-010 | `review-fix-pr` and `review-fix-code` must use this project's own workflow contracts and must not implement review by invoking, wrapping, or delegating to platform-native `/review` commands. | Raw Requirement non-goal, DESIGN rejected option | [ADDRESSED] |

## Interfaces

| ID | Interface Contract | Inputs | Outputs | Error Behavior | Trace | Status |
|---|---|---|---|---|---|---|
| SPEC-IF-001 | Route descriptors must expose route name, route kind (`document`, `pr`, `code`), default mode `review-and-fix`, default guard `git`, platform policy, rubric/rule key, and target context kind. | Route name | Descriptor or unsupported-route error | Unknown route fails before target reads or state creation. | DES-SPEC-001, RISK-SPEC-003, RISK-SPEC-004 | [ADDRESSED] |
| SPEC-IF-002 | Document route invocation must keep current grammar and add optional `rounds=<n>`: `<path>` or `target=<path>`, repeated `ref=<path>`, strictness, mode, assurance, guard, resume, ledger, root, debug, rounds. Omitted mode/guard materialize to `review-and-fix` and `guard=git`. | Document route tokens | Normalized invocation with `roundLimit`, defaulted mode/guard, or usage-only error | Invalid token/value stops before target/reference body reads. | DES-SPEC-002, DES-SPEC-003 | [ADDRESSED] |
| SPEC-IF-003 | `review-fix-pr` invocation grammar is `base=<branch>` plus optional mode, `guard=git|snapshot`, `rounds=<n>`, and `resume`; `base` is required. Omitted mode/guard materialize to `review-and-fix` and `guard=git`; `guard=snapshot` is accepted only when explicitly present. `resume` must be explicit and must not be inferred from existing state. | PR route tokens | PR target context, resume context, or usage/blocked result | Missing base, invalid base, same current branch, missing merge-base, or stale/mismatched explicit resume stop before review/fix. | DES-SPEC-012, RISK-SPEC-003, RISK-SPEC-004 | [ADDRESSED] |
| SPEC-IF-004 | `review-fix-code` invocation grammar is optional repeated `scope=<path>` plus optional mode, `guard=git|snapshot`, `rounds=<n>`, and `resume`; `base=<branch>` is invalid. Omitted mode/guard materialize to `review-and-fix` and `guard=git`. `resume` must be explicit and must not be inferred from existing state. Default traversal excludes `.git`, `.docs-review-fix`, `node_modules`, build outputs, dependency caches, temporary files, and other obvious non-source directories; explicit scopes into excluded locations are invalid. | CODE route tokens | Code target context, resume context, or usage/blocked result | Invalid base/scope/rounds or stale/mismatched explicit resume stop before review/fix. | DES-SPEC-013, RISK-SPEC-003, RISK-SPEC-004, RISK-SPEC-007 | [ADDRESSED] |
| SPEC-IF-005 | PR/CODE rules load in this exact order: workflow hard constraints, built-in route rubric, user-global rule file, then project-local rule file. PR paths are `~/.docs-review-fix/rules/PR.md` then `.docs-review-fix/rules/PR.md`; CODE paths are `~/.docs-review-fix/rules/CODE.md` then `.docs-review-fix/rules/CODE.md`. Project-local rules are more specific than user-global rules, but external rules cannot relax hard constraints. | Route kind, project root, home dir | Merged rule context and warnings/errors with rubric categories preserved | Symlink, invalid path, or hard-constraint conflict blocks or reports per existing strictness rules. | DES-SPEC-007, RISK-SPEC-010 | [ADDRESSED] |
...
| SPEC-STATE-003 | PR/code target state keys must include route kind and target identity: PR includes base/merge-base/current HEAD or equivalent stale-detection fields; CODE includes normalized scopes/exclusions/file-set identity; both include guard and `roundLimit` as invocation metadata that must match on resume. | Explicit `resume` succeeds only when the stored identity and invocation metadata (guard, `roundLimit`) still match; mismatch returns stale/externally-changed style status. A matching invocation without `resume` must not silently reuse old persistent state. | DES-SPEC-004, DES-SPEC-012, DES-SPEC-013 | [ADDRESSED] |
| SPEC-STATE-004 | `read-only` mode must not create `.docs-review-fix/targets/` automatic-fix state for document, PR, or CODE routes and must not persist an automatic-fix PASS state. | No-state tokens/guards may be used in memory or command payloads only; output may report read-only clean/findings/blocked/unsupported outcomes. | DES-SPEC-009, RISK-SPEC-003 | [ADDRESSED] |
...
| SPEC-ERR-005 | Stale PR/code state must refuse explicit resume, and existing PR/code state must not be silently reused without `resume`. | Explicit resume output must state stale target context rather than using old base/scope/file-set data. When matching persistent state exists but `resume` is omitted, output must not continue from that state; it must start through normal start/preflight semantics or report existing state with guidance to use `resume`, clean state, or start a new valid workflow. | DES-SPEC-004 | [ADDRESSED] |
...
| SPEC-ERR-009 | `read-only` mode must not enter the automatic fix loop. | A read-only invocation reports a read-only result and stops after one complete review; `read-only rounds=<n>` is unsupported and does not start workflow state. | RISK-SPEC-003, RISK-SPEC-005 | [ADDRESSED] |
...
| SPEC-SAFE-005 | CODE scope resolution must reject paths outside project root, symlink-unsafe paths, excluded directories, missing paths, and unsafe traversal. Mandatory excluded locations include `.git`, `.docs-review-fix`, `node_modules`, build outputs such as `dist`, `build`, `coverage`, dependency caches, temporary files, and other obvious non-source directories. | Rejected scopes cannot produce partial review output or state. | DES-SPEC-013, RISK-SPEC-007 | [ADDRESSED] |
...
| SPEC-SAFE-007 | Code routes must not call or wrap platform-native `/review` commands. | Generated entries and runtime workflow text must invoke this project's coordinator/reviewer/fixer contract directly; any platform-native `/review` delegation is a contract violation. | Raw Requirement non-goal, RISK-SPEC-012 | [ADDRESSED] |
...
| DES-BND-006 | Rulebook | route key -> merged rules | No mutation | Invalid/conflicting rules block/report | Document rules preserved | Allowlist expansion | Remove PR/CODE support safely | Hard constraints outrank external rules after exact PR/CODE rubric and rule-order loading. | SPEC-IF-005, SPEC-ERR-008 | Rulebook tests |
| DES-BND-007 | Platform generation | descriptor -> generated files | Manifest ownership | Unsupported platform/route fails | Existing install paths preserved | Generated route expansion | Manifest-backed uninstall | Six routes generated with correct platform wording and no code-route `/review` delegation. | SPEC-IF-006, SPEC-SAFE-007 | Shared asset/install tests |
| DES-BND-008 | Workflow lifecycle | context + reports -> status | receipts, ledger, manifest | Missing full re-review blocks PASS | Terminal statuses preserved | Lifecycle extension | Existing finalize semantics | Coordinator owns final pass; read-only no-state/no-PASS. | SPEC-FR-007, SPEC-FR-009, SPEC-SAFE-004 | Workflow tests |
...
| DES-INT-002 | Runtime workflow entry | user invocation -> semantic payload | normalized invocation + context | Invalid stops before reads/state | Document payload preserved | Payload schema expands | Resume stale refusal | Generated entries materialize `review-and-fix` and `guard=git` defaults before workflow calls. | SPEC-IF-002, SPEC-IF-003, SPEC-IF-004, SPEC-STATE-001 | Workflow JSON tests |
| DES-INT-003 | Rule file integration | rule paths -> merged context | No mutation | Invalid/conflict handling | Document rule files preserved | Rule name allowlist expands | No user rule deletion | PR/CODE rule paths participate in review context after hard constraints and built-in rubrics, with project-local rules loaded last. | SPEC-IF-005 | Rulebook tests |
...
| SPEC-ACC-011 | Given any route omits mode and guard, when invocation normalization runs, then mode is `review-and-fix` and guard is `git`; when `guard=snapshot` is absent, snapshot is not silently selected. | Parser/workflow payload tests for document, PR, and CODE route kinds. | RISK-SPEC-003, RISK-SPEC-004 | [ADDRESSED] |
| SPEC-ACC-012 | Given `read-only` is invoked without `rounds`, when review completes, then it reports a read-only clean/findings/blocked/unsupported result and creates no automatic-fix state, runs no fix, and claims no workflow PASS. | No-state/read-only workflow test with state-directory assertions. | RISK-SPEC-003, RISK-SPEC-008 | [ADDRESSED] |
| SPEC-ACC-013 | Given PR/CODE rule loading runs, when user-global and project-local rules exist, then the order is hard constraints, built-in rubric, user-global, project-local; rubric category coverage is preserved and hard-constraint relaxation blocks/reports conflict. | Rulebook unit tests with PR and CODE fixtures. | RISK-SPEC-010 | [ADDRESSED] |
| SPEC-ACC-014 | Given `review-fix-code scope=.git`, `scope=.docs-review-fix`, `scope=node_modules`, or a build/cache/temp directory, when preflight runs, then it rejects the scope and starts no review or state. | Scope resolver tests for mandatory exclusions. | RISK-SPEC-007 | [ADDRESSED] |
| SPEC-ACC-015 | Given generated code-route entries are inspected, when route workflow text is checked, then they do not call or wrap platform-native `/review` and instead describe this project's coordinator workflow. | Shared asset/platform generation text test. | Raw Requirement non-goal, RISK-SPEC-012 | [ADDRESSED] |
| SPEC-ACC-016 | Given `review-fix-pr base=main resume` or `review-fix-code scope=lib resume`, when stored route/project/base/scope/guard/roundLimit/file-set identity no longer matches the current invocation, then resume refuses with stale target context and starts no review or fix. | Resume/state identity tests for PR and CODE fixtures. | DES-SPEC-004 (resume safety); Raw Requirement resume | [ADDRESSED] |
| SPEC-ACC-017 | Given matching PR/CODE persistent state exists, when the same PR/CODE invocation runs without `resume`, then it does not silently continue old state and instead follows normal start/preflight behavior or reports existing state with guidance to use `resume`, clean state, or start a new valid workflow. | Workflow start/resume tests for PR and CODE fixtures. | DES-SPEC-004 (resume safety); Raw Requirement resume | [ADDRESSED] |
...
| SPEC-EDGE-003 | CODE scope points at excluded infrastructure such as `.git`, `.docs-review-fix`, `node_modules`, `dist`, `build`, `coverage`, a dependency cache, or a temp directory. | Reject before scanning, even if the path exists. | DES-SPEC-013, RISK-SPEC-007 | [ADDRESSED] |
...
|---|---|---|
| Requirement Brief Scope | `SPEC-FR-001` through `SPEC-FR-010` | [ADDRESSED] |
| Requirement Brief Acceptance | `SPEC-ACC-001` through `SPEC-ACC-015` | [ADDRESSED] |
| DESIGN Resume Coverage | `SPEC-ACC-016`, `SPEC-ACC-017` (via `DES-SPEC-004`) | [ADDRESSED] |
...
| SPEC-PLAN-002 | SPEC | SPEC-IF-002, SPEC-IF-003, SPEC-IF-004, SPEC-FR-009 | Invocation parsing | Add parser/preflight tests before implementation for document rounds, PR base, CODE scope, PR/CODE explicit `resume`, invalid tokens, invalid rounds, default `review-and-fix`, default `guard=git`, explicit-only `guard=snapshot`, and read-only rounds. | DES-PLAN-002 | [DEFERRED] |
...
| SPEC-PLAN-004 | SPEC | SPEC-STATE-001, SPEC-STATE-003, SPEC-SAFE-005 | Target context/state | Implement target context state keys, CODE mandatory exclusions, explicit PR/CODE resume identity checks, stale resume refusal, and no silent resume without `resume`; use local fixtures if needed. | DES-PLAN-004 | [DEFERRED] |
...
| SPEC-PLAN-006 | SPEC | SPEC-IF-005, SPEC-ERR-008, SPEC-FR-005 | Rulebook | Add PR/CODE rubric category assertions, exact rule loading order tests, project-local precedence checks, and hard-constraint conflict tests. | DES-PLAN-006, RISK-PLAN-009 | [DEFERRED] |
| SPEC-PLAN-007 | SPEC | SPEC-IF-006, SPEC-COMPAT-002, SPEC-SAFE-007 | Platform generation/install | Update generated outputs and manifest tests for six routes, Gemini advisory-only code behavior, and absence of platform-native `/review` delegation in code routes. | DES-PLAN-007, RISK-PLAN-006, RISK-PLAN-007 | [DEFERRED] |
| SPEC-PLAN-008 | SPEC | SPEC-FR-007, SPEC-FR-009, SPEC-SAFE-004, SPEC-STATE-004, SPEC-OBS-001 | Workflow lifecycle | Preserve reviewer readiness, write blocking, stdin proof, diff review, full re-review, redaction, final validation, and read-only no-fix/no-state/no-PASS behavior. | DES-PLAN-008, RISK-PLAN-008 | [DEFERRED] |
...
| SPEC-PLAN-011 | SPEC checkpoint repair | SPEC-ACC-011, SPEC-ACC-012, SPEC-ACC-013, SPEC-ACC-014, SPEC-ACC-015 | SPEC repair verification | PLAN must include explicit implementation/verification coverage for repaired SPEC findings: defaults/read-only semantics, rule categories/load order, mandatory CODE exclusions, and no platform `/review` delegation. | RISK-SPEC-003, RISK-SPEC-004, RISK-SPEC-007, RISK-SPEC-010, RISK-SPEC-012 | [DEFERRED] |
...
| DES-SPEC-007 | [ADDRESSED] Covered by exact rulebook category, path, order, and hard-constraint contracts. |
...
| DES-SPEC-013 | [ADDRESSED] Covered by CODE resolver contracts and mandatory exclusion contracts. |
...
| DES-BND-007 | [ADDRESSED] Covered by `SPEC-IF-006` and `SPEC-SAFE-007`. |
| DES-BND-008 | [ADDRESSED] Covered by `SPEC-FR-007`, `SPEC-FR-009`, and `SPEC-SAFE-004`. |
...
| SPEC-REPAIR-001 | [ADDRESSED] SPEC subagent `changes_requested` finding on defaults/read-only semantics covered by `SPEC-FR-009`, `SPEC-IF-001` through `SPEC-IF-004`, `SPEC-STATE-004`, `SPEC-ERR-009`, `SPEC-ACC-011`, `SPEC-ACC-012`, and `SPEC-PLAN-011`. |
| SPEC-REPAIR-002 | [ADDRESSED] SPEC subagent `changes_requested` finding on PR/CODE rubric categories and rule loading order covered by `SPEC-FR-005`, `SPEC-IF-005`, `SPEC-ACC-013`, and `SPEC-PLAN-006`. |
| SPEC-REPAIR-003 | [ADDRESSED] SPEC subagent `changes_requested` finding on CODE exclusions covered by `SPEC-IF-004`, `SPEC-SAFE-005`, `SPEC-EDGE-003`, `SPEC-ACC-014`, and `SPEC-PLAN-004`. |
| SPEC-REPAIR-004 | [ADDRESSED] SPEC subagent `changes_requested` finding on platform-native `/review` prohibition covered by `SPEC-FR-010`, `SPEC-SAFE-007`, `SPEC-ACC-015`, and `SPEC-PLAN-007`. |
...
- Status: ready
- Reason: All DESIGN Spec Inputs, change points, boundaries, integration boundaries, acceptance criteria, SPEC subagent repair findings, and carried Risk/Plan inputs have testable contracts or deferred PLAN inputs with closure status. External documentation was checked for Git and Node.js dependencies. No requirement scope or DESIGN decision was changed.
- Safe next node: SPEC Checkpoint, then PLAN.

## SPEC Checkpoint

- Status: ready for checkpoint decision
- Review Sources: main-agent review of approved Requirement Brief, Risk Discovery, DESIGN, DESIGN subagent review, SPEC workflow rules, Context7 documentation checks, and SPEC subagent review `spec-subagent-review-v1.md`.
- Required Changes: SPEC v1 subagent findings repaired in this artifact.
- Post-approval revision (2026-06-03): materialized explicit PR/CODE `resume` contracts that DESIGN `DES-SPEC-004` already required — added `SPEC-ACC-016`/`SPEC-ACC-017`, extended `SPEC-IF-003`/`SPEC-IF-004` with the explicit `resume` token, `SPEC-STATE-003` with resume identity (guard/`roundLimit`) and no-silent-reuse, and `SPEC-ERR-005` with no silent resume without the token. Re-cited the resume acceptance rows to `DES-SPEC-004` (the Requirement Brief carries no resume acceptance). Aligned `roundLimit` resume wording to strict match, consistent with `SPEC-ACC-016` and the requirement. No DESIGN decision changed.
- User Confirmations:
