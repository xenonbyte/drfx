...
- **Part 1 / Plan B**：`aggregate FAIL → triage → begin-fix → fix → end-fix → 增量重审受影响 unit → re-aggregate → earned PASS`。核心杠杆：在 end-fix 已验证 route-owned 的窗口固化新 inventory 指纹，复用 `nextUnit` resume + review cache，**删受影响 unit 的 summary = 标记待重审**。取代 v1（②′）的只读守卫。
...
**受影响集** = `unitsToReReview(changedFiles, units, summaries)`（`lib/workflow/file-set-unit-review.js:548`），显式三路：(a) changed-member ∪ (b) suggestedRef-hit ∪ (c) stored-extraRead-hit。`member_digest` 不变保证未受影响 unit 的旧 summary 仍成立。
...
**假设**：end-fix 时 worktree 变化 = 已验证 route-owned fix，且 fix 只改内容、不增删成员、不破坏分桶。
