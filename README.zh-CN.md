...
Review 整个 project root（省略 `scope=` 表示全项目），或限定到一个/多个根目录。whole-root CODE review 最多 300 个文件或 1,500,000 字节；项目更大时请使用 `scope=<path>` 或在项目根放置 `.drfxignore` 文件：
...
- `scope=<path>` 指定要 review 的 source root。可重复（repeatable）传入多个 `scope=`。省略 scope 表示整个 project root，最多 300 个文件或 1,500,000 字节；更大的 whole-root file set 会以 `file-set-too-large` 阻断，并要求使用更窄的 `scope=<path>` 或通过 `.drfxignore` 缩减 file set。显式传入 `scope=` 的运行不受该上限约束。
- 内置排除（固定、始终生效）：VCS 状态（`.git`、`.hg`、`.svn`）；本工具状态（`.drfx`、legacy `.docs-review-fix`）；本地 agent/tool 状态（`.claude`、`.codex`、`.codegraph`、`.gemini`、`.req-to-plan`）；依赖树与包缓存（`node_modules`、`bower_components`、`vendor`、`.pnp`、`.yarn`、`.pnpm-store`、`.gradle`、`.m2`）；构建产物（`dist`、`build`、`out`、`target`、`.next`、`.nuxt`、`.svelte-kit`、`.output`）；覆盖率与工具缓存（`coverage`、`.nyc_output`、`.cache`、`.parcel-cache`、`.turbo`、`__pycache__`、`.pytest_cache`、`.mypy_cache`、`.tox`）；临时与编辑器目录（`tmp`、`temp`、`.tmp`、`.idea`、`.vscode`）；以及 OS 杂项文件 `.DS_Store` 与 `Thumbs.db`。该清单是内置固定的：**不读取 `.gitignore`**，被 gitignore 忽略但不在该清单内的路径仍会进入 reviewed file set。
- 项目根的 `.drfxignore` 文件提供用户级排除：每行一个 root-relative 目录路径；`#` 开头的行是注释；空行忽略。无 glob、无否定。该文件必须是常规文件（符号链接形式的 `.drfxignore` 会被拒绝），每个条目必须指向存在的根内目录。显式 `scope=` 等于某条目或位于其内时，该条目本次失效（失效会被报告，绝不静默）。生效条目参与 review-target 身份：修改 `.drfxignore` 即产生不同的 review target，旧状态无法跨该变更 resume——请全新开始（或 `reset`）。
- `read-only` 或 `review-and-fix`（Claude Code 和 Codex 默认 `review-and-fix`；Gemini 上为 advisory read-only）。
- `guard=git` 为默认值；Git rollback anchor 不可用时使用 `guard=snapshot`。路由永远不会静默切换 guard mode。
- `resume` 显式从已保存的 state 继续。拒绝 stale state，不存在静默复用。
- `reset` 归档现有 target state（移到 `.drfx/archived/`，绝不删除）并全新开始 review。当 stale state 已无法 resume 时（例如排除策略变化改变了 file set），这是显式的逃生口。`resume` 与 `reset` 互斥。
