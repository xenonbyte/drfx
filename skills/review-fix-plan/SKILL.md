...
```text
review-fix-plan target=<path> [ref=<path>...] read-only|review-and-fix [strict|normal] [assurance=practical|strict-verified|advisory] [resume] [ledger=<target-local path>] [root=<project-root>]
```
