...
7. **Target resolution & file-set workflow** (`lib/target-context.js`, `lib/workflow/`): `lib/target-context.js` resolves PR file sets (local read-only git diff `base..HEAD`, never fetches) and CODE file sets (deterministic FS traversal with built-in infrastructure exclusions plus project-root `.drfxignore` user exclusions — one root-relative directory per line, no globs, never reads `.gitignore`; active entries join the target identity, and an explicit `scope=` overrides any entry that covers it), computing a per-file-set fingerprint from current worktree content. `lib/workflow/file-set-*.js` runs the PR/CODE lifecycle (context → review → triage → fix loop → diff review → finalize), bounded by `rounds=<n>` and guarded by git or snapshot file-set guards.
