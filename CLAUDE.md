...
1. **Routes & Generator** (`lib/routes.js`, `lib/generator.js`): `lib/routes.js` is the shared registry of the six routes (`review-fix-spec/plan/design/doc` plus `review-fix-pr/code`), each with a `routeKind` (document/pr/code) and a `targetContextKind` (single-document vs file-set). `lib/generator.js` renders platform-specific output files from `templates/` and `templates/fragments/`, embedding shared content from `shared/` — core workflow docs, rubrics, and reviewer/fixer/coordinator prompt fragments. Template rendering uses `{{PLACEHOLDER}}` substitution.
...
7. **Target resolution & file-set workflow** (`lib/target-context.js`, `lib/workflow/`): `lib/target-context.js` resolves PR file sets (local read-only git diff `base..HEAD`, never fetches) and CODE file sets (deterministic FS traversal with infrastructure exclusions), computing a per-file-set fingerprint from current worktree content. `lib/workflow/file-set-*.js` runs the PR/CODE lifecycle (context → review → triage → fix loop → diff review → finalize), bounded by `rounds=<n>` and guarded by git or snapshot file-set guards.
...
- **PASS is earned, never assumed**: read-only, advisory, Gemini, diff-review-only, unverified, and stale/drifted file-set runs can never claim a workflow PASS. File-set fixes stay inside the recorded dependency set, guarded by git (real `git status`) or snapshot (whole-tree fingerprint) file-set guards before any write.
