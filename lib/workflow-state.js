...
const TARGET_CONTEXT_KINDS = Object.freeze(['document', 'pr', 'code', 'r2q']);
...
// R2Q file-set identity block — emitted only for the r2q kind. Identity is
// file-set based (mirrors code/pr), NOT single-document. requirementDir is the
// relative path to the requirement directory; runMdSha256 is the SHA-256 of the
// protected read-only run.md file; fileSetFingerprint covers the editable files.
const MANIFEST_V2_R2Q_FILESET_FIELDS = Object.freeze([
...
const MANIFEST_V2_LABELS = new Map(
  [
    ...
    ...MANIFEST_V2_R2Q_FILESET_FIELDS,
...
function manifestV2FieldsForKind(kind) {
  ...
  if (kind === 'r2q') return [...MANIFEST_V2_COMMON_HEAD_FIELDS, ...MANIFEST_V2_R2Q_FILESET_FIELDS];
...
function normalizeManifestV2(manifest) {
  ...
  } else if (targetContextKind === 'r2q') {
    // R2Q file-set identity: a relative requirement directory path, the SHA-256
    // of the protected run.md, and a deterministic fingerprint over the editable
    // files. No base/merge-base/head; no list fields. roundLimit is uniform above.
    normalized.requirementDir = normalizeManifestRelativePath(normalized.requirementDir, 'Requirement dir');
