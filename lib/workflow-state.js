...
// R2P file-set identity block — emitted only for the r2p kind. Identity is
...
const MANIFEST_V2_R2P_FILESET_FIELDS = Object.freeze([
...
function manifestV2FieldsForKind(kind) {
  ...
  if (kind === 'r2p') return [...MANIFEST_V2_COMMON_HEAD_FIELDS, ...MANIFEST_V2_R2P_FILESET_FIELDS];
...
function normalizeManifestV2(manifest) {
  ...
  if (targetContextKind === 'document') {
    ...
    // R2P file-set identity: a relative requirement directory path, the SHA-256
