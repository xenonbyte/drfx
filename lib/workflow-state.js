...
// File-set identity block — emitted only for pr/code kinds. CODE (PLAN-TASK-004)
// extends this same block with its normalized scopes/exclusions.
const MANIFEST_V2_FILESET_FIELDS = Object.freeze([
  ['base', 'Base'],
...
const MANIFEST_V2_LABELS = new Map(
  [
    ...
    ...MANIFEST_V2_FILESET_FIELDS,
    ['targetContextKind', TARGET_CONTEXT_KIND_LABEL]
  ].map(([key, label]) => [label, key])
);

function manifestV2FieldsForKind(kind) {
  const tail = kind === 'document' ? MANIFEST_V2_DOCUMENT_FIELDS : MANIFEST_V2_FILESET_FIELDS;
  return [...MANIFEST_V2_COMMON_HEAD_FIELDS, ...tail];
}
...
function normalizeManifestV2(manifest) {
  ...
  if (targetContextKind === 'document') {
    ...
    // File-set identity: a base ref string, resolved base/merge-base/HEAD
