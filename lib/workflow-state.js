...
// `userExcludes` carries the ACTIVE .drfxignore entries (root-relative
// directory paths); absent in pre-.drfxignore manifests ⇒ parses as empty.
...
// Normalize the CODE userExcludes list: ordered .drfxignore PATTERN lines
// (gitignore syntax — globs, `!` negation). Order is semantic (last match
// wins), so the list is NEVER sorted; exact duplicates collapse. Entries are
// redacted, trimmed, non-empty text — no path-shape validation, because a
