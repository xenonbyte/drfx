...
function ensureBaseDirectories(platform, roots, homeDir, cwd) {
  ...
  fs.mkdirSync(path.join(homeDir, '.docs-review-fix', 'backups', platform), { recursive: true });
}
...
function backupPathFor(platform, originalPath, homeDir) {
  ...
  return path.join(homeDir, '.docs-review-fix', 'backups', platform, `${stamp}-${nonce}`, basename);
}
...
async function installPlatform(platform, options = {}) {
  ...
  copySharedAssets(path.join(homeDir, '.docs-review-fix'), { all: true });
  ...
  try {
    ...
    const manifest = {
      ...
      capabilityDescriptor: {
        path: `~/.docs-review-fix/capabilities/${normalizedPlatform}.json`,
        mutable: true
      },
      generated,
      backups
