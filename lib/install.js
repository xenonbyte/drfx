...
function validateGeneratedPlan(platform, generatedEntries) {
  ...
  for (const generated of generatedEntries) validateCodexOwnedSharedSource(generated);
}
...
function writeGeneratedDirectory(targetPath, generated, hooks = {}) {
  ...
  try {
    ...
    try {
      ...
      // Best-effort cleanup of the replaced directory; never mask the original error.
    }
    throw error;
  }
}
...
async function installPlatforms(options = {}) {
  ...
  const results = {};
  for (const platform of platforms) {
    // Clean reinstall: remove the previous install (by its own manifest) before writing the
    // new plan, so routes dropped or renamed between versions are not orphaned (left on disk
    // yet absent from the new manifest, hence un-reclaimable by a later uninstall).
    // uninstallPlatform no-ops when nothing is installed and preserves user-modified files
    // via its partial-uninstall path.
    await uninstallPlatform(platform, options);
