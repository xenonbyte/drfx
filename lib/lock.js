...
function lockPaths(projectRoot, targetKey) {
  const targetDir = path.join(path.resolve(projectRoot), '.docs-review-fix', 'targets', targetKey);
...
function manifestLastKnownSha({ manifest, manifestPath, projectRoot, targetKey }) {
  ...
  if (projectRoot && targetKey) {
    const defaultPath = path.join(path.resolve(projectRoot), '.docs-review-fix', 'targets', targetKey, 'MANIFEST.md');
