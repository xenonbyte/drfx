...
function collectSections(lines) {
  ...
  let current = null;
  const expectedOrder = ['Fixed:', 'Files changed:', 'Not fixed:', 'Residual risk:'];
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (line.endsWith(':') && !line.startsWith('- ') && !line.startsWith(' ')) {
      if (!SECTION_HEADINGS.has(line)) fail(`unknown section ${line.slice(0, -1)}`);
...
function parseFinalResponseBlock(text) {
  const lines = normalizePayload(text);
  ...
  if (parsed.filesChanged !== 'none' && parsed.filesChanged !== parsed.target) {
...
function readSemanticPayload({ filePath, projectRoot, content } = {}) {
  ...
  if (hasPathPart(fileReal, '.docs-review-fix')) {
  ...
  if (projectRoot) {
    ...
    if (isInsideOrEqual(fileReal, rootReal)) {
