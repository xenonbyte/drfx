...
//   Reviewed: true
//   Coverage risk: none
//   Review cache key: <64-char lowercase hex sha256, or "none">
//   Skipped:
//   - none
//   Extra reads:
//   - none
...
//   - "Review cache key:" is a scalar: 64-char lowercase hex, or the literal "none".
...
const UNIT_REVIEW_SCALARS = new Set(['Unit', 'Reviewed', 'Coverage risk', 'Review cache key']);
...
function parseUnitReviewReport(text) {
  ...
  if (!Object.hasOwn(scalars, 'Review cache key')) fail('missing required field: Review cache key');
  ...
  // Validate Review cache key: 64-char lowercase hex or "none".
  const rawCacheKey = scalars['Review cache key'];
  let reviewCacheKey;
  if (rawCacheKey === 'none') {
    reviewCacheKey = 'none';
  } else {
    reviewCacheKey = requirePattern(rawCacheKey, /^[0-9a-f]{64}$/, 'Review cache key');
  }
  ...
  function parseSkipped(entries) {
    ...
    return items.map((item) => {
      ...
      const skippedPath = redactText(pathPart.slice('path: '.length));
  ...
  function parseExtraReads(entries) {
    ...
    return items.map((item) => {
      ...
      const readPath = redactText(pathPart.slice('path: '.length));
      ...
      const contentId = redactText(idField.value);
