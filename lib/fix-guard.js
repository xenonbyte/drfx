...
function inspectStatusEntries({ projectRoot, targetPath, allowedStateDir, allowTarget }) {
  const root = path.resolve(projectRoot);
  const normalizedTarget = assertInsideProject(root, targetPath, 'target');
...
function checkGitRollbackAnchor({ projectRoot, targetPath }) {
  const root = path.resolve(projectRoot);
  let targetStats;
  try {
    targetStats = fs.statSync(targetPath);
  } catch (error) {
    throw guardError('rollback-unavailable', 'target is missing or unreadable', { cause: error });
  }
  if (!targetStats.isFile()) {
    throw guardError('rollback-unavailable', 'target must be a readable file');
  }
...
function checkTargetOnlyWorktree({ projectRoot, targetPath, allowedStateDir }) {
  try {
...
function inspectActualChangedFiles({ projectRoot, targetPath, allowedStateDir }) {
  try {
    return inspectStatusEntries({
      projectRoot,
      targetPath,
