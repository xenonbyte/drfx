...
// allowed set. Unit-testable in isolation. The SECURITY-relevant caller is
// checkFileSetWorktree, which feeds it REAL git status — never trust injected changedPaths
// for the live safety decision.
function classifyFileSetChanges({ changedPaths = [], allowedSet = [], allowedStateRelative = null }) {
...
// the allowed/route-owned set is allowed (route-owned prior-round changes); any file
// OUTSIDE it blocks (unexpected-worktree-change). git failure => blocked with the
// guard-unavailable reason, NEVER a silent pass.
function checkFileSetWorktree({ projectRoot, allowedFiles, allowedStateDir = null }) {
  const root = path.resolve(projectRoot);
  ...
  const classified = classifyFileSetChanges({ changedPaths, allowedSet: allowed, allowedStateRelative: stateRelative });
