...
// Registry: seven routes in canonical order (four document routes, then pr, code, r2q).
//
// `routeKind` and `targetContextKind` are deliberately separate axes. For the four
// document routes and pr/code they co-vary 1:1; r2q is the first route that mixes them
// (routeKind/targetContextKind 'r2q' resolves a file-set context, yet documentType PLAN
...
const ROUTE_LIST = Object.freeze([
  ...
  // review-fix-r2q reviews an r2p requirement directory (the 07-plan.md anchor) and
  // fixes backward into its owning upstream docs (03–06). Although it resolves a
  // FILE-SET context (targetContextKind 'r2q', routeKind 'r2q'), it is a DOCUMENT-rubric
  // route: documentType PLAN maps it to the COMMON+PLAN document rule stack, NOT the
  // PR/CODE route-rule stack. Its file-set guard defaults to snapshot (run.md is a
