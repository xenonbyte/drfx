...
// review-fix-r2q parser. The target is an r2p requirement DIRECTORY (not a .md
// file): bare path is shorthand for target=<requirement-dir>. r2q has a fixed PLAN
// rubric and a file-set guard whose DEFAULT is `snapshot` (not the shared `git`).
// It accepts only the shared flags and rejects the document-route surface
// (ref=/strict|normal/assurance=/ledger=) and the file-set surface (scope=/base=).
function parseInvocationR2q(entrySkill, tokens, options) {
  const includeMetadata = Boolean(options && options.includeMetadata);
  const defaultMode = includeMetadata ? options.defaultMode || 'review-and-fix' : 'review-and-fix';
  if (includeMetadata && !MODE_FLAGS.has(defaultMode)) fail('ERR_DEFAULT_MODE', `Invalid default mode: ${defaultMode}`);
  ...
  const result = {
    ...
    ledger: null,
    root: null,
    // r2q overrides the shared DEFAULT_GUARD_MODE: its file-set guard defaults to snapshot.
    guardMode: 'snapshot',
    roundLimit: null
  };
  ...
  for (const token of tokens) {
    ...
    if (pair) {
      ...
      if (key === 'target') {
        if (sawTarget || sawBareTarget) fail('ERR_DUPLICATE_TARGET', 'Duplicate target requirement directory');
        sawTarget = true;
        result.target = value;
        continue;
      }

      // Reject document-only and file-set-only key=value tokens explicitly.
      if (
        key === 'ref' || key === 'ledger' || key === 'assurance' ||
        key === 'type' || key === 'scope' || key === 'base'
      ) {
        fail('ERR_UNKNOWN_TOKEN', `Unknown token for review-fix-r2q: ${token}`);
      }
  ...
  if (!result.target) fail('ERR_MISSING_TARGET', 'Missing target requirement directory');
...
function parseInvocation(entrySkill, tokens, options = null) {
  ...
  if (routeKind === 'r2q') return parseInvocationR2q(entrySkill, tokens, options);
