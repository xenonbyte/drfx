...
// review-fix-r2p parser. The target is an r2p requirement DIRECTORY (not a .md
// file): bare path is shorthand for target=<requirement-dir>. r2p has a fixed PLAN
...
function parseInvocationR2p(entrySkill, tokens, options) {
  ...
  const result = {
    ...
    ledger: null,
    root: null,
    // r2p overrides the shared DEFAULT_GUARD_MODE: its file-set guard defaults to snapshot.
    guardMode: 'snapshot',
    roundLimit: null
  };
  ...
  for (const token of tokens) {
    ...
    if (pair) {
      ...
      if (
        ...
        fail('ERR_UNKNOWN_TOKEN', `Unknown token for review-fix-r2p: ${token}`);
      }
