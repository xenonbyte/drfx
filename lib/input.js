 ...
 */
function parseSharedCodeRouteToken(token, state, result, options) {
...
function parseInvocationPr(entrySkill, tokens, options) {
  ...
  for (const token of tokens) {
    ...
    if (pair) {
      ...
      if (parseSharedCodeRouteToken(token, state, result, { includeMetadata })) continue;
    ...
    if (!parseSharedCodeRouteToken(token, state, result, { includeMetadata })) {
      if (token.startsWith('-')) fail('ERR_UNKNOWN_TOKEN', `Unknown token: ${token}`);
      fail('ERR_UNKNOWN_TOKEN', `Unknown token: ${token}`);
    }
  }
