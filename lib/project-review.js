...
// suggestedRefs per non-chunk unit, and any freshly split chunk units for changed
// oversize_chunk sources (both need caller IO). This function only validates
...
function refreshPartitionPlanContent(oldPlan, newInventory, { nextSuggestedRefsByUnit = {}, nextOversizeChunksByPath, projectReviewFingerprint } = {}) {
  ...
  const usedUnitIds = new Set(
    oldPlan.units
      .filter((unit) => !(unit.oversize_chunk === true && changedChunkSources.has(unit.sourcePath)))
