...
// inventory, the live projectReviewFingerprint (F1), the re-resolved
// suggestedRefs per normal unit, and any freshly split chunk units for changed
// oversize sources (both need caller IO). This function only validates
// membership/bucket stability and replaces content-derived fields.
// Membership/bucket/refs drift throws — never a silent reuse of stale unit content.
// ---------------------------------------------------------------------------
...
function unitIdNumber(unitId) {
  const match = /^unit-(\d+)$/.exec(String(unitId || ''));
  return match ? Number(match[1]) : 0;
}

function nextUnusedUnitId(usedUnitIds) {
  let next = 0;
  for (const unitId of usedUnitIds) next = Math.max(next, unitIdNumber(unitId));
  let candidate;
  do {
    next += 1;
    candidate = formatUnitId(next);
  } while (usedUnitIds.has(candidate));
  usedUnitIds.add(candidate);
  return candidate;
}

function chunkOverrideFor(nextOversizeChunksByPath, sourcePath) {
  if (!nextOversizeChunksByPath) return undefined;
  if (nextOversizeChunksByPath instanceof Map) return nextOversizeChunksByPath.get(sourcePath);
  if (Object.prototype.hasOwnProperty.call(nextOversizeChunksByPath, sourcePath)) {
    return nextOversizeChunksByPath[sourcePath];
  }
  return undefined;
}

function normalizeRefreshedChunkUnit(chunk, { unitId, sourcePath, sourceContentId, chunkIndex, chunkCount }) {
  const files = (Array.isArray(chunk.files) ? chunk.files : []).map((file) => ({
    ...file,
    path: file.path || sourcePath,
    unit_id: unitId,
  }));
  if (files.length === 0) {
    throw partitionError(ERR_PARTITION_REBUCKET_REQUIRED, `refreshed chunk source produced an empty chunk: ${sourcePath}`);
  }
  const member_bytes = Number.isFinite(Number(chunk.member_bytes))
    ? Number(chunk.member_bytes)
  ...
  return {
    ...
    member_bytes,
    member_digest: typeof chunk.member_digest === 'string' && chunk.member_digest
      ? chunk.member_digest
      : computeMemberDigest(files),
    suggestedRefs: [],
  };
}

function assignRefreshedChunkUnits({ oldChunkUnits, nextChunks, sourcePath, sourceContentId, usedUnitIds }) {
  if (!Array.isArray(nextChunks) || nextChunks.length === 0) {
    throw partitionError(ERR_PARTITION_REBUCKET_REQUIRED, `oversize chunk source is no longer chunkable: ${sourcePath}`);
  }
  const chunkCount = nextChunks.length;
  return nextChunks.map((chunk, index) => {
    const reusableUnitId = oldChunkUnits[index] && oldChunkUnits[index].unit_id;
    const unitId = reusableUnitId && !usedUnitIds.has(reusableUnitId)
      ? reusableUnitId
      : nextUnusedUnitId(usedUnitIds);
    usedUnitIds.add(unitId);
    return normalizeRefreshedChunkUnit(chunk, {
      unitId,
      sourcePath,
      sourceContentId,
      chunkIndex: index,
...
function refreshPartitionPlanContent(oldPlan, newInventory, { nextSuggestedRefsByUnit = {}, nextOversizeChunksByPath, projectReviewFingerprint } = {}) {
  ...
  const chunkUnitsBySourcePath = new Map();
  for (const unit of oldPlan.units) {
    if (unit.oversize_chunk === true) {
      const source = byPath.get(unit.sourcePath);
      if (!source) {
        throw partitionError(ERR_PARTITION_MEMBERSHIP_CHANGED, `oversize chunk source missing from inventory: ${unit.sourcePath}`);
      }
      if (!chunkUnitsBySourcePath.has(unit.sourcePath)) chunkUnitsBySourcePath.set(unit.sourcePath, []);
      chunkUnitsBySourcePath.get(unit.sourcePath).push(unit);
    }
  }
  const changedChunkSources = new Set();
  for (const [sourcePath, chunkUnits] of chunkUnitsBySourcePath.entries()) {
    const source = byPath.get(sourcePath);
    if (String(source.contentId) !== String(chunkUnits[0].sourceContentId)) {
      const nextChunks = chunkOverrideFor(nextOversizeChunksByPath, sourcePath);
      if (!Array.isArray(nextChunks) || nextChunks.length === 0) {
        throw partitionError(ERR_PARTITION_MEMBERSHIP_CHANGED, `oversize chunk source changed; re-split required: ${sourcePath}`);
      }
      changedChunkSources.add(sourcePath);
    }
  }
  const changedLegacyOversizeSources = new Set();
  const legacyOversizeUnitBySourcePath = new Map();
  for (const unit of oldPlan.units) {
    if (unit.oversize_file !== true) continue;
    const sourcePath = unit.files[0].path;
    const source = byPath.get(sourcePath);
    if (!source) {
      throw partitionError(ERR_PARTITION_MEMBERSHIP_CHANGED, `oversize unit source missing from inventory: ${sourcePath}`);
    }
    if (String(source.contentId) === String(unit.files[0].contentId)) continue;
    if (Number(source.size) <= unitByteBudget) continue;
    const nextChunks = chunkOverrideFor(nextOversizeChunksByPath, sourcePath);
    if (!Array.isArray(nextChunks) || nextChunks.length === 0) {
      throw partitionError(ERR_PARTITION_MEMBERSHIP_CHANGED, `changed oversize blocker source requires re-split: ${sourcePath}`);
    }
    changedLegacyOversizeSources.add(sourcePath);
    legacyOversizeUnitBySourcePath.set(sourcePath, unit);
  }
  const usedUnitIds = new Set(
    oldPlan.units
      .filter((unit) => {
        if (unit.oversize_chunk === true && changedChunkSources.has(unit.sourcePath)) return false;
        if (unit.oversize_file === true && changedLegacyOversizeSources.has(unit.files[0].path)) return false;
        return true;
      })
      .map((unit) => unit.unit_id)
  );
  const refreshedChunkUnitsBySourcePath = new Map();
  for (const sourcePath of changedChunkSources) {
    const source = byPath.get(sourcePath);
    refreshedChunkUnitsBySourcePath.set(sourcePath, assignRefreshedChunkUnits({
      oldChunkUnits: chunkUnitsBySourcePath.get(sourcePath),
      nextChunks: chunkOverrideFor(nextOversizeChunksByPath, sourcePath),
      sourcePath,
      sourceContentId: source.contentId,
      usedUnitIds,
    }));
  }
  for (const sourcePath of changedLegacyOversizeSources) {
    const source = byPath.get(sourcePath);
    refreshedChunkUnitsBySourcePath.set(sourcePath, assignRefreshedChunkUnits({
      oldChunkUnits: [legacyOversizeUnitBySourcePath.get(sourcePath)],
      nextChunks: chunkOverrideFor(nextOversizeChunksByPath, sourcePath),
      sourcePath,
      sourceContentId: source.contentId,
      usedUnitIds,
  ...
  const emittedChangedChunkSources = new Set();
  const units = [];
  for (const unit of oldPlan.units) {
    if (unit.oversize_chunk === true) {
      if (changedChunkSources.has(unit.sourcePath)) {
        if (!emittedChangedChunkSources.has(unit.sourcePath)) {
          units.push(...refreshedChunkUnitsBySourcePath.get(unit.sourcePath));
          emittedChangedChunkSources.add(unit.sourcePath);
        }
        continue;
      }
      units.push({ ...unit });
    ...
    if (unit.oversize_file === true) {
      const sourcePath = unit.files[0].path;
      if (changedLegacyOversizeSources.has(sourcePath)) {
        units.push(...refreshedChunkUnitsBySourcePath.get(sourcePath));
        continue;
      }
      // Legacy oversize blocker unit: unchanged single member, refresh its content row only.
      ...
      units.push({ ...unit, files, member_bytes: row.size, member_digest: computeMemberDigest(files) });
      continue;
    }
    ...
    units.push({ ...unit, files, member_bytes, member_digest: computeMemberDigest(files), suggestedRefs: nextRefs });
  }
...
// exceeds the budget, or the optional chunk-count cap is exceeded, the file is
// unsplittable -> null (caller keeps the legacy oversize blocker).
function computeOversizeChunks({
  text,
  chunkLines = CHUNK_LINES,
  overlapLines = CHUNK_OVERLAP_LINES,
  chunkByteBudget = MAX_UNIT_BYTES,
  maxChunks = Infinity,
}) {
  ...
  const chunkCountCap = Number.isFinite(maxChunks) ? Math.max(0, Math.floor(maxChunks)) : Infinity;
  ...
  while (primaryStart <= total) {
    if (chunks.length >= chunkCountCap) return null;
