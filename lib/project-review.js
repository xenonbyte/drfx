...
function suggestRefsFor(unitFiles, inRootSet) {
  ...
  for (const file of unitFiles) {
    ...
    for (const spec of specifiers) {
      ...
      // The specifier may or may not include an extension. Try:
      // 1. exact path as-is
      // 2. with .js appended
      // 3. with .ts appended
      // 4. /index.js appended (directory imports)
      let contentId = inRootSet.get(resolved);
      if (contentId === undefined) {
        contentId = inRootSet.get(resolved + '.js');
        if (contentId !== undefined) resolved = resolved + '.js';
      }
      if (contentId === undefined) {
        contentId = inRootSet.get(resolved + '.ts');
        if (contentId !== undefined) resolved = resolved + '.ts';
      }
      if (contentId === undefined) {
        contentId = inRootSet.get(resolved + '/index.js');
        if (contentId !== undefined) resolved = resolved + '/index.js';
      }

      if (contentId === undefined) continue; // not in root
...
function aggregate(summaries, findings) {
  // --- Dedup findings by (location, category) using first-occurrence wins ---
  const dedupedFindings = [];
  const findingKeys = new Set();
  for (const f of findings) {
    const key = `${f.location}\0${f.category}`;
  ...
  const pass = allNoneCoverage && noOpenHighMedium;
