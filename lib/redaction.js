...
const CREDENTIAL_ASSIGNMENT_PATTERN = /((?:"|')?\b(?:user(?:name)?|login|password|passwd|pwd|secret|client[_-]?secret)\b(?:"|')?\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\s|,;]+)/gi;
...
const CREDENTIAL_KEY_PATTERN = /^(?:user(?:name)?|login|password|passwd|pwd|secret|client[_-]?secret)$/i;
