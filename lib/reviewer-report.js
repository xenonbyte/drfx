...
const OPTIONAL_FIELDS = Object.freeze(['sensitive', 'owner_stage', 'reason', 'required_action']);
const REQUIRED_FIELD_ORDER = Object.freeze([...REQUIRED_FIELDS]);
...
const ALLOWED_FIELDS = new Set([...REQUIRED_FIELD_ORDER, ...OPTIONAL_FIELDS, ...Object.keys(ALIASES)]);
...
function parseFinding(lines, startIndex, warnings, seenIds) {
  ...
  let redacted = false;
  let rawIndex = 0;
  for (let expectedPosition = 0; expectedPosition < REQUIRED_FIELD_ORDER.length; expectedPosition += 1, rawIndex += 1) {
    if (rawIndex >= rawFields.length) fail(`missing required field ${REQUIRED_FIELD_ORDER[expectedPosition]}`);
    const raw = rawFields[rawIndex];
    const normalized = normalizeFieldName(raw.name, warnings, finding.id);
    const expected = REQUIRED_FIELD_ORDER[expectedPosition];
    ...
    const value = ['id', 'severity', 'confidence'].includes(normalized)
      ? raw.value
      : redactText(raw.value).value;
    if (!['id', 'severity', 'confidence'].includes(normalized)) {
      redacted = redacted || value !== raw.value;
    }
    finding[normalized] = value;
  }

  for (; rawIndex < rawFields.length; rawIndex += 1) {
    const raw = rawFields[rawIndex];
    const normalized = normalizeFieldName(raw.name, warnings, finding.id);
    if (!OPTIONAL_FIELDS.includes(normalized)) fail(`unknown field ${normalized}`);
    if (Object.hasOwn(finding, normalized)) fail(`duplicate field ${normalized}`);
    const value = normalized === 'sensitive'
      ? raw.value
      : redactText(raw.value).value;
