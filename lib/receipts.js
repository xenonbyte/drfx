...
function writeExistingRoundReceipt(options, receiptPath) {
  const safeReceiptPath = validateReceiptPath(options, receiptPath);
  let stats;
  try {
    stats = fs.lstatSync(safeReceiptPath);
  } catch (error) {
    if (error && error.code === 'ENOENT') {
      const missing = new Error('reserved receipt path is missing');
      missing.code = 'ERR_RECEIPT_WRITE_FAILED';
      throw missing;
    }
    throw error;
  }
  if (!stats.isFile()) {
    const invalid = new Error('reserved receipt path must be a regular file');
    invalid.code = 'ERR_RECEIPT_WRITE_FAILED';
    throw invalid;
  }
  const text = redactReceiptText(formatRoundReceipt({
    ...options,
  ...
  atomicWriteFile(safeReceiptPath, text);
  return safeReceiptPath;
}
