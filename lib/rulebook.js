...
function validateRulesDirectory(rulesDir) {
  ...
  for (const entry of fs.readdirSync(rulesDir, { withFileTypes: true })) {
    if (entry.name.endsWith('.md') && !ALLOWED_RULE_FILE_SET.has(entry.name)) {
      fail('ERR_UNKNOWN_CUSTOM_RULE_FILE', `Unknown custom rule file: ${path.join(rulesDir, entry.name)}`);
    }
  }
  return true;
}

function loadRuleRoot({ root, rootKind, documentType }) {
  const loaded = {};
  const contentPaths = [];
  if (!root) return { rules: loaded, contentPaths };

  const rulesDir = path.join(root, '.docs-review-fix', 'rules');
  if (!validateRulesDirectory(rulesDir)) return { rules: loaded, contentPaths };
...
function loadCustomRuleFiles({ projectRoot = null, documentType, homeDir = null } = {}) {
  validateDocumentType(documentType);
  ...
  const userLoaded = loadRuleRoot({ root: userHome, rootKind: 'user', documentType });
  const projectLoaded = loadRuleRoot({ root: projectRoot, rootKind: 'project', documentType });
