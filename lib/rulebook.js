...
function readRouteRuleFile(root, routeKind, rootKindName = 'project') {
  if (!root) return null;
  assertNoLegacyRulebook(root, rootKindName);
  const rulesDir = path.join(root, '.drfx', 'rules');
  if (!validateRulesDirectory(rulesDir, { strictness: 'strict', warnings: [] })) return null;
 ...
 * @returns {{ layers: Array<{name: string, text: string}> }}
 */
function loadRouteRuleContext({
  ...
  if (resolvedHomeDir !== null && resolvedHomeDir !== undefined) {
    resolvedUserRules = readRouteRuleFile(resolvedHomeDir, routeKind, 'user');
  }
  if (projectRoot !== null && projectRoot !== undefined) {
    resolvedProjectRules = readRouteRuleFile(projectRoot, routeKind, 'project');
  }
