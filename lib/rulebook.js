...
function readRouteRuleFile(root, routeKind) {
  if (!root) return null;
  const filePath = path.join(root, '.docs-review-fix', 'rules', routeRuleFileName(routeKind));
...
function loadRouteRuleContext({
  ...
  if (homeDir !== null && homeDir !== undefined) {
    resolvedUserRules = readRouteRuleFile(homeDir, routeKind);
  }
  if (projectRoot !== null && projectRoot !== undefined) {
    resolvedProjectRules = readRouteRuleFile(projectRoot, routeKind);
  }
