...
const ROUTE_RULE_FILENAMES = Object.freeze(['PR.md', 'CODE.md']);
const ALLOWED_RULE_FILENAMES = Object.freeze([
  ...CANONICAL_SECTIONS.map((section) => `${section}.md`),
...
const ROUTE_KIND_SET = new Set(['pr', 'code']);
...
function validateRulesDirectory(rulesDir, { strictness = 'strict', warnings = [] } = {}) {
  ...
  for (const entry of fs.readdirSync(rulesDir, { withFileTypes: true })) {
    ...
    // Type-check every .md entry regardless of name: an unknown-named .md must
    // not bypass the symlink / regular-file guards just because normal policy
    // downgrades the unknown name to a warning instead of an error.
...
function loadRuleRoot({ root, rootKind, documentType, strictness, warnings }) {
  ...
  const rulesDir = path.join(root, '.drfx', 'rules');
...
function assertNoLegacyRulebook(root, rootKind) {
  if (!root) return;
  const legacyPath = path.join(root, '.drfx', 'RULE.md');
...
function validateRouteKind(routeKind) {
  if (!ROUTE_KIND_SET.has(routeKind)) {
    fail('ERR_UNKNOWN_ROUTE_KIND', `Unknown route kind: ${routeKind}`);
  }
}

function routeRuleFileName(routeKind) {
  validateRouteKind(routeKind);
  return `${routeKind.toUpperCase()}.md`;
}

function readRouteRuleFile(root, routeKind, rootKindName = 'project', warnings = []) {
  if (!root) return null;
  assertNoLegacyRulebook(root, rootKindName);
  const rulesDir = path.join(root, '.drfx', 'rules');
  // PR/CODE routes expose no strict|normal token; unknown .md files warn and
  // continue (D1) rather than block. Symlink/non-file are still hard errors.
  if (!validateRulesDirectory(rulesDir, { strictness: 'normal', warnings })) return null;
  const filePath = path.join(rulesDir, routeRuleFileName(routeKind));
  return readRuleFileIfPresent(filePath);
}

/**
 * Load the 4-layer rule context for a route kind (pr or code).
 * Layer order: workflow-hard-constraints → built-in-<routeKind>-rubric
 *              → user-global-<routeKind>-rules → project-local-<routeKind>-rules
 * NO COMMON layer. Hard constraints are non-overridable.
 *
 * @param {object} opts
 * @param {string} opts.routeKind - 'pr' or 'code'
 * @param {string} opts.builtInRubric - built-in rubric text (content of shared/rubrics/<routeKind>.md)
 * @param {string|null} [opts.userRules] - user-global rule text or null (pre-loaded)
 * @param {string|null} [opts.projectRules] - project-local rule text or null (pre-loaded)
 * @param {string|null} [opts.homeDir] - home directory for filesystem-based loading
 * @param {string|null} [opts.projectRoot] - project root for filesystem-based loading
 * @returns {{ layers: Array<{name: string, source: string, text: string}>, warnings: Array<{code: string, message: string, filePath: string}> }}
 */
function loadRouteRuleContext({
  routeKind,
  builtInRubric,
  userRules,
  projectRules,
  ...
  // If filesystem roots are provided, load from disk (overrides pre-loaded text args)
  let resolvedUserRules = userRules !== undefined ? userRules : null;
  let resolvedProjectRules = projectRules !== undefined ? projectRules : null;
  const resolvedHomeDir = homeDir !== undefined
    ? homeDir
    : (userRules === undefined ? process.env.HOME || null : null);

  const warnings = [];
  if (resolvedHomeDir !== null && resolvedHomeDir !== undefined) {
    resolvedUserRules = readRouteRuleFile(resolvedHomeDir, routeKind, 'user', warnings);
  }
  if (projectRoot !== null && projectRoot !== undefined) {
    resolvedProjectRules = readRouteRuleFile(projectRoot, routeKind, 'project', warnings);
  }

  const layers = [];
  // Each layer carries both `name` (the ordered route-layer contract) and `source`
  // (so the shared ruleSourceCategory/ruleSourceEntry helpers can consume route layers
  // the same way they consume document mergeRules layers).
  const pushLayer = (name, text) => layers.push({ name, source: name, text });

  // Layer 0: workflow hard constraints (always present, non-overridable)
  pushLayer('workflow-hard-constraints', HARD_CONSTRAINTS);

  // Layer 1: built-in route rubric
  if (builtInRubric !== null && builtInRubric !== undefined && builtInRubric !== '') {
    pushLayer(`built-in-${routeKind}-rubric`, String(builtInRubric));
  }

  // Layer 2: user-global route rules (custom — check hard constraints)
  if (resolvedUserRules !== null && resolvedUserRules !== undefined && String(resolvedUserRules).trim() !== '') {
    assertNoHardConstraintConflict(resolvedUserRules);
    pushLayer(`user-global-${routeKind}-rules`, String(resolvedUserRules).trim());
  }

  // Layer 3: project-local route rules (custom — check hard constraints)
  if (resolvedProjectRules !== null && resolvedProjectRules !== undefined && String(resolvedProjectRules).trim() !== '') {
    assertNoHardConstraintConflict(resolvedProjectRules);
    pushLayer(`project-local-${routeKind}-rules`, String(resolvedProjectRules).trim());
  }
