...
const PACKAGE_NAME = '@xenonbyte/document-review-loop';
...
function platformInvocationText(platform, route) {
  if (platform === 'claude') {
    return [
      ...
      '`read-only` or `review-and-fix`, optional `resume`, optional `ledger=<target-local path>`, and optional `root=<project-root>`.'
    ].join('\n');
  }
  if (platform === 'codex') {
    return [
  ...
  if (platform === 'gemini') {
    return [
      'Use this Gemini command with `target=<path>`, optional repeated `ref=<path>`, `strict` or `normal`, optional `resume`,',
      'optional `ledger=<target-local path>`, and optional `root=<project-root>`.',
