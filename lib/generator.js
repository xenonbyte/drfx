...
const { getRouteDescriptor, listDocumentRoutes } = require('./routes');
...
function routeFor(routeName) {
  // Delegate to the shared registry but restrict to document routes for generation.
  // getRouteDescriptor throws ERR_ROUTE for completely unknown names.
  const descriptor = getRouteDescriptor(routeName);
  if (descriptor.routeKind !== 'document') fail('ERR_ROUTE', `unsupported route for generation: ${routeName}`);
  return descriptor;
}
...
function generatePlatformFiles(platform, options = {}) {
  const normalizedPlatform = normalizePlatform(platform);
  const packageVersion = options.packageVersion || readPackageVersion();
  const routes = listDocumentRoutes();
