...
function platformInvocationText(platform, route) {
  if (platform === 'claude') {
    return [
      'Use this Claude Code command with `target=<path>`, optional repeated `ref=<path>`, `strict` or `normal`,',
      'optional `read-only` or `review-and-fix`, optional `assurance=practical|strict-verified|advisory`,',
      'optional `resume`, optional `ledger=<target-local path>`, optional `root=<project-root>`, and optional `debug`.',
  ...
  if (platform === 'gemini') {
    return [
      ...
      'optional `read-only`, optional `assurance=advisory`, optional `root=<project-root>`, and optional `debug`.',
