...
// prior single-target guard wording; PR/CODE describe the file-set guard (whole-worktree
// git status with in-set-only changes, or snapshot rollback anchor).
function guardWriteRequirementFor(route) {
  if (route.routeKind === 'pr' || route.routeKind === 'code') {
    return 'Automatic file-set writes require `review-and-fix` and a selected guard mode: use `guard=git` with a worktree whose changes stay inside the resolved file set, or `guard=snapshot` with a valid snapshot rollback anchor. The file-set guard must remain available and parseable before and after writes.';
  }
