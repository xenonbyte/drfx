...
function partitionedReviewFlowFor(platform, route) {
  ...
  return [
    ...
    '6. After all units and backstops are recorded, run `drfx workflow aggregate-review <targetStateDir> --json`. Treat the aggregate verdict as authoritative: only `verdict: PASS` may proceed to finalization, and `reason: coverage-incomplete` or any forced re-read finding must be resolved before claiming workflow PASS.',
    ''
  ].join('\n');
}
