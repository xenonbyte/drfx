...
function partitionedReviewFlowFor(platform, route) {
  ...
  return [
    ...
    '7. After all units and backstops are recorded, run `drfx workflow aggregate-review <targetStateDir> --json`. Treat the aggregate verdict as authoritative: only `verdict: PASS` may proceed to finalization. If `reason: coverage-incomplete`, review the uncovered units/backstops. If aggregate returns `stopped-with-deferrals` with a reviewer report path, record-triage the accepted findings and run `begin-fix`; after partitioned `end-fix` returns `reviewMode: partitioned`, do not run the generic `record-diff-review` step. Instead, return to step 1 so only the affected units and backstops are re-reviewed before re-aggregating.',
    ''
  ].join('\n');
}
