...
function reviewAndFixFlowFor(platform, route) {
  ...
  if (route.routeKind === 'r2p') {
    return [
      ...
      `4. Triage every finding semantically and submit the triage with \`drfx workflow record-triage ${routeName} ${targetToken} review-and-fix --assurance practical --runtime-platform ${runtimePlatform} --runtime-subagent-probe ready --runtime-stdin-handoff ready --runtime-downgrade-reason none --triage-stdin --json=compact\`.`,
      '5. If accepted or reopened blocking issues remain, run `drfx workflow record-r2p-repair-plan <targetStateDir> --json=compact`.',
      '6. Apply the validated repair command with `drfx workflow apply-r2p-repair <targetStateDir> --json=compact`. Direct artifact writes are forbidden: do not run `begin-fix`, `refresh-lock`, `end-fix`, `abort-fix`, or `record-diff-review` for r2p.',
      `7. After \`apply-r2p-repair\`, stop at checkpoint. Tell the user to run \`r2p-continue\`, let r2p regenerate artifacts, then rerun \`${routeName} workId=<new-or-same-WF-...>\`.`,
      '8. If no accepted blocking issues remain after triage, finalize through `drfx workflow finalize <targetStateDir> --final-response-stdin --json=compact`. PASS is allowed only when this rerun is clean and no repair command ran in the current round.'
    ].join('\n');
  }
