...
function routeFor(routeName) {
  // Delegate to the shared registry. All seven route kinds (document/pr/code/r2q) are
...
function targetTokenFor(route) {
  ...
  if (route.routeKind === 'r2q') return 'target=<requirement-dir>';
  return 'target=<path>';
}
...
function runtimeFlagsContent(platform, route) {
  const template = readText(path.join('shared', route.routeKind === 'r2q' ? 'runtime-flags.r2q.md' : 'runtime-flags.md'));
...
function advisoryProbeSkipTextFor(route) {
  if (route.routeKind === 'r2q') {
    return 'r2q exposes no advisory assurance token. A materialized read-only r2q flow uses the practical no-state path unless an internal strict-verified state-backed path is selected.';
  }
...
function strictVerifiedIntroTextFor(route) {
  if (route.routeKind === 'r2q') {
    return 'Only the internal same-flow strict proof branch may use `strict-verified` for r2q. Do not accept a user `assurance=` token to request it. In that branch, run:';
  }
...
function invocationGrammarFor(platform, route) {
  ...
  if (route.routeKind === 'r2q') {
    // r2q reviews an r2p requirement directory (07-plan.md anchor) and has a fixed
...
function routeSummaryFor(route) {
  ...
  if (route.routeKind === 'r2q') return 'r2p requirement plans (07-plan.md)';
...
function metadataTypeFor(route) {
  ...
  if (route.routeKind === 'r2q') return 'review_target = "r2p-requirement"';
...
function defaultGuardFor(route) {
  return route.routeKind === 'r2q' ? 'snapshot' : 'git';
}
...
function platformInvocationText(platform, route) {
  ...
  if (route.routeKind === 'r2q') {
    return r2qInvocationText(platform, route);
  }
  if (platform === 'claude') {
    return [
...
// Top-of-route usage prose for review-fix-r2q. r2q reviews an r2p requirement
...
// advisory-only.
function r2qInvocationText(platform, route) {
  const name = route.routeName;
  if (platform === 'gemini') {
    return [
...
function generatePlatformFiles(platform, options = {}) {
  const normalizedPlatform = normalizePlatform(platform);
  const packageVersion = options.packageVersion || readPackageVersion();
  // Emit all seven routes (four document routes, then pr, then code, then r2q).
