...
const RESERVED_DIRECTORIES = new Set(['LOCK', 'stale-locks', 'rounds', 'context', 'reports']);
const ACTIVE_STATUSES = new Set(['review', 'triage', 'fix', 'diff-review', 'full-re-review']);
...
function findStateRoot(startPath) {
  return findAncestor(startPath, (candidate) => {
    const stateDir = path.join(candidate, '.drfx');
    return (
      fs.existsSync(path.join(stateDir, 'rules')) ||
      fs.existsSync(path.join(stateDir, 'targets')) ||
      fs.existsSync(path.join(stateDir, 'RULE.md'))
    );
  });
}
...
function deriveR2pTargetKey({ projectRoot, workId }) {
  ...
  const slug = slugifyTarget(normalizedWorkId);
  const hash12 = crypto.createHash('sha256')
    .update(['r2p', rootRealPath, normalizedWorkId].join('\x1f'))
    .digest('hex')
    .slice(0, 12);
  return {
    slug,
...
function targetStateDir(projectRootRealPath, targetKey) {
  return path.join(projectRootRealPath, '.drfx', 'targets', targetKey);
}
...
function targetStateRelativeParts(relativePath, label) {
  ...
  if (parts.some((part) => RESERVED_BASENAMES.has(part))) {
    failStateValidation(`${label} must not target reserved state files`);
  }
  return parts;
}

function assertNoSymlinkInTargetStatePath(filePath, targetStateDirPath, label) {
  const absolute = path.resolve(filePath);
  const targetDir = path.resolve(targetStateDirPath);
  const relative = path.relative(targetDir, absolute);
  if (relative === '' || relative.startsWith('..') || path.isAbsolute(relative)) {
    failStateValidation(`${label} escapes target state directory`);
  }
  if (pathExists(targetDir) && fs.lstatSync(targetDir).isSymbolicLink()) {
    failStateValidation(`${label} target state directory must not be a symlink`);
  }
  ...
  for (const part of relative.split(path.sep).filter(Boolean)) {
    ...
    if (!pathExists(current)) return;
    if (fs.lstatSync(current).isSymbolicLink()) {
      failStateValidation(`${label} must not be a symlink or pass through one`);
    }
  }
}

function validateTargetStateOwnedPath({
  targetStateDir: targetStateDirPath,
  relativePath,
  allowedDirectories = [],
  label = 'state path'
}) {
  const parts = targetStateRelativeParts(relativePath, label);
  if (allowedDirectories.length > 0 && !allowedDirectories.includes(parts[0])) {
    failStateValidation(`${label} must be under ${allowedDirectories.join(', ')}`);
  }

  const targetDirPath = path.resolve(targetStateDirPath);
  const requestedPath = path.join(targetDirPath, ...parts);
  const lexicalRelative = path.relative(targetDirPath, requestedPath);
  if (lexicalRelative === '' || lexicalRelative.startsWith('..') || path.isAbsolute(lexicalRelative)) {
    failStateValidation(`${label} escapes target state directory`);
  }

  assertNoSymlinkInTargetStatePath(requestedPath, targetDirPath, label);

  const targetDirRealPath = pathExists(targetDirPath) ? fs.realpathSync.native(targetDirPath) : targetDirPath;
  const parentRealPath = fs.realpathSync.native(nearestExistingParent(requestedPath));
  if (pathExists(targetDirPath) && !isInsideOrEqual(parentRealPath, targetDirRealPath)) {
    failStateValidation(`${label} resolves outside target state directory`);
  }

  if (pathExists(requestedPath)) {
    const stats = fs.statSync(requestedPath);
    if (stats.isDirectory()) failStateValidation(`${label} must be a file path`);
    const realPath = fs.realpathSync.native(requestedPath);
    if (!isInsideOrEqual(realPath, targetDirRealPath)) {
      failStateValidation(`${label} resolves outside target state directory`);
    }
  }
...
function normalizeManifestV1(manifest) {
  return {
    ...manifest,
    manifestSchema: 1,
    guardMode: manifest.guardMode || 'git',
    assurance: manifest.assurance || 'advisory',
    assuranceProof: manifest.assuranceProof || 'none',
    runtimePlatform: manifest.runtimePlatform || 'manual',
    descriptorPlatform: manifest.descriptorPlatform || 'none',
    runtimeSubagentProbe: manifest.runtimeSubagentProbe || 'not-required',
    runtimeSubagentProbeEvidence: manifest.runtimeSubagentProbeEvidence || 'none',
    runtimeFingerprintGuard: manifest.runtimeFingerprintGuard || 'not-run',
    runtimeStdinHandoff: manifest.runtimeStdinHandoff || 'not-required',
    runtimeStdinHandoffEvidence: manifest.runtimeStdinHandoffEvidence || 'none',
    runtimeDowngradeReason: manifest.runtimeDowngradeReason || 'none',
    blockingReason: manifest.blockingReason || 'none',
    statusReason: manifest.statusReason || 'none',
    currentPhase: manifest.currentPhase || (ACTIVE_STATUSES.has(manifest.status) ? manifest.status : 'final')
  };
}

function readManifestAny(manifestPath) {
  const text = fs.readFileSync(manifestPath, 'utf8');
  const schemaMatch = text.match(/^Manifest schema:\s*(\S+)\s*$/m);
  if (!schemaMatch) return normalizeManifestV1(parseManifest(text));
  if (schemaMatch[1] === '2') {
    const { parseManifestV2 } = require('./workflow-state');
    return parseManifestV2(text);
  }
  fail('ERR_STATE_VALIDATION_FAILED', `state-validation-failed: unsupported Manifest schema: ${schemaMatch[1]}`);
}
