...
//
// NOTE: The exact run.md field formatting (## Status token values and
// ## Active Artifacts plan-stage tokens) is UNCONFIRMED — no checked-in
// run.md fixtures exist yet (Task 13 adds them). The parser uses the shapes
// documented in the approved design and will need a focused adjustment if
// the real token text differs. Mark any run.md fixture you create consistent
// with what parseRunMdGate accepts.
// ---------------------------------------------------------------------------
 ...
 * Returns empty string when the section is absent.
 */
 ...
 * tokens depend on the r2q workflow's run.md format, which has no checked-in
 * fixtures yet. Adjust the extraction predicates here (only) when Task 13
 * lands fixtures and confirms the exact text.
 */
...
function buildR2qIdentity({ context, guardMode, roundLimit } = {}) {
  ...
  const relativeRequirementDir = path.relative(context.projectRoot, context.requirementDir);
  return {
    targetContextKind: 'r2q',
    guardMode: String(guardMode),
    roundLimit: roundLimit === null || roundLimit === undefined ? 'none' : String(roundLimit),
