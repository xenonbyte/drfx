...
// ---------------------------------------------------------------------------
// User-level CODE exclusions (.drfxignore)
//
// A project may carry a `.drfxignore` file at the project root: one
// root-relative directory path per line; a line whose first non-blank
// character is `#` is a comment; blank lines are ignored. There are NO globs
// and NO negation — every entry is a plain directory path matched as a whole
// walk prefix. It applies to CODE traversal only (PR diffs and document
// routes never consult it), and the resolver never reads `.gitignore`.
// ---------------------------------------------------------------------------
const DRFXIGNORE_FILENAME = '.drfxignore';

// Parse .drfxignore content into ordered { raw, lineNumber } entries.
function parseDrfxignore(content) {
  const entries = [];
  const lines = String(content).split(/\r?\n/);
  for (let index = 0; index < lines.length; index += 1) {
    const raw = lines[index].trim();
    if (raw === '' || raw.startsWith('#')) continue;
    entries.push({ raw, lineNumber: index + 1 });
  }
  return entries;
}

// Strictly resolve one .drfxignore entry to a root-relative posix directory
// path. Mirrors resolveScope's checks, but reports exclude-specific errors
// that name the .drfxignore line so a stale entry is directly actionable.
function resolveUserExcludeEntry(rootRealPath, raw, lineNumber) {
  const at = `${DRFXIGNORE_FILENAME} line ${lineNumber}`;
  const absolute = path.resolve(rootRealPath, raw);
  const lexicalRelative = path.relative(rootRealPath, absolute);
  if (lexicalRelative.startsWith('..') || path.isAbsolute(lexicalRelative)) {
    fail('ERR_CODE_USER_EXCLUDE_OUTSIDE_ROOT', `${at}: entry resolves outside project root: ${raw}`);
  }
  let stats;
  try {
    stats = fs.lstatSync(absolute);
  } catch {
    fail('ERR_CODE_USER_EXCLUDE_MISSING', `${at}: entry path must exist: ${raw}`);
  }
  if (stats.isSymbolicLink()) {
  ...
  if (!fs.statSync(realPath).isDirectory()) {
    fail('ERR_CODE_USER_EXCLUDE_NOT_DIRECTORY', `${at}: entry must be a directory: ${raw}`);
  }
  const relative = path.relative(rootRealPath, realPath);
  if (relative === '') {
    fail('ERR_CODE_USER_EXCLUDE_ROOT', `${at}: entry must not exclude the project root: ${raw}`);
  }
...
// Load and resolve the project's .drfxignore entries (missing file ⇒ []).
// Strict mode (the resolver) fails loudly on any invalid entry. Lenient mode
// (identity-only key derivation) falls back to the trimmed raw entry instead
// of throwing, mirroring normalizeCodeScopesForIdentity: the invalid entry
// still yields a deterministic (throwaway) key while the resolver layer
// reports the entry error as the actionable failure.
function loadUserExcludes(rootRealPath, { lenient = false } = {}) {
  const ignorePath = path.join(rootRealPath, DRFXIGNORE_FILENAME);
  // Existence probe first: the common no-.drfxignore case must not read ANY
  // file content (the whole-root gate guarantees zero content reads before
  // blocking, and that guarantee is test-enforced).
  let configStats;
  try {
  ...
  // Reject a symlinked .drfxignore outright, matching the repo-wide symlink
  // posture (install preflight, uninstall, scope resolution all refuse
  // symlinks): review-narrowing config must live in the worktree, not behind
  // a link. Lenient (identity-only) mode must never throw — it returns a
  // deterministic empty list and the resolver reports the actionable error.
  ...
  for (const { raw, lineNumber } of parseDrfxignore(content)) {
    let relative;
    if (lenient) {
      try {
        relative = resolveUserExcludeEntry(rootRealPath, raw, lineNumber);
      } catch {
        relative = raw;
      }
    } else {
    ...
    if (relative !== '') resolved.add(relative);
  }
  return [...resolved].sort();
}

// Split user-exclude entries into the ACTIVE list (prunes the walk and enters
// the target identity) and the OVERRIDDEN list (entries that are, or contain,
// an explicit scope=: per-run explicit scope intent beats the durable config
// default, so those entries are inert for this run but still reported).
// Entries covered by a built-in exclusion, disjoint from every walk root, or
// nested inside another active entry never affect the file set and are
// dropped entirely so they cannot churn the target identity.
function partitionUserExcludes(entries, walkRoots) {
  const overridden = new Set();
  const candidates = new Set();
  for (const entry of entries) {
    if (entry.split('/').some((segment) => isExcludedBasename(segment))) continue;
    const coversScope = walkRoots.some(
      (walkRoot) => walkRoot !== '' && (entry === walkRoot || walkRoot.startsWith(`${entry}/`))
    );
    if (coversScope) {
      overridden.add(entry);
      continue;
    }
    const insideWalk = walkRoots.some(
      (walkRoot) => walkRoot === '' || entry.startsWith(`${walkRoot}/`)
    );
    if (insideWalk) candidates.add(entry);
  }
  // Lexicographic order puts an ancestor before its descendants, so nested
  // entries collapse onto the broadest active ancestor.
  const active = [];
  for (const entry of [...candidates].sort()) {
    if (active.some((kept) => entry === kept || entry.startsWith(`${kept}/`))) continue;
    active.push(entry);
  }
  return { active, overridden: [...overridden].sort() };
}
...
// entries to `out`. Excluded dirs (built-in basenames plus active .drfxignore
// prefixes) are pruned; symlinks are never followed (so an escaping link can
// never leak content outside the root); device / fifo / socket files are
// skipped.
function walkDirectory(rootRealPath, relativePrefix, out, wholeRootStats, userExcludeSet) {
  ...
  for (const dirent of dirents) {
    ...
    if (dirent.isDirectory()) {
      ...
      if (userExcludeSet && userExcludeSet.has(childRelative)) continue;
      walkDirectory(rootRealPath, childRelative, out, wholeRootStats, userExcludeSet);
      if (wholeRootStats && wholeRootStats.blocked) return;
 ...
 *   userExcludes, overriddenUserExcludes, files } or
...
async function resolveCodeTarget({ cwd, scopes } = {}) {
  ...
  // User-level .drfxignore exclusions: active entries prune the walk (and so
  // are counted BEFORE the whole-root cap); entries covering an explicit
  // scope= are overridden for this run and surfaced, never silently applied.
  const userExcludeEntries = loadUserExcludes(rootRealPath);
  const { active: userExcludes, overridden: overriddenUserExcludes } = partitionUserExcludes(
    userExcludeEntries,
    walkRoots
  );
  const userExcludeSet = new Set(userExcludes);

  const seen = new Set();
  const relativeFiles = [];
  for (const scopeRelative of walkRoots) {
    walkDirectory(rootRealPath, scopeRelative, relativeFiles, wholeRootStats, userExcludeSet);
...
function describeCodeBlock(context) {
  if (context && context.reason === 'file-set-too-large') {
    ...
    return {
      ...
      nextAction: `pass scope=<path> to narrow the review, or list directories in ${DRFXIGNORE_FILENAME} to shrink the whole-root file set`
    };
  }
...
// ACTIVE user-exclude list for the requested scopes. Lenient like
// normalizeCodeScopesForIdentity — an invalid .drfxignore entry must yield a
// deterministic (throwaway) key here, never an uncaught throw; the resolver
// layer reports the entry error as the actionable failure.
function normalizeCodeUserExcludesForIdentity({ cwd, scopes } = {}) {
  let rootRealPath;
  try {
    rootRealPath = fs.realpathSync.native(path.resolve(cwd || '.'));
  } catch {
  ...
  const normalizedScopes = normalizeCodeScopesForIdentity({ cwd, scopes });
  const walkRoots = normalizedScopes.length > 0 ? normalizedScopes : [''];
  const entries = loadUserExcludes(rootRealPath, { lenient: true });
  return partitionUserExcludes(entries, walkRoots).active;
}
...
function buildCodeIdentity({ context, guardMode, roundLimit } = {}) {
  ...
  return {
    ...
    userExcludes: sortedStringList(context.userExcludes),
    guardMode: String(guardMode),
    roundLimit: roundLimit === null || roundLimit === undefined ? 'none' : String(roundLimit),
    fileSetFingerprint: computeFileSetFingerprint(context.files)
  };
 ...
 * policy: they stay STRICT like scopes — an edited exclude list is a
 * different review target, never a silent resume. Lists compare in
 * normalized, order-stable form.
 */
