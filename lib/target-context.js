...
async function resolveTargetContext({ routeName, base, cwd, commandLog } = {}) {
  ...
  for (const entry of rawFiles) {
    ...
    // blob OID (40-hex); CODE routes (PLAN-TASK-004) reuse the same `contentId`
    // field with a content hash. For deletions, fall back to the base blob so
    // the deletion still contributes a stable, change-sensitive value.
    let contentId;
    if (entry.status === 'deleted') {
      contentId = await blobSha(mergeBase, entry.path, ctx);
    } else {
      contentId = await blobSha(head, entry.path, ctx);
    }
...
// target-state.js's RESERVED_DIRECTORIES (which names .docs-review-fix-internal
// subdirs): CODE excludes the whole `.docs-review-fix` state tree plus VCS,
...
const CODE_EXCLUDED_DIRECTORIES = Object.freeze(new Set([
  // VCS + this tool's own state tree
  '.git',
  '.hg',
  '.svn',
  '.docs-review-fix',
...
function walkDirectory(rootRealPath, relativePrefix, out) {
  ...
  for (const dirent of dirents) {
    ...
    if (dirent.isDirectory()) {
      ...
      walkDirectory(rootRealPath, childRelative, out);
...
async function resolveCodeTarget({ cwd, scopes } = {}) {
  ...
  for (const scope of requestedScopes) {
    ...
    // A scope that IS an excluded directory is refused before review starts.
    const firstSegment = relative.split('/')[0];
    if (relative !== '' && isExcludedBasename(firstSegment) && relative === firstSegment) {
      return { status: 'blocked', reason: 'excluded-scope', scope: firstSegment };
    }
    if (relative === '') {
      // The root itself collapses to whole-root traversal: no normalized scope.
      continue;
    }
  ...
  for (const scopeRelative of walkRoots) {
    walkDirectory(rootRealPath, scopeRelative, relativeFiles);
  }
 ...
 * STRICT equality on EVERY field including roundLimit and the scope/exclusion
 * lists; ANY drift makes the stored state stale. Lists compare in normalized,
 * order-stable form. Returns { match, mismatches:[...] }.
 */
