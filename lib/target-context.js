...
//   (c) projectReviewFingerprint computed over sorted contentIds (CONTROLLER DECISION)
//
...
//   projectReviewFingerprint = sha256( sortedContentIds.join('\n') )
// where sortedContentIds = inventory sorted by path → map to contentId → join
// with '\n'. Sort-first makes the fingerprint order-independent over the set;
// joining with '\n' is unambiguous because sha256 hex digests never contain '\n'.
// This style mirrors the existing `sha256(stableJson(canonical))` pattern in
// computeFileSetFingerprint but uses a simpler join for a list of fixed-length
// hex strings where stableJson overhead is unnecessary.
// ---------------------------------------------------------------------------
 ...
 * Algorithm: sort inventory by path, extract contentIds, join with '\n',
 * sha256 the result. Sorting makes the fingerprint order-independent over the
 * set (same logical tree ⇒ same fingerprint regardless of directory-read order).
 * @param {Array<{path:string, contentId:string}>} inventory
 * @returns {string} lowercase hex sha256
 */
function computeProjectReviewFingerprint(inventory) {
  const sorted = (Array.isArray(inventory) ? inventory : [])
    .slice()
    .sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
  const joined = sorted.map((row) => String(row.contentId)).join('\n');
...
async function resolveCodeInventory({ cwd, scopes, commandLog } = {}) {
  ...
  const { matcher: userMatcher } = loadUserExcludeMatcher(rootRealPath);
  ...
  const inventory = await Promise.all(
    sortedRelatives.map(async (relative) => {
