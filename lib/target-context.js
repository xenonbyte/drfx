...
async function resolveCodeTarget({ cwd, scopes } = {}) {
  ...
  // Whole-root means no narrowing scope survived normalization (omitted scope,
  // `scope=.`, a scope equal to the root, or a root+narrower combo all collapse
  // to an empty normalizedScopes here).
  if (normalizedScopes.length === 0) {
    // Second metadata-only pass: hashFileContent returns only the oid, and the
    // readdir walk carries no size, so byte totals are gathered here. Runs only
    // for whole-root and the files are already in the page cache from hashing.
    let totalBytes = 0;
    for (const file of files) {
      totalBytes += fs.statSync(path.join(rootRealPath, file.path)).size;
    }
    if (files.length > MAX_WHOLE_ROOT_FILES || totalBytes > MAX_WHOLE_ROOT_BYTES) {
      return { status: 'blocked', reason: 'file-set-too-large', fileCount: files.length, totalBytes };
    }
  }
