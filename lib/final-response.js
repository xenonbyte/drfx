...
function validateReadOnly(finalResponse, state) {
  ...
  if (state && state.persistent && READ_ONLY_FINAL_STATUSES.has(finalResponse.finalStatus)) {
    ...
    if (finalResponse.finalStatus === 'read-only-findings' && blockingIssueIds.length === 0) {
      fail('ERR_FINAL_READ_ONLY_FINDINGS_EMPTY', 'read-only-findings requires blocking findings or issues');
    }
  }
}
