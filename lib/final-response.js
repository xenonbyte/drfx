...
function validatePass(finalResponse, state) {
  ...
  // PASS is earned, never assumed: a pass is unconditional success, so it must
  // carry Status reason: none. Any durable status reason (e.g. coverage-incomplete,
  // round-limit, deferred-findings) is a deferral/pause signal and cannot coexist
  // with pass.
  if ((finalResponse.statusReason || 'none') !== 'none') {
    fail('ERR_FINAL_PASS_STATUS_REASON', 'pass requires Status reason: none');
  }
}
...
function validateReadOnly(finalResponse, state) {
  ...
  if (state && state.persistent && finalResponse.finalStatus === 'stopped-with-deferrals') {
    ...
    // durable status reason. A coverage-incomplete stop (PLAN-TASK-010) is the
    // partitioned-review aggregate outcome when some units could not be covered:
    // it has NO deferred reviewer issue id, and is instead gated on a coverage
    // deferral owner + next action carried in `Deferrals or blockers`. The
    // deferred-findings and round-limit reasons still require genuinely deferred
    // findings.
    if (!['deferred-findings', 'round-limit', 'coverage-incomplete'].includes(finalResponse.statusReason)) {
      fail(
        'ERR_FINAL_DEFERRED_STATUS_REASON',
        'stopped-with-deferrals requires Status reason: deferred-findings, round-limit, or coverage-incomplete'
      );
    }
    if (finalResponse.statusReason !== 'coverage-incomplete' && deferredIssueIds.length === 0) {
      fail('ERR_FINAL_DEFERRED_FINDINGS_EMPTY', 'stopped-with-deferrals requires deferred high/medium findings');
    }
    if (finalResponse.statusReason === 'coverage-incomplete' && (finalResponse.deferralsOrBlockers || 'none') === 'none') {
      fail(
        'ERR_FINAL_COVERAGE_DEFERRAL_MISSING',
        'coverage-incomplete requires a Deferrals or blockers entry with a coverage-deferral owner and next action'
      );
    }
  }
