...
function validateReadOnly(finalResponse, state) {
  ...
  if (state && state.persistent && stateMode === 'read-only' && finalResponse.finalStatus === 'stopped-with-deferrals') {
    const deferredIssueIds = normalizeIssueIds(state.deferredBlockingIssueIds || []);
    if (finalResponse.statusReason !== 'deferred-findings') {
      fail('ERR_FINAL_DEFERRED_STATUS_REASON', 'stopped-with-deferrals requires Status reason: deferred-findings');
    }
...
function validateResumeState({
  ...
  if (manifest.status === 'pass' && currentSha256 && currentSha256 !== manifest.lastPassedContentSha256) {
    return {
      ...manifest,
      status: 'review',
      currentPhase: 'review',
      ...
      lastKnownContentSha256: currentSha256,
      fileSize: currentSize || manifest.fileSize
    };
  }
