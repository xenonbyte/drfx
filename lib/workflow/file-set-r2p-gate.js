...
// once, at resolve time (resolveR2pTarget). But the fix lifecycle then writes and PASSes
...
const { parseRunMdGate, resolveR2pTarget } = require('../target-context');
...
function snapshotForceIncludeDirs(metadata) {
  if (metadata.routeKind !== 'r2p' || !metadata.manifest.requirementDir) return [];
  return [path.resolve(metadata.projectRoot, metadata.manifest.requirementDir)];
}

// Re-resolve the LIVE r2p file set from the durable manifest identity (read-only). r2p
...
// (doc-name-keyed) r2p fingerprint.
...
function revalidateR2pGate(metadata) {
  ...
  const requirementDir = metadata.manifest.requirementDir
    ...
    : null;
  if (!requirementDir) {
    return drift('r2p manifest is missing the requirement directory; cannot revalidate the run.md gate');
  }
