...
function findPriorR2pRepairReceipt({ projectRoot, workId, currentTargetKey }) {
  ...
  for (const receiptRoot of receiptRoots) {
    if (!fs.existsSync(receiptRoot)) continue;
    for (const entry of fs.readdirSync(receiptRoot)) {
      if (!entry || entry === currentTargetKey) continue;
      const roundsDir = path.join(receiptRoot, entry, 'rounds');
      if (!fs.existsSync(roundsDir)) continue;
      for (const receiptName of fs.readdirSync(roundsDir)) {
        const receiptPath = path.join(roundsDir, receiptName);
        let text;
        try {
