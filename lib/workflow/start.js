...
async function runFileSetStart(parsed, options) {
  ...
  const routeKind = parsed.invocation.routeKind;
  const targetKey = deriveFileSetTargetKey(parsed);
  ...
  // No silent resume: a fresh start with existing state is refused (reuse requires the
  // explicit `resume` token, dispatched before reaching here).
