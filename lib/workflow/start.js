...
async function runFileSetStart(parsed, options) {
  ...
  const targetMetadata = resolveRouteTargetMetadata(parsed, { ...options, cwd: projectRoot, rootCwd: originalCwd });
  ...
  // rule-file warnings without failing start when no custom rule files exist. r2q is the
  // exception: it is a DOCUMENT-rubric (PLAN) route and merges the COMMON+PLAN stack via
  // loadMergedRules (loadRouteRuleContext rejects 'r2q' as an unknown route kind).
  try {
    if (routeKind === 'r2q') {
  ...
  try {
    if (routeKind === 'pr') {
      ...
      // r2q resolves an r2p requirement directory (run.md gate + 03–07 doc chain),
      // NOT a CODE scope traversal. Any gate/shape failure throws ERR_R2Q_* and is
      // surfaced as a clean blocked result by the catch below. The editable fix
      // surface is the 03–07 set; run.md is a PROTECTED read-only dependency whose
      // sha256 enters the identity but is NEVER part of the editable set.
      context = resolveR2qTarget({
        cwd: projectRoot,
        target: targetMetadata.requirementDir,
        commandLog: options.commandLog
      });
      identity = buildR2qIdentity({ context, guardMode, roundLimit: parsed.invocation.roundLimit });
    } else {
  ...
  if (routeKind === 'pr') {
    ...
    // r2q identity: a relative requirement directory, the protected run.md sha256,
