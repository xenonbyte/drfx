...
function parseLinkedR2pRepairReceipt(text, workId) {
  if (!/\n- Kind: r2p-repair\b/m.test(`\n${text}`)) return null;
  const newWorkIdMatch = text.match(/^New work ID: ([^\n]+)$/m);
  if (!newWorkIdMatch || newWorkIdMatch[1].trim() !== workId) return null;
  const priorWorkIdMatch = text.match(/^- Target: workId=([^\n]+)$/m);
  const receiptIdMatch = text.match(/^- Receipt ID: ([^\n]+)$/m);
  return {
    priorWorkId: priorWorkIdMatch ? priorWorkIdMatch[1].trim() : null,
    priorReceiptId: receiptIdMatch ? receiptIdMatch[1].trim() : null
  };
}

function findPriorR2pRepairReceipt({ projectRoot, workId, currentTargetKey }) {
  const drfxRoot = path.join(projectRoot, '.drfx');
  const receiptRoots = [
    path.join(drfxRoot, 'targets'),
    path.join(drfxRoot, 'archived')
  ];
  const candidates = [];
  for (const receiptRoot of receiptRoots) {
    let stateEntries;
    try {
      if (!fs.lstatSync(receiptRoot).isDirectory()) continue;
      stateEntries = fs.readdirSync(receiptRoot, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const entry of stateEntries) {
      if (!entry.isDirectory() || !entry.name || entry.name === currentTargetKey) continue;
      const targetStateDir = path.join(receiptRoot, entry.name);
      for (const receipt of readRoundReceiptArtifacts(targetStateDir)) {
        const text = receipt.text;
        const parsed = parseLinkedR2pRepairReceipt(text, workId);
        if (!parsed) continue;
        candidates.push({
          priorWorkId: parsed.priorWorkId,
          priorReceiptId: parsed.priorReceiptId || roundReceiptId({ targetKey: entry.name }, receipt.receiptPath),
...
function maybeWriteR2pReopenLinkReceipt({ projectRoot, targetKey, workId }) {
  const prior = findPriorR2pRepairReceipt({ projectRoot, workId, currentTargetKey: targetKey });
  if (!prior) return null;
  const receiptPath = writeRoundReceipt({
    projectRoot,
    targetKey,
    round: 1,
    kind: 'r2p-reopen-link',
    status: 'review',
    target: `workId=${workId}`,
    issueIds: [],
    filesChanged: 'none',
    verification: 'linked prior r2p repair receipt',
    blockingReason: 'none',
    statusReason: 'none',
    summary: `Linked reopen from workId=${prior.priorWorkId || 'none'}\nPrior receipt ID: ${prior.priorReceiptId || 'none'}`,
    nextAction: 'run workflow context for initial review'
  });
  return {
    priorWorkId: prior.priorWorkId || null,
    priorReceiptId: prior.priorReceiptId || null,
    linkageReceiptPath: receiptPath
  };
}
...
async function runFileSetStart(parsed, options) {
  ...
  const fallbackRouteKind = parsed && parsed.invocation && parsed.invocation.routeKind
    ? parsed.invocation.routeKind
    : 'document';
  let projectRoot;
  let targetMetadata;
  try {
    ...
    return {
      ...
      guardMode: fallbackRouteKind === 'r2p' ? 'snapshot' : (parsed.invocation.guardMode || 'git'),
  ...
  if (routeKind === 'pr') {
    ...
    // r2p identity: active workId + read-only run.md sha256 + review-set fingerprint.
    // No PR base/head and no CODE scope/exclusion lists.
    manifest = {
      ...commonManifest,
      workId: identity.workId,
      runMdSha256: identity.runMdSha256,
      reviewSetFingerprint: identity.reviewSetFingerprint
    };
  } else {
    manifest = {
      ...commonManifest,
  ...
  const r2pReopenLink = routeKind === 'r2p'
    ? maybeWriteR2pReopenLinkReceipt({ projectRoot, targetKey, workId: targetMetadata.workId })
    : null;
  ...
  return {
    ...
    round: 1,
    currentPhase: 'review',
    fileSetFingerprint: routeKind === 'r2p' ? identity.reviewSetFingerprint : identity.fileSetFingerprint,
