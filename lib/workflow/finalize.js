...
function runPersistentFinalize(parsed, options) {
  ...
  if (ARCHIVE_ON_FINALIZE.has(finalResponse.finalStatus)) {
    ...
    if (archiveResult.archiveWarning) {
      nextAction = archiveFailureNextAction();
      try {
        writeWorkflowSummary(metadata, nextAction);
      } catch {
        // SUMMARY.md is command-owned and optional; final status has already been validated and persisted.
...
function runPersistentResume(parsed, options) {
  ...
  // A live pass/read-only-clean manifest is an anomaly (pre-upgrade leftover or an
  // archive-failed finalize). Archive it and signal the dispatcher to run a fresh start;
  // never re-report the old terminal status, never re-review in place.
  if (ARCHIVE_ON_FINALIZE.has(metadata.manifest.status)) {
    ...
    if (archiveWarning) {
      return stateCommandBase(metadata, {
        ...
        statusReason: 'none',
        archiveWarning,
        nextAction: 'delete or reset the leftover passed state directory, then retry'
      });
    }
