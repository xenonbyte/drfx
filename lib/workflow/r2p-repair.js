...
const { writeRoundReceipt, roundReceiptId } = require('../receipts');
...
async function readRunStatus(paths, workId, options = {}) {
  ...
  if (typeof entry.status !== 'string') {
    throw makeError(
      'ERR_R2P_JSON_CONTRACT_UNAVAILABLE',
      'r2p-json-contract-unavailable',
      `r2p-status JSON did not expose status for work_id ${workId}`
    );
  }
...
function fingerprintRun(runDir, artifactNames = REVIEW_ARTIFACTS) {
  ...
  const fileSetFingerprint = computeFileSetFingerprint(artifactNames.map((artifact) => ({
    path: artifact,
    status: 'modified',
    contentId: crypto.createHash('sha256').update(fs.readFileSync(path.join(runDir, artifact), 'utf8')).digest('hex')
  })));
  return {
    runMdSha256,
    fileSetFingerprint
...
function buildNextAction(rerunWorkId) {
  return `Run r2p-continue for workId=${rerunWorkId}, then rerun review-fix-r2p workId=${rerunWorkId}.`;
}
...
function writeReceipt(result) {
  ...
  if (result && result.projectRoot && result.targetKey && result.round) {
    ...
    return {
      receiptPath,
      receiptId: roundReceiptId({ targetKey: result.targetKey }, receiptPath),
