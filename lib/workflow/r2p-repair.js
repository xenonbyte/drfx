...
function buildNextAction(rerunWorkId, options = {}) {
  ...
  return `Run r2p-continue for workId=${rerunWorkId}, then rerun review-fix-r2p workId=${rerunWorkId}${resumeToken}.`;
}
...
async function runRepairCommand(paths, plan, options = {}) {
  ...
  return {
    ...
    routeId,
    staledStages,
    nextAction: buildNextAction(rerunWorkId, { resume: sameWorkIdRerun }),
    status: 'checkpoint',
