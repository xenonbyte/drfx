...
function normalizeOpenRoutesDetail(detail) {
  ...
  return detail.map((route) => ({
    routeId: route && route.route_id ? String(route.route_id) : null,
    ownerStage: route && route.owner_stage ? ensureStage(String(route.owner_stage), 'open route owner stage') : null,
    requiredAction: route && route.required_action ? singleLine(route.required_action) : ''
  }));
}
...
async function readRunStatus(paths, workId, options = {}) {
  ...
  const currentStage = ensureStage(String(entry.current_stage), 'current stage');
