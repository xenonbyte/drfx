...
// PLAN-TASK-009 (Phase B): the no-state token flow (context-pack, review guard, and
// state tokens) is single-file-shaped today; the file-set generalization is not yet
// wired. Until it is, a PR/CODE no-state command must NOT crash on the undefined
// single-file target and must NEVER create automatic-fix state or claim PASS. Return an
// explicit, observable unsupported result instead. This preserves the read-only safety
// boundary (no writes, no auto-fix state, no PASS) for file-set routes.
function fileSetNoStateUnsupported(parsed, options) {
  ...
  return {
    ...
    nextAction: 'run the PR/CODE route through persistent review-and-fix state instead of no-state'
  };
}
...
async function runNoStateWorkflowCommand(parsed, options) {
  if (isFileSetRoute(parsed)) return fileSetNoStateUnsupported(parsed, options);
