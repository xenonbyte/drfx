  ...
  const serializable = {
    ...
    entries: baseline.entries.map((entry) => {
      ...
      if (!record.missing) {
        ...
        atomicWriteBuffer(bodyPath, entry.body);
...
// PLAN-TASK-009 (Phase C): the recorded dependency set is the EXPLICIT allowed/monitored
// file set the file-set fixer may write. It is the union of the live file-set member paths
// (the files under review) PLUS any recorded necessary dependency files. Each entry is
// normalized to a safe in-root POSIX project-relative path; recorded dependencies also reject
// excluded directories. Anything outside the root or unsafe is REJECTED loudly (never
// silently dropped) so the Task-6 guard can never be handed an unvalidated allowed set.
function normalizeDependencyEntry(projectRoot, entry, { allowExcludedDirectories = false } = {}) {
  const raw = typeof entry === 'string' ? entry : (entry && entry.path);
  if (typeof raw !== 'string' || raw.trim() === '') {
    failStateValidation('dependency entry must be a non-empty path');
  }
  if (raw.includes('\0')) failStateValidation('dependency path must not contain null bytes');
  ...
  if (relative === '' || relative.startsWith('..') || path.isAbsolute(relative)) {
    failStateValidation(`dependency path must be inside project root: ${raw}`);
  }
  const posix = relative.split(path.sep).join('/');
  const segments = posix.split('/');
  if (!allowExcludedDirectories && segments.some((segment) => CODE_EXCLUDED_DIRECTORIES.has(segment))) {
    failStateValidation(`dependency path must not be in an excluded directory: ${posix}`);
  }
...
// Build the recorded, guarded dependency set: the live file-set members plus extra
// dependency entries (from the manifest), deduped and sorted. Live PR diff members may be
// inside directories CODE scope traversal normally excludes; extra dependencies still must
// be non-excluded. The result is the fixer's expectedChangedFileSet and the monitored set
// the Task-6 file-set guard covers BEFORE any write.
function recordedDependencySet({ projectRoot, liveFileSet, extraDependencies = [] }) {
  const byPath = new Map();
  for (const member of fileSetMemberEntries(liveFileSet)) {
    const record = normalizeDependencyEntry(projectRoot, member, { allowExcludedDirectories: true });
    if (!byPath.has(record.path)) byPath.set(record.path, record);
  }
  for (const dependency of extraDependencies || []) {
    const record = normalizeDependencyEntry(projectRoot, dependency);
    const existing = byPath.get(record.path);
    if (existing) {
      if (record.reason && !existing.reason) existing.reason = record.reason;
      if (record.issueId && !existing.issueId) existing.issueId = record.issueId;
    } else {
...
function recordedDependenciesFromManifest(metadata) {
  const stored = metadata.manifest && Array.isArray(metadata.manifest.recordedDependencies)
    ? metadata.manifest.recordedDependencies
    : [];
  return stored;
}

// File-set fixer guard: pins expectedChangedFileSet to the RECORDED dependency set (file-set
// members + recorded dependencies), not a single normalizedTarget. This is the allowed write
// boundary handed to the Task-6 file-set guard.
function buildFileSetFixerGuard({ metadata, ledger, liveFileSet, round }) {
  const fixableIssues = fixableIssuesFromLedger(ledger);
  if (fixableIssues.length === 0) {
    fail('ERR_FIX_CONTEXT_NO_ACCEPTED_ISSUES', 'fix context requires accepted or reopened issue IDs');
  }
  const dependencySet = recordedDependencySet({
    projectRoot: metadata.projectRoot,
    liveFileSet,
    extraDependencies: recordedDependenciesFromManifest(metadata)
  });
  return {
    activeLock: activeLockContext(metadata.projectRoot, metadata.targetKey),
    expectedChangedFileSet: dependencySet.map((entry) => entry.path),
    recordedDependencies: dependencySet,
    issueIds: fixableIssues.map((issue) => issue.id),
    latestFixGuardReportPath: latestFixGuardReportPath(metadata.targetStateDir, round),
    fileSetWriteRule: 'Write only files in the recorded file set and recorded necessary dependencies.',
    safeLocationAnchors: fixableIssues.map((issue) => issue.location).filter(Boolean)
  };
}
...
function archiveTargetState({ targetStateDir, options }) {
  ...
  if (stats.isSymbolicLink() || !stats.isDirectory()) {
    fail('ERR_STATE_RESET', 'reset requires a regular target-state directory; refusing to archive a symlink or non-directory');
  }
