...
// normalized to a safe in-root, non-excluded, POSIX project-relative path. Anything outside
// the root, an excluded directory, or unsafe is REJECTED loudly (never silently dropped) so
// the Task-6 guard can never be handed an unvalidated allowed set.
function normalizeDependencyEntry(projectRoot, entry) {
  ...
  if (segments.some((segment) => CODE_EXCLUDED_DIRECTORIES.has(segment))) {
...
function fileSetMemberPaths(liveFileSet) {
...
// dependency entries (from the manifest), deduped and sorted. Each is validated in-root +
// non-excluded. The result is the fixer's expectedChangedFileSet and the monitored set the
// Task-6 file-set guard covers BEFORE any write.
function recordedDependencySet({ projectRoot, liveFileSet, extraDependencies = [] }) {
  const byPath = new Map();
  for (const member of fileSetMemberPaths(liveFileSet)) {
    const record = normalizeDependencyEntry(projectRoot, member);
