...
const { loadCustomRuleFiles, mergeRules } = require('../rulebook');
const {
  computeFingerprint,
  deriveTargetKey,
  normalizeReferences,
...
function loadMergedRules({ projectRoot, documentType, homeDir = null } = {}) {
  ...
  const loaded = loadCustomRuleFiles({ projectRoot, documentType, homeDir: userHome });
  return mergeRules({ documentType, user: loaded.user, project: loaded.project });
}
...
module.exports = {
  ...
  BOOLEAN_FLAGS,
  DOWNGRADE_REASONS,
  NO_STATE_SUBCOMMANDS,
  NO_STATE_TOKEN_MAX_AGE_MS,
  PREFLIGHT_FORBIDDEN_FLAGS,
  RESERVED_STATE_PATH_BASENAMES,
  REVIEW_BACKED_NO_STATE_SUBCOMMANDS,
  RUNTIME_PLATFORMS,
  STATUS_REASONS,
  STDIN_HANDOFFS,
  SUBAGENT_PROBES,
  TARGET_STATE_DIR_SUBCOMMANDS,
  TARGET_STATE_SUBCOMMANDS,
  VALUE_FLAGS,
  ...
  assertExistingStateDirectorySafe,
  assertExistingStateFileSafe,
  assertFixEligible,
  assertPreFixFingerprint,
  ...
  deferredBlockingIssues,
  deriveTargetKey,
  descriptorPlatformFor,
  endFixBlocked,
  enrichTriageDecisions,
  fail,
  failStartStateValidation,
  failStateValidation,
  fallbackWorkflowBase,
  ...
  fs,
  guardBaselineFor,
  includedLowIdsFromCurrentContext,
  inspectActualChangedFiles,
  isRuntimeDowngrade,
  isStateValidationError,
  latestFixGuardReportPath,
  latestFullReviewComplete,
  latestReportPathByPrefix,
  loadCustomRuleFiles,
  loadMergedRules,
  ...
  noStateStaleTokenFailure,
  noStateTokenTooLarge,
  ...
  projectRootFromTargetStateDir,
  readContextManifest,
  readFixReportPayload,
  readJsonReportBlock,
  readLatestFixGuardBaseline,
  readLatestFixGuardReport,
  readLease,
  readLedgerIfPresent,
  readManifestAny,
  readManifestReport,
  ...
  readPackageVersion,
  readPersistedLeaseForTarget,
  readRequiredResumeLedger,
  readResumeDeterministicInputs,
  readResumeReceiptsIfNeeded,
  readResumeReferencedReports,
  readReviewerReport,
  readSemanticPayload,
  readStateCommandPayload,
  readWorkflowPayload,
  receiptFailureResult,
  referenceFingerprintsForManifest,
  referenceRecordsFor,
  refreshLock,
  releaseLock,
  ...
  resolveTargetFromStateManifest,
  resolveTargetMetadata,
  resumeRequiresReceipt,
  resumeStateValidationFailure,
  reviewerBlockingIssueIds,
  reviewerFindingsById,
  safeReferenceFingerprints,
  ...
  triageOutcome,
  triageSuppressedReviewerIds,
  triageSuppressesReviewerFinding,
