...
function runAbortFix(parsed) {
  ...
  if (['snapshot', 'git'].includes(abortGuardMode)) {
    const guardBaseline = readLatestFixGuardBaseline(metadata);
    const rollbackAnchor = guardBaseline.ok ? guardBaseline.report.rollbackAnchor : null;
    // Snapshot mode always records a rollback snapshot, so a missing/invalid baseline is
    // a real error. A git fix that predates per-fix snapshots legitimately has no
    // snapshotPath; keep its original no-restore abort behavior instead of failing.
    if (abortGuardMode === 'snapshot' && !guardBaseline.ok) {
