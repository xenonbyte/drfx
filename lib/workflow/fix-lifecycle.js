...
function hasCurrentRoundFixReportMismatchReceipt(metadata) {
  let text;
  try {
    text = fs.readFileSync(currentRoundFixBlockedReceiptPath(metadata), 'utf8');
  } catch {
    return false;
  }
  return receiptField(text, 'Kind') === 'fix-blocked' &&
...
function fixReportMismatchRetryResult(metadata, options) {
  ...
  let lease;
  try {
    const currentTargetFingerprint = computeFingerprint(metadata.targetPath);
    ...
    lease = acquireLock({
      ...
      lastKnownContentSha256: currentTargetFingerprint.sha256,
