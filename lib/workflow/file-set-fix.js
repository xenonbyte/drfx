...
async function resolveLiveFileSet(metadata, options) {
  ...
  if (metadata.routeKind === 'r2q') {
    ...
    // stored relative requirementDir — NOT a CODE scope walk and NEVER a partition plan.
    // The fixer's writable boundary is EXACTLY the 03–07 docs (07-plan + the owning
    // upstream doc): run.md is carried as a PROTECTED read-only dependency and is NOT in
    // `files`, so the file-set guard already refuses it (and any non-03–07 path) as
    // out-of-set. This branch precedes the partitioned/CODE fallback so an r2q target can
    // never inherit the whole-project CODE write boundary.
    ...
    // The fix guard resolves monitored members against the PROJECT ROOT and reads them on
    // disk, so the writable set must carry project-root-relative POSIX paths (the 03–07
    // docs live under the requirement directory, not at the project root). The
    // requirementDir-relative doc names are kept on each member as `requirementRelativePath`
    // so liveFileSetFingerprint can match the persisted (doc-name-keyed) r2q fingerprint.
    return {
      routeKind: 'r2q',
