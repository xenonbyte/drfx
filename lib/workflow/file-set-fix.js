...
async function runBeginFix(parsed, options) {
  ...
  // Partitioned project review is read-only for the fix loop in this version. An ACTIVE
  // over-cap partition plan owns this target; a content-changing fix would move the
  // projectReviewFingerprint and invalidate every per-unit coverage receipt, and the
  // in-place re-partition/re-review loop is not built yet. Reject at the door (before any
  // lock, baseline, or fixer work) instead of letting the fixer spend a round only to
  // late-block at end-fix. The aggregate path still earns PASS (mode is unchanged); the
  // end-fix freshness gate stays as a backstop for an inactive/stale plan.
  if (readActivePartitionedPlan(metadata)) {
    return beginFixBlocked(metadata, null, {
      ...
      summary: 'partitioned project review is read-only in this version; the in-place fix loop is unavailable for an over-cap partitioned target',
      nextAction: 'fix the findings manually, or narrow scope= to a fixable subset and rerun review-fix-code; the partitioned aggregate review still earns PASS without an in-place fix loop'
    });
  }
