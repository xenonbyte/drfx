...
async function runBeginFix(parsed, options) {
  ...
  try {
    ...
    // Persist only a metadata baseline for end-fix validation. Raw file bodies are not safe
    // workflow state, especially when CODE scope defaults to the whole project.
...
// Persist a metadata-only baseline. It is enough for end-fix to detect in-set and out-of-set
// changes, while avoiding raw file-body persistence in workflow state.
