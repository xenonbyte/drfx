...
const { CODE_EXCLUDED_DIRECTORIES, describeCodeBlock } = require('../target-context');
const {
  readUnitsPlan,
  readAllSummaries,
  unitsToReReview
...
//
// A partitioned CODE review writes project-review/units.json under the target key
// (Task 3). When that plan is present, the post-fix re-review must be BOUNDED to the
// affected units (SPEC-BEHAVIOR-007: directly-changed ∪ suggestedRefs-hit ∪
// extraReads-hit), then re-aggregated — NOT a whole-project re-review. This helper is
// purely additive: it READS the plan + the persisted unit summaries and computes the
// affected unit ids from the fix's actual changed files. It does not touch the fix
// guard, the monitored-set subset check, or the fixAttemptCount counting; a missing
// plan (non-partitioned PR/CODE) returns null and the legacy flow is byte-identical.
function partitionedAffectedUnits(metadata, actualChanged) {
  let plan;
  try {
    plan = readUnitsPlan(metadata.targetStateDir);
  } catch {
    // No partitioned plan (or an unreadable one) ⇒ non-partitioned target; the caller
    // keeps today's behavior unchanged. Detection never blocks a valid end-fix.
    return null;
  }
  // extraReads-hit detection reads the persisted per-unit summaries (no bodies). A
  // summaries dir that does not yet exist simply yields no extraReads matches.
  const summaries = new Map(readAllSummaries(metadata.targetStateDir).map((entry) => [entry.id, entry.body]));
  return unitsToReReview(actualChanged || [], plan, summaries);
}
...
async function runEndFix(parsed, options) {
  ...
  // PLAN-TASK-011: for a PARTITIONED CODE target, bound the post-fix re-review to the
  // affected units (directly-changed ∪ suggestedRefs-hit ∪ extraReads-hit) instead of a
  // whole-project re-review, then re-aggregate. The actual re-review/aggregate work is
  // coordinator-driven through the unit-review CLI; here we only surface the bounded set
  // + the directing nextAction. A non-partitioned target returns null ⇒ unchanged result.
  const affectedUnits = partitionedAffectedUnits(metadata, actualChanged);
  if (affectedUnits) {
    return stateCommandBase(metadata, {
      ok: true,
      status: 'end-fix',
      ...
      reReviewScope: 'bounded-affected-units',
      affectedUnits,
      nextAction: affectedUnits.length > 0
        ? `re-review only the affected units (${affectedUnits.join(', ')}) then re-aggregate, then run record-diff-review`
        : 're-aggregate the partitioned project review, then run record-diff-review'
    });
  }
