...
async function resolveLiveFileSet(metadata, options) {
  ...
  if (metadata.routeKind === 'r2q') {
    // r2q precedes the partitioned/CODE fallback so it never inherits the whole-project CODE
    // write boundary. See file-set-r2q-gate.js.
    return resolveR2qLiveFileSet(metadata, options);
  }
...
function retryBlockedFixBeginFix(metadata, options, round) {
  ...
  const retryGateDrift = revalidateR2qGate(metadata);
  if (retryGateDrift) {
...
async function runBeginFix(parsed, options) {
  ...
  // CHECKPOINT 1 (before begin-fix): revalidate the r2q run.md gate before resolving the live
  // file set or opening the write path; a drifted/deleted/incomplete gate blocks here. See
  // file-set-r2q-gate.js.
  const beginGateDrift = revalidateR2qGate(metadata);
  if (beginGateDrift) {
...
function runRefreshLock(parsed, options) {
  ...
  try {
    ...
    const refreshGateDrift = revalidateR2qGate(metadata);
    if (refreshGateDrift) {
...
async function runEndFix(parsed, options) {
  ...
  // is not a monitored member), but the re-read here fails and BLOCKS. See file-set-r2q-gate.js.
  const endGateDrift = revalidateR2qGate(metadata);
  if (endGateDrift) {
