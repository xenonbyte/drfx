...
async function runBeginFix(parsed, options) {
  ...
  try {
    ...
    ledger = assertFileSetFixEligible(metadata);
  } catch (error) {
  ...
  const existingLease = readLease({ projectRoot: metadata.projectRoot, targetKey: metadata.targetKey });
  ...
  try {
    ...
    if (isStateValidationError(error)) return stateValidationResult(parsed.targetStateDir, error);
    return stateValidationResult(parsed.targetStateDir, error);
  }
  ...
  let lease = null;
  try {
    // PRE-WRITE worktree guard: every monitored file must be in the recorded set; any
    // unexpected non-monitored worktree change blocks before the fixer writes anything.
    if (guardMode === 'git') {
    ...
    const baseline = captureFileSetBaseline({ projectRoot: metadata.projectRoot, monitoredFiles: monitoredSet });
    ...
    const anchorPath = writeLockAnchor(metadata, fingerprint);
...
function persistBaseline(metadata, baseline) {
  const serializable = {
    ...
    entries: baseline.entries.map((entry) => ({
      ...
      body: entry.body ? Buffer.from(entry.body).toString('base64') : null
    }))
  };
...
function readPersistedBaseline(metadata) {
  ...
  return {
    ...
    entries: parsed.entries.map((entry) => ({
      ...entry,
      body: entry.body ? Buffer.from(entry.body, 'base64') : undefined
    }))
  };
}
...
async function runEndFix(parsed, options) {
  ...
  try {
    ...
    return endFixBlocked(metadata, 'state-validation-failed', {
      summary: `unable to resolve the live file set: ${error && error.message ? error.message : String(error)}`
    });
  }
  ...
  let declaredFiles;
  try {
    declaredFiles = (fixReport.filesChanged || []).map((file) => normalizeDependencyEntry(metadata.projectRoot, file).path);
  } catch (error) {
    return endFixBlocked(metadata, 'fix-report-mismatch', {
      issueIds: fixReport.fixed.map((fixed) => fixed.issue_id),
  ...
  // ACTUAL changed-file inspection against the recorded set. For git, read real worktree
  // status. For snapshot, validate against the captured baseline.
  const guardMode = guardModeFor(metadata);
  let actualChanged;
  if (guardMode === 'git') {
    const guardResult = checkFileSetWorktree({
    ...
    actualChanged = guardResult.changedFiles || [];
  } else {
