 ...
 *   inventoryRows:Array<{path,size,ext,contentId,unit_id}> }}
 */
function assemblePartitionPlan({ inventory, projectReviewFingerprint, projectRoot, unitByteBudget = MAX_UNIT_BYTES }) {
  const units = partitionInventory(inventory, { unitByteBudget });
  ...
  for (const unit of units) {
    if (unit.oversize_file === true) {
      // Oversize body is never read; suggestedRefs stays the empty array.
      unit.suggestedRefs = [];
      continue;
    }
  ...
  // inventory.jsonl rows: one per surviving file, flattened from units in
  // deterministic (unit order, then file order) form, then path-sorted so the
  // serialized file is byte-identical for identical trees.
  const inventoryRows = [];
  for (const unit of units) {
  ...
  inventoryRows.sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
...
function writeProjectReviewPlan(targetStateDir, plan) {
  const planDir = path.join(targetStateDir, PROJECT_REVIEW_DIRNAME);
  fsExtra.mkdirSync(planDir, { recursive: true });
  atomicWriteFile(path.join(planDir, 'inventory.jsonl'), formatInventoryJsonl(plan));
  atomicWriteFile(path.join(planDir, 'units.json'), formatUnitsJson(plan));
...
async function resolveWholeRootPartitionPlan({ projectRoot, commandLog }) {
  const { inventory, projectReviewFingerprint } = await resolveCodeInventory({
    cwd: projectRoot,
    scopes: [],
    commandLog
  });
  return assemblePartitionPlan({ inventory, projectReviewFingerprint, projectRoot });
}
