...
function loadFileSetRules(metadata, options) {
  // r2p is a DOCUMENT-rubric (PLAN) route: it merges the COMMON+PLAN document rule
  // stack via loadMergedRules, NOT the self-contained PR/CODE route-rubric stack
  // (loadRouteRuleContext rejects 'r2p' as an unknown route kind). PR/CODE keep the
...
function isFileSetResolveError(error) {
  return error && (
    error.code === 'ERR_FILE_SET_RESOLVE' ||
    (typeof error.code === 'string' && error.code.startsWith('ERR_R2P_'))
  );
}
...
function blockFileSetResolve(parsed, metadata, error) {
  ...
  return persistentBase(parsed, metadata, {
    ...
    statusReason: 'none',
    nextAction: (error && error.nextAction) || (isR2pResolveError
      ? 'restore the r2p requirement directory before continuing'
...
function reviewedIdentityManifestUpdates(metadata, liveFileSet) {
  ...
  if (metadata.routeKind === 'r2p') {
    // r2p may re-anchor only the reviewed 03-07 fingerprint after review. run.md is a
    ...
    return {
      reviewSetFingerprint: liveFileSet.reviewSetFingerprint || liveFileSetFingerprint(liveFileSet)
    };
  }
...
async function runFileSetContext(parsed, options) {
  ...
  try {
    ...
    const contextPack = buildFileSetContextPack({
      ...
      changedSinceLastReview: liveFingerprint !== (
        metadata.routeKind === 'r2p'
      ...
      fixerGuard,
      // r2p carries run.md as a protected read-only dependency; other file-set
