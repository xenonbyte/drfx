...
async function resolveLiveFileSet(metadata, options) {
  ...
  if (metadata.routeKind === 'r2p') {
    // r2p re-resolves the requirement directory (run.md gate + 03–07 chain) from the
...
function reviewedIdentityManifestUpdates(metadata, liveFileSet) {
  ...
  if (metadata.routeKind === 'r2p') {
    // r2p may re-anchor only the editable 03-07 fingerprint after review. run.md is a
