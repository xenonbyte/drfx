...
function runAggregateReview(parsed, options) {
  ...
  // and true for any reviewed subset (unreviewed units are simply absent). Before
  // any PASS can surface we MUST reconcile against the plan: every unit declared in
  // units.json has to carry a coverage_risk:'none' summary (SPEC-BEHAVIOR-005).
  const plan = readUnitsPlan(targetStateDir);
  const plannedUnitIds = plan.units.map((unit) => unit.unit_id);
  const noneCoveredUnitIds = new Set(
    summaryEntries
      .filter((entry) => entry.body && entry.body.coverage_risk === 'none')
      .map((entry) => entry.id)
  );
  const uncoveredUnitIds = plannedUnitIds.filter((unitId) => !noneCoveredUnitIds.has(unitId));

  const outPath = aggregatePath(targetStateDir);

  if (uncoveredUnitIds.length > 0) {
    // Missing entirely OR present-but-high. Never call aggregate-and-report-PASS
    // over a partial/empty set: force the honest verdict and persist THAT. The
    // coverage proof's discovered count is reconciled against the EXPECTED unit set
    // (plan.units.length), not just the present summaries, so the proof matches
    // reality. NOTE: backstop coverage is enforced by aggregate()'s own gate on the
    // full set; this guard is scoped to the unit-coverage hole.
    ...
    return {
      ...
      nextAction: 'coverage incomplete; review the uncovered units before claiming a project PASS'
    };
  }

  // Full unit coverage holds (every planned unit has a coverage_risk:'none'
  // summary). Behavior is unchanged: Task 1 pure aggregate is authoritative for the
  // verdict and PASS is allowed iff its gate holds. The CLI never elevates to PASS.
