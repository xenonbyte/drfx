...
const { aggregate, dedupChunkFindings } = require('../project-review');
...
// Collapse overlap-duplicate findings across an oversize file's chunks BEFORE the
// pure aggregate dedup. Gated on the partitioned chunk path: only when the plan is
// partitioned AND carries oversize_chunk units. Non-chunk (and non-partitioned)
// aggregation is byte-unchanged — dedupChunkFindings returns the array untouched.
function normalizeChunkOverlapFindings(plan, findings) {
  const isPartitioned = plan && plan.reviewMode === 'partitioned';
  const hasChunkUnits = isPartitioned && Array.isArray(plan.units)
    && plan.units.some((unit) => unit && unit.oversize_chunk === true);
  if (!hasChunkUnits) return findings;
  return dedupChunkFindings(findings, plan.units);
}

function aggregateWithUniqueReviewerFindingIds(summaries, findings, plan) {
  const normalizedFindings = normalizeChunkOverlapFindings(plan, findings);
  const result = aggregate(summaries, normalizedFindings);
  return {
    ...result,
...
async function runAggregateReview(parsed, options) {
  ...
  let metadata;
  try {
    metadata = resolveFileSetStateMetadata(targetStateDir);
  } catch (error) {
    return stateValidationResult(targetStateDir, error);
  }
  ...
  if (uncoveredUnitIds.length > 0 || uncoveredBackstops.length > 0) {
    ...
    const forcedHigh = aggregateWithUniqueReviewerFindingIds(summaries, findings, plan);
    const result = {
  ...
  const result = aggregateWithUniqueReviewerFindingIds(summaries, findings, plan);
