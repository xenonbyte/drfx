...
function unitContext({ targetStateDir, projectRoot, unitId, homeDir = null }) {
  ...
  // PLAN-TASK-014: a chunk unit (oversize_chunk) takes the NORMAL review branch.
  // Task 12 stored chunkIndex/chunkCount on the UNIT, not on files[0]; the chunk
  // member already carries contextLineRange/primaryLineRange. Copy the unit-level
  // chunk position onto each member so buildFileSetContextPack emits concrete
  // chunk.index/chunk.count metadata (NEVER the slice body). Non-chunk units pass
  // unit.files through unchanged so the persisted pack stays byte-identical.
  const memberFiles = unit.oversize_chunk === true
    ? unit.files.map((file) => ({ ...file, chunkIndex: unit.chunkIndex, chunkCount: unit.chunkCount }))
    : unit.files;
  const contextPack = buildFileSetContextPack({
    ...
    fileSet: { routeKind: 'code', files: memberFiles, normalizedScopes: [] },
...
async function recordUnitReview({
  ...
  // A skipped member file means the unit was NOT fully covered. The only documented
  // skip scenario (oversize) is forced to coverage_risk:high above, so a normal unit
  // pairing a non-empty Skipped list with coverage_risk:none is a contradictory receipt
  // and is refused (mirrors ERR_UNIT_REVIEW_INCONSISTENT_RECEIPT). "PASS is earned."
  if (receipt.coverageRisk === 'none' && Array.isArray(receipt.skipped) && receipt.skipped.length > 0) {
    fail(
      'ERR_UNIT_REVIEW_SKIPPED_WITH_NONE',
      'coverage_risk:none requires Skipped:none; a skipped member file means coverage is incomplete (report Coverage risk: high)'
    );
  }
  if (!await extraReadsStillMatch(projectRoot, receipt.extraReads)) {
    fail(
      'ERR_UNIT_REVIEW_EXTRA_READ_STALE',
      'extraRead contentId does not match the current file content'
    );
  }
  const rules = loadMergedRulesForRoot(projectRoot, { homeDir });
...
async function nextUnit(targetStateDir, _summaries, options = {}) {
  ...
  for (const unit of plan.units) {
    if (!await unitHasValidSummary(targetStateDir, unit, rules, projectRoot)) {
...
async function unitHasValidSummary(targetStateDir, unit, rules, projectRoot) {
  ...
  return summary.reviewCacheKey === expected &&
    await extraReadsStillMatch(projectRoot, summary.extraReads);
}
...
function invalidateAllBackstopReviews(targetStateDir) {
  ...
  return cleared.sort((x, y) => (x < y ? -1 : x > y ? 1 : 0));
}
...
function recordCrosscuttingReview({
  ...
  // A non-empty Skipped list is residual coverage risk: a confirming receipt must have
  // reviewed everything (Skipped:none). Fold it into the none-gate so a skipped file can
  // never coexist with a backstop coverage_risk:none (consistent with the unit gate).
  const receiptConfirmsNone = receipt.reviewed === true &&
    receipt.coverageRisk === 'none' &&
    !(Array.isArray(receipt.skipped) && receipt.skipped.length > 0);
