...
async function resolveNoStateFileSetReviewMetadata(parsed, options) {
  ...
  if (routeKind === 'pr') {
    ...
    liveFileSet = {
      ...
      requirementDir: context.runLocation,
...
async function runFileSetWriteEligibilityPreflight(parsed, options) {
  ...
  try {
    targetMetadata = resolveRouteTargetMetadata(parsed, options);
    projectRoot = targetMetadata.projectRoot;
    base = workflowBase(parsed, {
      ...options,
      cwd: projectRoot,
      rootCwd: options.cwd || process.cwd()
    });
    ...
    if (!base && error && error.blockingReason === 'invalid-project-root') {
      throw error;
    }
    const fallbackBase = base || {
      ...fallbackFileSetNoStateBase(parsed),
      mode: parsed.invocation.mode,
      modeNormalizedFrom: parsed.invocation.modeNormalizedFrom
    };
    ...
    return {
      ...
      blockingReason,
      statusReason: 'none',
      errorCode: error && error.code ? error.code : 'ERR_FILE_SET_PREFLIGHT',
      message: error && error.message ? error.message : String(error),
      nextAction: (error && error.nextAction) || 'commit or restore the file set, or rerun with read-only'
    };
  }
}
