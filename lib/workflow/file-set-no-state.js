...
async function resolveNoStateFileSetReviewMetadata(parsed, options) {
  ...
  if (routeKind === 'pr') {
  ...
  } else if (routeKind === 'r2q') {
    // r2q resolves an r2p requirement directory, NOT a CODE scope traversal: it
    // enforces the run.md gate + 03–07 doc chain and refuses archived dirs. Any
    // gate/shape failure throws (ERR_R2Q_*) and surfaces as a clean blocked
    // result via the caller's catch — it never reaches reviewer-recording. The
    // reviewed anchor is 07-plan.md (read-only advisory); the file-set
    // fingerprint covers the full editable 03–07 set so drift is still detected.
    ...
    r2qContext = context;
    const files = context.editableFiles.map((file) => ({
      path: file.relativePath,
      status: 'modified',
      contentId: file.sha256
    }));
    const reviewedAnchor = context.editableFiles.filter((file) => file.relativePath === '07-plan.md');
    liveFileSet = {
      routeKind: 'r2q',
      requirementDir: context.requirementDir,
      runMdPath: context.runMdPath,
      // The read-only advisory anchor is 07-plan.md; the wider editable set
      // (03–07) is the fix surface in later review-and-fix tasks.
      normalizedScopes: ['07-plan.md'],
      files: reviewedAnchor.length > 0 ? reviewedAnchor.map((file) => ({
        path: file.relativePath,
        status: 'modified',
        contentId: file.sha256
      })) : files
  ...
  // r2q's identity + drift fingerprint covers the full editable 03–07 set (the
  ...
  const fileSetFingerprint = r2qContext
    ? r2qContext.fileSetFingerprint
...
async function runFileSetNoStateContext(parsed, options) {
  ...
  // r2q is a DOCUMENT-rubric route (documentType PLAN): it merges the COMMON+PLAN
  // document rule stack via loadMergedRules, NOT the self-contained PR/CODE
  // route-rubric stack (loadRouteRuleContext rejects 'r2q' as an unknown route
...
async function runFileSetWriteEligibilityPreflight(parsed, options) {
  ...
  try {
    ...
    } else if (routeKindFor(parsed) === 'r2q') {
      // r2q write eligibility resolves via resolveR2qTarget (run.md gate + 03–07
      // chain), NOT resolveCodeTarget. Any gate/shape failure throws ERR_R2Q_* and
      // is surfaced as a clean blocked result by the catch below. The fix surface
      // monitored by the write guard is the full editable 03–07 set.
      const context = resolveR2qTarget({
        cwd: projectRoot,
        target: parsed.invocation.target,
        commandLog: options.commandLog
