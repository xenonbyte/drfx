...
async function resolveNoStateFileSetReviewMetadata(parsed, options) {
  ...
  if (routeKind === 'pr') {
    ...
    const context = resolveR2qTarget({
      cwd: projectRoot,
      target: targetMetadata.requirementDir,
      commandLog: options.commandLog
    });
    r2qContext = context;
    const files = context.editableFiles.map((file) => ({
      path: path.relative(projectRoot, file.absolutePath).split(path.sep).join('/'),
...
async function runFileSetWriteEligibilityPreflight(parsed, options) {
  ...
  try {
    if (routeKindFor(parsed) === 'pr') {
      ...
      const context = resolveR2qTarget({
        cwd: projectRoot,
        target: path.join(projectRoot, targetMetadata.requirementDir),
        commandLog: options.commandLog
      });
      liveFileSet = {
        routeKind: 'r2q',
