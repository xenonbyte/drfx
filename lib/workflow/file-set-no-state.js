...
async function resolveNoStateFileSetReviewMetadata(parsed, options) {
  ...
  if (routeKind === 'pr') {
    ...
    // r2p resolves an r2p requirement directory, NOT a CODE scope traversal: it
...
async function runFileSetWriteEligibilityPreflight(parsed, options) {
  ...
  try {
    ...
    } else if (routeKindFor(parsed) === 'r2p') {
      // r2p write eligibility resolves via resolveR2pTarget (run.md gate + 03–07
      // chain), NOT resolveCodeTarget. Any gate/shape failure throws ERR_R2P_* and
