...
const R2P_LOCATION_STAGE_PREFIXES = Object.freeze([
  ['00-raw-requirement.md', 'raw_requirement'],
  ['03-requirement-brief.md', 'requirement_brief'],
  ['04-risk-discovery.md', 'risk_discovery'],
  ['05-design.md', 'design'],
  ['06-spec.md', 'spec']
]);

function inferR2pOwnerStageFromFinding(finding) {
  const location = String((finding && finding.location) || '');
  for (const [prefix, stage] of R2P_LOCATION_STAGE_PREFIXES) {
    if (location.startsWith(prefix)) return stage;
  }
  const haystack = [
  ...
  if (haystack.includes('raw requirement')) return 'raw_requirement';
  if (/(requirement brief|scope|goal|non-goal|acceptance direction)/.test(haystack)) return 'requirement_brief';
  if (/(risk|rollback|migration|security|dependency)/.test(haystack)) return 'risk_discovery';
  if (/(architecture|interface|module-boundary|implementation-strategy|design)/.test(haystack)) return 'design';
  if (/(verification|acceptance criterion|observable behavior|spec\b)/.test(haystack)) return 'spec';
  if (/(plan|task decomposition|ordering|execution command)/.test(haystack)) return 'plan';
  return null;
}
...
function enrichR2pTriageDecisions(decisions, reviewerReport) {
  ...
  return decisions.map((decision) => {
    ...
    const ownerStage = decision.owner_stage || finding.owner_stage || finding.ownerStage || inferR2pOwnerStageFromFinding(finding);
...
function acceptedR2pFindings(metadata, liveStatus) {
  const fromTriageReport = acceptedR2pFindingsFromTriageReport(metadata);
  if (fromTriageReport.length > 0) {
    const fallbackOwnerStage = ((liveStatus.openRouteOwnerStages || [])[0] || liveStatus.currentStage);
    return fromTriageReport.map((finding) => ({
      ...finding,
      owner_stage: finding.owner_stage || fallbackOwnerStage
    }));
  }
  ...
  return (ledger.issues || [])
    ...
    .map((issue, index) => ({
      ...
      owner_stage: issue.owner_stage || issue.ownerStage || fallbackOwnerStage,
...
async function runRecordR2pRepairPlanCommand(parsed, options) {
  ...
  try {
    ...
    const accepted = acceptedR2pFindings(metadata, liveStatus);
