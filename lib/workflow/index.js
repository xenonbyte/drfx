...
const R2P_INVOCATION_PREFLIGHT_SUBCOMMANDS = new Set(['start', 'context', 'record-review', 'record-triage']);
...
function acceptedR2pFindings(metadata, liveStatus) {
  ...
  return (ledger.issues || [])
    ...
    .map((issue, index) => ({
      ...
      owner_stage: String(issue.owner_stage || issue.ownerStage || fallbackOwnerStage)
    }));
}

function r2pWorkIdFromMetadata(metadata) {
  if (metadata.manifest.workId) return String(metadata.manifest.workId);
  return path.posix.basename(String(metadata.manifest.requirementDir || '').split(path.sep).join('/'));
}
...
function runR2pRecordTriageCommand(parsed, options) {
  ...
  const decisions = enrichTriageDecisions(triage, reviewerReport);
