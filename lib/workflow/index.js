...
// third file-set kind (r2q) is labeled correctly instead of mislabeled 'code'.
...
function freshStartFailureAfterArchive(parsed, options, error, archivedStatePath) {
  ...
  try {
    ...
    base = {
      ...
      // so r2q is labeled 'r2q', never mislabeled 'code'. Fall back to the parsed
...
function latestReadableFinalArtifactPath(full) {
  ...
  try {
    const receiptNames = fs.readdirSync(roundsDir)
      .filter((name) => name.endsWith('.md'))
      .sort();
    if (receiptNames.length > 0) return path.join(roundsDir, receiptNames[receiptNames.length - 1]);
  } catch {
    // PASS finalization has no receipt; SUMMARY.md is the durable final artifact.
  }
  const summaryPath = path.join(stateDir, 'SUMMARY.md');
  try {
    if (fs.statSync(summaryPath).isFile()) return summaryPath;
  } catch {
    return null;
  }
  return null;
