...
function readableFilePath(filePath) {
  try {
...
function receiptKindFromName(name) {
  ...
  return match[2].replace(/-attempt-\d+$/, '');
}
...
function finalReceiptKindsFor(full) {
  ...
  if (full && full.status === 'blocked') kinds.push('gate-drift');
  return [...new Set(kinds)];
}

function latestReceiptPath(roundsDir, receiptNames, predicate) {
  const matches = receiptNames.filter(predicate).sort();
  if (matches.length === 0) return null;
  return path.join(roundsDir, matches[matches.length - 1]);
}
...
function latestReadableFinalArtifactPath(full) {
  ...
  const summaryPath = readableFilePath(path.join(stateDir, 'SUMMARY.md'));
  ...
  return latestReceiptPath(roundsDir, receiptNames, () => true);
}
