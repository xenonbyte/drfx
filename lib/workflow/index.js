...
// PLAN-TASK-009 (Phase B): invocation-based persistent lifecycle commands for PR/CODE
// routes refuse cleanly (read-only, no PASS, no crash) until the file-set lifecycle is
// wired. The file-set MANIFEST.md created by start remains intact and resumable later.
function fileSetLifecycleUnsupported(parsed, options) {
  return withReadOnlyMode({
    ...
    nextAction: 'PR/CODE persistent review lifecycle is not yet available beyond start'
  });
}
...
async function runWorkflowCommand(subcommand, args, options = {}) {
  ...
  // PLAN-TASK-009 (Phase B): live PR/CODE state is created by `start` (handled below),
  // but the persistent review/triage lifecycle that consumes it (context-pack, reviewer
  // guard baseline, fixer dependency boundary, diff review, full re-review, finalize) and
  // explicit/stale resume handling are still single-file-shaped. Until that file-set
  // generalization lands, every persistent command EXCEPT the live `start` must refuse
  // cleanly for a file-set route — never crash on the undefined single-file target, never
  // claim PASS. The file-set MANIFEST created by start stays intact and resumable later.
  if (isFileSetRoute(parsed) && parsed.subcommand === 'start' && parsed.invocation.mode === 'review-and-fix' && !parsed.invocation.resume) {
