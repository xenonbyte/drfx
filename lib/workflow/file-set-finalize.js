...
const { revalidateR2qGate } = require('./file-set-r2q-gate');
...
async function liveIdentityFor(metadata, identityInput, options) {
  ...
  if (metadata.routeKind === 'r2q') {
    // r2q is its OWN file-set kind, NOT code: it re-resolves the requirement directory
    // (run.md gate + 03–07 chain) from the stored relative requirementDir and builds the
    // r2q identity (requirementDir/runMdSha256/fileSetFingerprint), so a clean r2q run
    ...
    // resolveR2qTarget already returns the editable-set fileSetFingerprint and the
    // protected run.md sha256, in the shape buildR2qIdentity consumes.
    return buildR2qIdentity({ context, guardMode, roundLimit });
  }
...
function storedIdentityFor(metadata) {
  ...
  if (metadata.routeKind === 'r2q') {
    // r2q stored identity mirrors buildR2qIdentity's shape (scalar-only, no list fields):
    // a code-shaped stored identity would carry phantom exclusions/scopes and never match
    // the r2q live identity.
    return {
...
function compareFileSetIdentity(routeKind, stored, requested) {
  ...
  if (routeKind === 'r2q') return compareR2qIdentity({ stored, requested });
...
// Stop a final PASS as a guarded drift blocker when the r2q run.md gate drifted/was
...
async function runFileSetFinalize(parsed, options) {
  ...
  try {
    ...
    if (finalResponse.finalStatus === 'pass') {
      // CHECKPOINT 4 (before final PASS): an r2q PASS may not be claimed from a stale gate.
      ...
      const passGateDrift = revalidateR2qGate(metadata);
      if (passGateDrift) {
