...
async function liveIdentityFor(metadata, identityInput, options) {
  ...
  if (metadata.routeKind === 'r2p') {
    const workId = String(
      metadata.manifest.workId ||
      path.posix.basename(String(metadata.manifest.requirementDir || '').split(path.sep).join('/'))
    );
...
function currentRoundReceiptFiles(metadata) {
  ...
  return fs.readdirSync(roundsDir)
    ...
    .sort()
    .map((entry) => path.join(roundsDir, entry));
}
...
function currentRoundR2pRepairReceipt(metadata) {
  ...
  for (const receiptPath of currentRoundReceiptFiles(metadata)) {
    let text;
    try {
    ...
    const parsed = parseR2pRepairReceipt(text);
    if (parsed) return { receiptPath, ...parsed };
  }
  return null;
}
...
function r2pResumeCanRefresh(metadata, comparison) {
  return metadata.routeKind === 'r2p' &&
    ...
    comparison.mismatches.every((field) => field === 'runMdSha256' || field === 'fileSetFingerprint');
}
...
function r2pResumeRefreshState(parsed, metadata, liveIdentity, options) {
  updatePersistentManifest(metadata, {
    ...
    requirementDir: liveIdentity.requirementDir,
    runMdSha256: liveIdentity.runMdSha256,
    fileSetFingerprint: liveIdentity.fileSetFingerprint
  });
  return fileSetBase(parsed, metadata, {
    ok: true,
    status: 'review',
