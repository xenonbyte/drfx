...
const { revalidateR2pGate } = require('./file-set-r2p-gate');
...
async function liveIdentityFor(metadata, identityInput, options) {
  ...
  if (metadata.routeKind === 'r2p') {
    // r2p is its OWN file-set kind, NOT code: it re-resolves the requirement directory
    // (run.md gate + 03–07 chain) from the stored relative requirementDir and builds the
    // r2p identity (requirementDir/runMdSha256/fileSetFingerprint), so a clean r2p run
    // earns PASS instead of failing the CODE compare with phantom `exclusions` drift.
    const requirementTarget = path.resolve(metadata.projectRoot, metadata.manifest.requirementDir);
    const context = resolveR2pTarget({
      cwd: metadata.projectRoot,
      target: requirementTarget,
      commandLog: options.commandLog
    });
    // resolveR2pTarget already returns the editable-set fileSetFingerprint and the
    // protected run.md sha256, in the shape buildR2pIdentity consumes.
    return buildR2pIdentity({ context, guardMode, roundLimit });
  }
...
function storedIdentityFor(metadata) {
  ...
  if (metadata.routeKind === 'r2p') {
    // r2p stored identity mirrors buildR2pIdentity's shape (scalar-only, no list fields):
    // a code-shaped stored identity would carry phantom exclusions/scopes and never match
    // the r2p live identity.
    return {
      targetContextKind: 'r2p',
      guardMode,
      roundLimit,
      requirementDir: String(metadata.manifest.requirementDir),
...
function compareFileSetIdentity(routeKind, stored, requested) {
  ...
  if (routeKind === 'r2p') return compareR2pIdentity({ stored, requested });
...
// Stop a final PASS as a guarded drift blocker when the r2p run.md gate drifted/was
// deleted/became incomplete (CHECKPOINT 4). Mirrors the file-set guard's blocked result
// (status blocked, blockingReason unexpected-worktree-change) — never a PASS.
function finalizeGateDriftBlocked(metadata, gateDrift) {
  try {
    writeFinalReceipt(metadata, {
      finalStatus: 'blocked',
      fixedIssueIds: 'none',
      filesChanged: 'none',
      verificationPerformed: 'run.md gate revalidation',
      deferralsOrBlockers: gateDrift.summary,
      blockingReason: gateDrift.blockingReason,
      statusReason: 'none'
    }, { kind: 'gate-drift', nextAction: 'restore the run.md gate to its reviewed state before retrying finalize' });
  } catch {
    // receipt is best-effort; the manifest still records the block
  }
  updatePersistentManifest(metadata, {
    status: 'blocked',
    currentPhase: 'final',
...
async function runFileSetFinalize(parsed, options) {
  ...
  try {
    ...
    if (finalResponse.finalStatus === 'pass') {
      // CHECKPOINT 4 (before final PASS): an r2p PASS may not be claimed from a stale gate.
      // Re-read run.md and re-run the gate; a drifted/deleted/incomplete run.md stops the
      // run as a guarded blocker (the same out-of-set drift plumbing) instead of PASS. This
      // runs before assertLiveFileSetFreshForPass so a missing gate blocks regardless of the
      // editable-set fingerprint, and before any PASS is validated or persisted.
      const passGateDrift = revalidateR2pGate(metadata);
      if (passGateDrift) {
        return finalizeGateDriftBlocked(metadata, passGateDrift);
      }
