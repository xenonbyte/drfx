...
function fileSetIdentitySeed(parsed) {
  const kind = routeKindFor(parsed);
  if (kind === 'pr') {
  ...
  if (kind === 'code') {
    const scopes = Array.isArray(parsed.invocation.scopes)
...
function deriveFileSetTargetKey(parsed) {
  const seed = fileSetIdentitySeed(parsed);
...
function resolveRouteTargetMetadata(parsed, options = {}) {
  ...
  return {
    ...
    projectRoot,
    targetKey: deriveFileSetTargetKey(parsed),
