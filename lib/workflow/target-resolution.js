...
function r2qRequirementDir(parsed, options = {}) {
  ...
  const baseDir = parsed.invocation.root
    ? path.resolve(explicitRootBase, parsed.invocation.root)
    : cwd;
...
function resolveFileSetProjectRoot(parsed, options = {}) {
  ...
  if (kind === 'r2q') {
    ...
    if (parsed.invocation.root) {
      const explicitRootBase = options.rootCwd || options.originalCwd || options.cwd || process.cwd();
      const explicitRootReal = resolveExistingDirectory(path.resolve(explicitRootBase, parsed.invocation.root));
