  ...
  if (kind === 'code') {
    const scopes = Array.isArray(options.normalizedScopes)
      ...
      : (Array.isArray(parsed.invocation.scopes)
        ...
        : []);
    // Active .drfxignore entries are part of what the file set IS, so they are
    // part of the target identity. CONDITIONAL: an empty exclude list keeps
    // the seed byte-identical to its pre-.drfxignore form so every existing
    // CODE target key (and its on-disk state directory) stays valid.
    const userExcludes = Array.isArray(options.normalizedUserExcludes)
      ? options.normalizedUserExcludes.map((entry) => String(entry)).sort()
      : [];
...
function resolveRouteTargetMetadata(parsed, options = {}) {
  ...
  const normalizedUserExcludes = routeKindFor(parsed) === 'code'
    ? normalizeCodeUserExcludesForIdentity({ cwd: projectRoot, scopes: parsed.invocation.scopes || [] })
    : null;
  return {
