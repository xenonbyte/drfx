...
function resolveFileSetProjectRoot(parsed, options = {}) {
  ...
  if (kind === 'r2p') {
    const requirementDir = r2pRequirementDir(parsed, options);
    if (!isRequirementDirShape(requirementDir)) {
      fail('ERR_R2P_TARGET_SHAPE', `r2p target must be <project>/.req-to-plan/WF-*: ${requirementDir}`);
    }
    if (hasSymlinkSegmentFromReqToPlan(requirementDir)) {
      fail('ERR_R2P_TARGET_SYMLINK', `r2p target must not resolve through a symlink: ${requirementDir}`);
    }
    const projectRoot = projectRootFromRequirementDir(requirementDir);
    if (!projectRoot) {
      fail('ERR_R2P_TARGET_SHAPE', `r2p target must be inside a .req-to-plan project: ${requirementDir}`);
    }
    const projectRootReal = resolveExistingDirectory(projectRoot);
    if (parsed.invocation.root) {
      const explicitRootReal = resolveExistingDirectory(r2pRootDir(parsed, options));
      if (projectRootReal !== explicitRootReal) {
        fail('ERR_R2P_ROOT_CONTAINMENT', `review-fix-r2p root= must contain the requirement directory: ${explicitRootReal}`);
      }
    }
...
function fileSetIdentitySeed(parsed, options = {}) {
  ...
  if (kind === 'r2p') {
    // r2p identity is the resolved requirement directory (root-relative): the same
    ...
    const requirementDir = options.requirementDir !== undefined
      ...
      : r2pRequirementDir(parsed, options);
...
function resolveRouteTargetMetadata(parsed, options = {}) {
  ...
  if (routeKindFor(parsed) === 'r2p') {
    let requirementDir;
    try {
      requirementDir = fs.realpathSync.native(r2pRequirementDir(parsed, options));
    } catch {
      fail('ERR_R2P_TARGET_SHAPE', 'r2p target must be an existing <project>/.req-to-plan/WF-* directory');
    }
