...
function cwdAnchoredR2qProjectRoot(parsed, cwd) {
...
function r2qRootDir(parsed, options = {}) {
  const cwd = options.cwd || process.cwd();
  ...
  if (
    ...
    const cwdProjectRoot = cwdAnchoredR2qProjectRoot(parsed, cwd);
...
function r2qRequirementDir(parsed, options = {}) {
  ...
  const baseDir = r2qRootDir(parsed, options);
...
function resolveFileSetProjectRoot(parsed, options = {}) {
  ...
  if (kind === 'r2q') {
    ...
    if (hasSymlinkSegmentFromReqToPlan(requirementDir)) {
      fail('ERR_R2Q_TARGET_SYMLINK', `r2q target must not resolve through a symlink: ${requirementDir}`);
    }
    const projectRoot = projectRootFromRequirementDir(requirementDir);
    if (!projectRoot) {
    ...
    if (parsed.invocation.root) {
      const explicitRootReal = resolveExistingDirectory(r2qRootDir(parsed, options));
