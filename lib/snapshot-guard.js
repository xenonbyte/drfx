...
const { computeFingerprint } = require('./target-state');
...
function assertTargetIdentity({ projectRoot, targetPath, expectedNormalizedTarget, blockingReason, allowMissing = false }) {
  ...
  try {
    ...
    if (allowMissing && error && error.code === 'ENOENT') return normalizedTarget;
...
function snapshotPathFromAnchor(targetStateDir, round, rollbackAnchor = {}) {
  ...
  if (path.isAbsolute(relativePath) || path.win32.isAbsolute(relativePath)) {
    throw guardError('rollback-unavailable', 'snapshot path must be target-state relative');
  }
  const parts = String(relativePath).split(/[\\/]+/);
  if (parts.some((part) => part === '' || part === '.' || part === '..')) {
    throw guardError('rollback-unavailable', 'snapshot path contains unsafe segments');
  }
  const resolved = path.join(path.resolve(targetStateDir), ...parts);
  if (!isInsideOrEqual(resolved, targetStateDir)) {
    throw guardError('rollback-unavailable', 'snapshot path escapes target state directory');
  }
  return { relativePath: parts.join('/'), absolutePath: resolved };
}
...
function captureSnapshot({
  ...
  fs.copyFileSync(path.resolve(targetPath), absolutePath);
...
function monitorPathRecord({ projectRoot, filePath, normalizedTarget, role }) {
  assertRegularNonSymlink(filePath, role === 'target' ? 'target' : 'monitored file', 'target-only-guard-unavailable');
  const fingerprint = computeFingerprint(filePath);
  const projectRelative = relativeToProject(projectRoot, filePath);
  const normalizedPath = projectRelative || path.resolve(filePath);
  return {
    path: normalizedPath,
    pathSha256: pathSha256(filePath),
    role: normalizedPath === normalizedTarget ? 'target' : role,
    sha256: fingerprint.sha256,
    size: fingerprint.size,
...
function collectMonitorRecords({
  ...
  try {
    ...
    throw guardError('target-only-guard-unavailable', 'target directory is unreadable', { cause: error });
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      throw guardError('target-only-guard-unavailable', 'monitored target directory file must not be a symlink');
    }
...
function checkSnapshotTargetOnly({
  ...
  try {
    ...
    return {
      status: 'passed',
      guardMode: 'snapshot',
      monitorScope: 'target-dir-direct-files-and-references',
      entries: records
    };
  } catch (error) {
...
function restoreSnapshot({
  ...
  fs.copyFileSync(absolutePath, absoluteTarget);
  return {
    status: 'passed',
    guardMode: 'snapshot',
    normalizedTarget,
