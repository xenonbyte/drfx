...
const PLATFORMS = new Set(['claude', 'codex', 'gemini', 'opencode']);
...
const PROVENANCE_SOURCES = new Set(['drfx-doctor-probe', 'runtime-capability-api', 'installer-default']);
...
function containsProjectStateSegment(filePath) {
  return path.resolve(filePath).split(path.sep).includes('.drfx');
}
...
function canonicalizeProbeTmpDir(directoryPath) {
  if (containsProjectStateSegment(directoryPath)) {
    fail(
      ...
      'fingerprint guard probe temp directory must not be inside project .drfx state'
    );
  }
...
function buildDescriptor({
  ...
  provenanceSource = 'drfx-doctor-probe',
...
function validateCurrentDescriptor(
  ...
  if (!provenance || typeof provenance !== 'object') {
    ...
    if (!['drfx doctor', 'drfx install'].includes(provenance.generatedBy)) {
