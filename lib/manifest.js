...
// null byte cannot appear in a filename, so this can never collide with a real entry
const SYMLINK_SENTINEL = '\0 symlink';
...
function directoryTreeMetadata(directoryPath) {
  ...
  function walk(current) {
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) { files.push(SYMLINK_SENTINEL); continue; }
