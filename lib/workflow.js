...
function parseWorkflowArgs(subcommand, args) {
  ...
  if (TARGET_STATE_SUBCOMMANDS.has(subcommand)) {
...
function assertReferencesUnchanged(metadata, guardReport) {
  ...
  const actual = referenceFingerprintsForManifest(metadata);
...
function runBeginFix(parsed, options) {
  ...
  try {
    ...
    if (targetOnlyGuard.status === 'blocked') {
      return endFixBlocked(metadata, targetOnlyGuard.blockingReason, {
    ...
    atomicWriteFile(reportPath, formatFixGuardReport({
      round,
      normalizedTarget: metadata.normalizedTarget,
      targetFingerprint: canonicalFingerprint(computeFingerprint(metadata.targetPath)),
      referenceFingerprints: referenceFingerprintsForManifest(metadata),
      rollbackAnchor,
      targetOnlyGuard,
      lock: {
        ownerId: lease.ownerId,
...
function runEndFix(parsed, options) {
  ...
  const fixReport = parseFixReport(payload);
  const ledger = readLedgerIfPresent(metadata.ledgerPath);
  ...
  const { report: guardReport } = readLatestFixGuardReport(metadata);
