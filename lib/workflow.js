...
function assertNoSymlinkInExistingPath(filePath, stopAt, label) {
...
function validateTargetStateRelativePath({ targetStateDir, relativePath, allowedDirectories, label }) {
  const parts = manifestPathParts(relativePath, label);
  if (allowedDirectories && allowedDirectories.length > 0 && !allowedDirectories.includes(parts[0])) {
    failStateValidation(`${label} must be under ${allowedDirectories.join(', ')}`);
  }

  const absolutePath = path.join(targetStateDir, ...parts);
  const lexicalRelative = path.relative(targetStateDir, absolutePath);
  if (lexicalRelative === '' || lexicalRelative.startsWith('..') || path.isAbsolute(lexicalRelative)) {
    failStateValidation(`${label} escapes target state directory`);
  }
  if (pathExists(targetStateDir) && fs.lstatSync(targetStateDir).isSymbolicLink()) {
    failStateValidation(`${label} target state directory must not be a symlink`);
  }
  const targetDirRealPath = pathExists(targetStateDir) ? fs.realpathSync.native(targetStateDir) : targetStateDir;
  assertNoSymlinkInExistingPath(absolutePath, targetStateDir, label);

  const parentRealPath = fs.realpathSync.native(nearestExistingParent(absolutePath));
  if (pathExists(targetStateDir) && !isInsideOrEqual(parentRealPath, targetDirRealPath)) {
...
function targetStatePathFromManifest(targetStateDir, storedPath, fallbackName, options = {}) {
  ...
  return validateTargetStateRelativePath({
    targetStateDir,
    relativePath,
