...
function workflowBase(parsed) {
  return {
    ...
    targetKey: targetKeyFor(parsed.invocation.target),
    ...
    assuranceSource: parsed.invocation.assuranceSource || (parsed.payloadFlags.assurance ? 'explicit' : 'default'),
...
function unsupportedFrom(parsed, statusReason, strictProofError = null) {
...
function blockedFrom(parsed, blockingReason) {
  return {
    ...workflowBase(parsed),
    ok: false,
    status: 'blocked',
...
async function runWorkflowCommand(subcommand, args, options = {}) {
  ...
  if (parsed.runtimeCheck.stdinHandoff.status === 'unavailable') {
    return blockedFrom(parsed, 'unsafe-handoff-file');
  }

  if (parsed.assurance === 'strict-verified' && parsed.runtimePlatform === 'gemini') {
    return unsupportedFrom(parsed, 'unsupported-runtime-capability');
  }
  ...
  if (parsed.assurance === 'advisory' && parsed.invocation.mode === 'review-and-fix') {
    if (isRuntimeDowngrade(parsed)) return withReadOnlyMode(workflowBase(parsed));
    return unsupportedFrom(parsed, 'advisory-review-and-fix-unsupported');
  }

  return workflowBase(parsed);
}
