...
function buildFileSetContextPack({
  ...
  reviewerGuardBaseline = null,
  fixerGuard = null,
  // r2q carries run.md as a PROTECTED read-only dependency: a [{path, readOnly, sha256}]
  // list the reviewer/fixer must respect but never edit. Other file-set routes pass null,
  // so the pack stays byte-identical (the field is omitted when absent/empty).
  ...
  if (routeKind === 'pr') {
    ...
    // r2q is the requirement-doc backward-fix route: its set is the fixed 03–07 chain
    // resolved from one requirement directory, NOT a CODE scope walk. It carries a
    // neutral requirementDir descriptor and OMITS `scopes` so it never inherits CODE's
    // '(whole project root)' / userExcludes / versionIgnoreSource scope cosmetics.
    descriptor.requirementDir = redactIdentifier(String(fileSet.requirementDir || 'unknown'), projectRoot);
  } else {
  ...
  return {
    ...
    target: 'none',
    ...(fixerGuard ? { fixerGuard } : {}),
    ...(Array.isArray(protectedDependencies) && protectedDependencies.length > 0
      ? { protectedDependencies: normalizeProtectedDependencies(protectedDependencies, projectRoot) }
      : {}),
    ...(isPartitioned ? { reviewMode: 'partitioned', unit_id: unitId } : {})
  };
}

// Protected read-only dependencies (r2q's run.md gate). The path is redacted/relativized
// like file-set members; readOnly is always true; the sha256 fingerprint is carried so the
// reviewer/fixer can confirm the gate is unchanged without ever editing it.
function normalizeProtectedDependencies(deps, projectRoot = null) {
  return (Array.isArray(deps) ? deps : []).map((dep) => ({
    path: redactIdentifier(String((dep && dep.path) || 'unknown'), projectRoot),
    readOnly: true,
    sha256: redactSensitive(String((dep && dep.sha256) || 'none'))
  }));
}
