...
function buildFileSetContextPack({
  ...
  if (routeKind === 'pr') {
    ...
    descriptor.scopes = (Array.isArray(fileSet.normalizedScopes) && fileSet.normalizedScopes.length > 0)
      ...
      : ['(whole project root)'];
    // ACTIVE .drfxignore entries shape the reviewed set, so the reviewer must
    // see them; overridden entries (config beaten by an explicit scope=) are
    // surfaced so the narrowing is never silent.
    if (Array.isArray(fileSet.userExcludes) && fileSet.userExcludes.length > 0) {
      descriptor.userExcludes = fileSet.userExcludes.map((entry) => redactIdentifier(entry, projectRoot));
    }
    if (Array.isArray(fileSet.overriddenUserExcludes) && fileSet.overriddenUserExcludes.length > 0) {
      descriptor.overriddenUserExcludes = fileSet.overriddenUserExcludes.map(
