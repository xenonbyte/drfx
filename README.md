...
Review the whole project root (`scope=` omitted means whole project), or scope it to one or more directories or files. Whole-root CODE review is capped at 300 files or 1,500,000 bytes (counted after all exclusions); use `scope=<path>` or a project-root `.drfxignore` file when a project is larger:
...
- `scope=<path>` names a directory to walk or a single file to include. Repeat `scope=` for multiple scopes. Empty scope means the whole project root, capped at 300 files or 1,500,000 bytes (counted after all exclusions); larger whole-root file sets block as `file-set-too-large` and require a narrower `scope=<path>` or ignore rules that shrink the file set. Explicit non-root directory/file scopes are not subject to the cap; scopes that normalize to the project root, such as `scope=.`, are treated as whole-root and remain capped.
- Built-in exclusions (fixed, always applied): VCS state (`.git`, `.hg`, `.svn`); this tool's state (`.drfx`, legacy `.docs-review-fix`); local agent/tool state (`.claude`, `.codex`, `.codegraph`, `.gemini`, `.opencode`, `.req-to-plan`); dependency trees and package caches (`node_modules`, `bower_components`, `vendor`, `.pnp`, `.yarn`, `.pnpm-store`, `.gradle`, `.m2`); build outputs (`dist`, `build`, `out`, `target`, `.next`, `.nuxt`, `.svelte-kit`, `.output`); coverage and tool caches (`coverage`, `.nyc_output`, `.cache`, `.parcel-cache`, `.turbo`, `__pycache__`, `.pytest_cache`, `.mypy_cache`, `.tox`); temp and editor scratch (`tmp`, `temp`, `.tmp`, `.idea`, `.vscode`); plus the OS scratch files `.DS_Store` and `Thumbs.db`.
...
- Well-known infrastructure directories (`.git`, `.claude`, `.codex`, `.codegraph`, `.gemini`, `.opencode`, `.req-to-plan`, `node_modules`, `.pnpm-store`, `.yarn`, `.cache`, `dist`, `build`, `coverage`) are excluded from monitoring unless the target or a reference lives inside one.
