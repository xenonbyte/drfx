...
function makeSandbox(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'drfx-r2q-test-'));
...
test('parseRunMdGate: garbage input throws ERR_R2Q_RUNMD_UNRECOGNIZED', () => {
  assert.throws(
    () => parseRunMdGate(garbageRunMd),
    (error) => error.code === 'ERR_R2Q_RUNMD_UNRECOGNIZED'
  );
});
...
// ---------------------------------------------------------------------------
// Tests: resolveR2qTarget — happy path
// ---------------------------------------------------------------------------

test('resolveR2qTarget: happy path returns correct shape', (t) => {
  const root = makeSandbox(t);
  const wfDir = makeWfDir(root, 'WF-20260101-aaa-demo');

  const ctx = resolveR2qTarget({ cwd: root, target: wfDir });

  assert.equal(ctx.routeKind, 'r2q');
...
test('resolveR2qTarget: editable file entries have required fields', (t) => {
  const root = makeSandbox(t);
...
// ---------------------------------------------------------------------------
// Tests: resolveR2qTarget — shape/containment errors
// ---------------------------------------------------------------------------

test('resolveR2qTarget: outside .req-to-plan throws ERR_R2Q_TARGET_SHAPE', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: outsideReqToPlanDir }),
    (error) => error.code === 'ERR_R2Q_TARGET_SHAPE'
  );
});

test('resolveR2qTarget: non-WF-* name throws ERR_R2Q_TARGET_SHAPE', (t) => {
  const root = makeSandbox(t);
  const reqDir = path.join(root, '.req-to-plan', 'run-1');
  fs.mkdirSync(reqDir, { recursive: true });

  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: reqDir }),
    (error) => error.code === 'ERR_R2Q_TARGET_SHAPE'
  );
});

test('resolveR2qTarget: symlinked WF dir throws ERR_R2Q_TARGET_SYMLINK', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: symlinkedWfDir }),
    (error) => error.code === 'ERR_R2Q_TARGET_SYMLINK'
  );
});

// Darwin /var -> /private/var alias: resolveR2qTarget must not reject a valid
...
test('resolveR2qTarget: darwin /var alias does not cause false rejection', (t) => {
  ...
  const varBase = fs.mkdtempSync(path.join(os.tmpdir(), 'drfx-r2q-alias-'));
  t.after(() => fs.rmSync(varBase, { recursive: true, force: true }));

  const privateVarAliasWfDir = makeWfDir(varBase, 'WF-20260101-alias');

  assert.doesNotThrow(() =>
    resolveR2qTarget({ cwd: varBase, target: privateVarAliasWfDir })
  );
});

// ---------------------------------------------------------------------------
// Tests: resolveR2qTarget — run.md errors
// ---------------------------------------------------------------------------

test('resolveR2qTarget: missing run.md throws ERR_R2Q_RUNMD_MISSING', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: missingRunMdWfDir }),
    (error) => error.code === 'ERR_R2Q_RUNMD_MISSING'
  );
});

test('resolveR2qTarget: run.md is a directory throws ERR_R2Q_RUNMD_MISSING', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: directoryRunMdWfDir }),
    (error) => error.code === 'ERR_R2Q_RUNMD_MISSING'
  );
});

test('resolveR2qTarget: run.md is a symlink throws ERR_R2Q_RUNMD_SYMLINK', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: symlinkRunMdWfDir }),
    (error) => error.code === 'ERR_R2Q_RUNMD_SYMLINK'
  );
});

// ---------------------------------------------------------------------------
// Tests: resolveR2qTarget — owner doc chain errors
// ---------------------------------------------------------------------------

test('resolveR2qTarget: missing owner doc throws ERR_R2Q_DOC_CHAIN_INCOMPLETE', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: missingSpecWfDir }),
    (error) => error.code === 'ERR_R2Q_DOC_CHAIN_INCOMPLETE'
  );
});

test('resolveR2qTarget: renamed owner doc throws ERR_R2Q_DOC_CHAIN_INCOMPLETE', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: renamedPlanWfDir }),
    (error) => error.code === 'ERR_R2Q_DOC_CHAIN_INCOMPLETE'
  );
});

// ---------------------------------------------------------------------------
// Tests: resolveR2qTarget — fingerprint stability and content-sensitivity
// ---------------------------------------------------------------------------

test('resolveR2qTarget: fingerprint is order-stable (reverse matches forward)', (t) => {
  const root = makeSandbox(t);
...
test('resolveR2qTarget: fingerprint changes when a file content changes', (t) => {
  const root = makeSandbox(t);
  ...
  const after = resolveR2qTarget({ cwd: root, target: wfDir }).fileSetFingerprint;

  assert.notEqual(after, before);
});

// ---------------------------------------------------------------------------
// Tests: resolveR2qTarget — gate errors
// ---------------------------------------------------------------------------

test('resolveR2qTarget: plan not generated throws ERR_R2Q_GATE_PLAN_INCOMPLETE', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: incompletePlanWfDir }),
    (error) => error.code === 'ERR_R2Q_GATE_PLAN_INCOMPLETE'
  );
});

test('resolveR2qTarget: archived dir throws ERR_R2Q_GATE_ARCHIVED', (t) => {
  const root = makeSandbox(t);
  ...
  assert.throws(
    () => resolveR2qTarget({ cwd: root, target: archivedWfDir }),
    (error) => error.code === 'ERR_R2Q_GATE_ARCHIVED'
  );
});

// ---------------------------------------------------------------------------
// Tests: r2q identity family
// ---------------------------------------------------------------------------

test('buildR2qIdentity: builds identity with correct scalar fields', (t) => {
  const root = makeSandbox(t);
  const wfDir = makeWfDir(root, 'WF-20260101-identity');

  const ctx = resolveR2qTarget({ cwd: root, target: wfDir });
  const identity = buildR2qIdentity({ context: ctx, guardMode: 'git', roundLimit: 3 });

  assert.equal(identity.targetContextKind, 'r2q');
...
test('buildR2qIdentity: roundLimit null serializes as "none"', (t) => {
  const root = makeSandbox(t);
  const wfDir = makeWfDir(root, 'WF-20260101-identity-no-limit');

  const ctx = resolveR2qTarget({ cwd: root, target: wfDir });
  const identity = buildR2qIdentity({ context: ctx, guardMode: 'snapshot', roundLimit: null });

  assert.equal(identity.roundLimit, 'none');
});

test('buildR2qIdentity: throws on wrong routeKind', () => {
  assert.throws(
    () => buildR2qIdentity({ context: { routeKind: 'code' }, guardMode: 'git' }),
    (error) => error.code === 'ERR_R2Q_IDENTITY'
  );
});

test('formatR2qIdentityFields / parseR2qIdentityFields: round-trips all scalar fields', (t) => {
  const root = makeSandbox(t);
  const wfDir = makeWfDir(root, 'WF-20260101-identity-roundtrip');

  const ctx = resolveR2qTarget({ cwd: root, target: wfDir });
  const identity = buildR2qIdentity({ context: ctx, guardMode: 'git', roundLimit: 2 });

  const fields = formatR2qIdentityFields(identity);
  const parsed = parseR2qIdentityFields(fields);

  assert.deepEqual(parsed, identity);
});

test('compareR2qIdentity: identical identities match', (t) => {
  const root = makeSandbox(t);
  const wfDir = makeWfDir(root, 'WF-20260101-cmp-match');

  const ctx = resolveR2qTarget({ cwd: root, target: wfDir });
  const identity = buildR2qIdentity({ context: ctx, guardMode: 'git', roundLimit: 1 });

  const result = compareR2qIdentity({ stored: identity, requested: identity });
  assert.equal(result.match, true);
  assert.deepEqual(result.mismatches, []);
});

test('compareR2qIdentity: runMdSha256 drift causes mismatch', (t) => {
  const root = makeSandbox(t);
  ...
  const result = compareR2qIdentity({ stored: stale, requested: identity });
  assert.equal(result.match, false);
  assert.ok(result.mismatches.includes('runMdSha256'));
});

test('compareR2qIdentity: fileSetFingerprint drift causes mismatch', (t) => {
  const root = makeSandbox(t);
...
test('compareR2qIdentity: guardMode drift causes mismatch', (t) => {
  const root = makeSandbox(t);
  const wfDir = makeWfDir(root, 'WF-20260101-cmp-guard-drift');

  const ctx = resolveR2qTarget({ cwd: root, target: wfDir });
  const stored = buildR2qIdentity({ context: ctx, guardMode: 'git', roundLimit: 1 });
  const requested = buildR2qIdentity({ context: ctx, guardMode: 'snapshot', roundLimit: 1 });

  const result = compareR2qIdentity({ stored, requested });
  assert.equal(result.match, false);
  assert.ok(result.mismatches.includes('guardMode'));
});

test('compareR2qIdentity: requirementDir drift causes mismatch', (t) => {
  const root = makeSandbox(t);
