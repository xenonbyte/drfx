...
测试：unit context 有界（只含该 unit + 建议引用，断言无越界）；oversize unit 不含正文、只产 metadata-only coverage blocker；按需读入记入 receipt；缓存跳过必须同时满足 member、suggestedRefs、extraReads 指纹不变；修一个契约文件会强制所有读取过它的 unit 复审；兜底 pack 断言无正文；`unit-review-report` parser；只读 verdict 诚实性（任一 unit `coverage_risk≠none` → 不得 clean）。
