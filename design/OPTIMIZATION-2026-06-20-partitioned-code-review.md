...
- 日期：2026-06-20
- 范围：`lib/target-context.js`、`lib/context-pack.js`、`lib/workflow-state.js`、`lib/workflow/*`、`lib/project-review.js`(新)、`bin/drfx.js`、`shared/rubrics/code.md`、`shared/prompts/{reviewer,coordinator}.md`、`lib/generator.js`、`templates/fragments/route-contract.code.*`、`test/`
...
### 4.2 Key decisions
1. **whole-file 字节分桶**（纯确定性）：按目录子树自然序 bin-pack，每 unit ≤ `MAX_UNIT_BYTES`（初值见 §9）；**永不拆分单个文件**（单文件超预算 → 自成一个 over-budget unit，标记需显式 scoped 复审，不硬切）。`unit_id = unit-NNN` + member 内容 digest（缓存键）。
...
4. **按需契约读取**：unit context 用确定性 `require/import` 正则把该 unit 依赖的 in-root 文件列为**建议只读引用**；reviewer 在 `CONTRACT_READ_BUDGET`（初值见 §9）内拉取；额外读入记进 coverage receipt；超预算 → `coverage_risk:high` → 非 PASS。
5. **earned PASS gate**：全部 high-risk unit 已 body-review + cross-cutting 兜底完成 + findings 全 triage + 无未决 high/medium + `coverage_risk≠high`。任一不满足 → `stopped-with-deferrals` + `coverage-incomplete`，**绝不 PASS**。
...
.drfx/targets/<target-key>/project-review/
  ...
  units.json         # {reviewMode,unitByteBudget,
                     #  units:[{unit_id,member_count,member_bytes,member_digest,files[],suggestedRefs[]}],
                     #  crosscuttingBackstops:[固定列表],
                     #  projectReviewFingerprint}
  summaries/<unit_id>.json  # coverage receipt + interface/contracts-touched 摘要(Phase 2)
...
- `projectReviewFingerprint` = 全 unit `member_digest` 有序 sha256（任一文件变 → 聚合失效；单 unit digest 变 → 仅该 unit 失效 = 增量缓存键）。
...
- `lib/workflow/file-set-context.js` / `start.js`：CODE whole-root 命中超限 → 构建并写 plan，返回 `{status:'partitioned-review',reviewMode,targetStateDir,reviewPlanPath:'project-review/units.json',unitCount,nextAction}`；**不创建 active review manifest**（planning 输出）→ 无需新 `STATUS_VALUES`。
...
测试：`partitionInventory` 确定性 & whole-file 不拆；`suggestRefsFor` 只产 in-root 路径；全量遍历不被 cap 截断；`.drfxignore` / version-ignore / scope-wins 仍生效；`partitioned-review` JSON；`npm run syntaxcheck` + `npm test`；手动对 >cap fixture 跑 `drfx workflow start review-fix-code` 得 plan。
...
- `bin/drfx.js` + 新 `lib/workflow/file-set-unit-review.js`：
  ...
  - `drfx workflow record-review ... --phase unit-review --unit <id> --result-stdin --json` → 写 `findings/<id>.json` + `summaries/<id>.json`（coverage receipt：reviewed / skipped+reason / extra-reads / coverage_risk + contracts-touched 摘要）；缓存键 `member_digest`，未变 unit 跳过（增量）。
- `lib/semantic-parsers.js`：新增 `unit-review-report` schema（additive，不动现有）。
...
测试：unit context 有界（只含该 unit + 建议引用，断言无越界）；按需读入记入 receipt；缓存跳过；兜底 pack 断言无正文；`unit-review-report` parser；只读 verdict 诚实性（任一 unit `coverage_risk≠none` → 不得 clean）。
...
- `lib/workflow-state.js`：`STATUS_REASONS` 加 1 个 `coverage-incomplete`，映射到 `stopped-with-deferrals`（pause，永不 PASS）。
- fix 集成：聚合后接入**现有** triage / fix / diff-review / full-re-review；fix guard（`buildFileSetFixerGuard`，`lib/workflow/helpers.js:834`）写边界 = inventory 文件并集，天然 in-set；修后只重审受影响 unit（增量）再 re-aggregate；PASS 仅经 Key decision 5 的 gate。
...
测试：metadata-only / extra-read-overflow high-risk → `stopped-with-deferrals` + `coverage-incomplete`（从不 PASS）；高严重复核；fix 不越 in-set；gate 满足才 PASS；全生命周期 lifecycle 测试。

---

## 7. 规模 / 回滚 / 迁移

- **规模**：大型功能 + 1 新模块，跨 `target-context / project-review(新) / context-pack / workflow-state / lib/workflow/* / bin/drfx / shared/{rubrics,prompts} / generator+templates+fixtures` + 大测试增量（现 806）。L2 重塑后比初版更小（主力机制复用现有 references / merged-rules，去掉了 summary-first 为主的复杂度）。
...
- `MAX_UNIT_BYTES = 1_000_000`：每 unit 自身正文上限。
...
- 完成判据：whole-root 超限不再硬阻塞而是产出 partition plan（P1）；可跑只读分片审查并得 coverage 化只读结论（P2）；aggregator 出覆盖证明、高严重复核、gate 满足才 PASS、否则 `stopped-with-deferrals` + `coverage-incomplete`（P3）。
