...
### 12.1 v1 实际边界：partitioned 为**只读 fix loop**
...
- **v1 采用的闸门**：保留 `Mode: review-and-fix`（PASS 路径不动），仅在 **fix loop 入口**拒绝 active partition plan：
...
1. **（打磨）`record-triage` 入口早守卫**：当前 partitioned 下 `record-triage` 仍会"成功"并把 manifest 推进到 `fix` phase，直到 `begin-fix` 才拒——略有误导。可在 `record-triage` 入口加同样的 `readActivePartitionedPlan` 只读守卫，让只读边界在更早、更一致处提示。改动面：1 处 lib + 1 条测试。**非缺陷**（`begin-fix` 已兜住安全，且 aggregate nextAction 已引导用户绕开 triage）。

2. **（Plan B，主要）partitioned 自动修复闭环**：实现 §6 Phase 3 原设想的增量重审，让 partitioned 支持真正的 in-place fix —— `end-fix` 后重算 inventory / `projectReviewFingerprint`，更新/重写 `project-review/units.json`，用 `unitsToReReview`（changed ∪ suggestedRefs-hit ∪ extraReads-hit）标记受影响 unit / backstop 为待重审，bounded re-review 后再 re-aggregate。工作量大、风险高，属独立版本，落地前需单独 `/think`。
