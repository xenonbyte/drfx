...
- **不建文件内分块 review**：单个文件大于 `MAX_UNIT_BYTES` 时，本计划不把它切成多段让 model 拼接理解；该情况按 coverage blocker 处理（见 §4.2/§6），不允许假 PASS。
  - **【P2 已落地 / FULFILLED】** 本条预留的"未来文件内分块设计"已由 Part 2（确定性 line-window chunk-as-sub-unit review）实现：可拆分的超预算**文本**文件按行窗口切成 `oversize_chunk` 子 unit，逐块审查赚取覆盖——任一 chunk 未确证（`coverage_risk≠none`）即令整文件维持 coverage blocker（不允许假 PASS）；不可拆分文件（单行超预算 / 二进制）仍维持 `oversize_file:true` 的诚实 blocker。此处的历史决策保留原状，仅标注其预留出口已被 P2 兑现。
...
1. **whole-file 字节分桶**（纯确定性）：按目录子树自然序 bin-pack，每普通 unit ≤ `MAX_UNIT_BYTES`（初值见 §9）；**永不拆分单个文件**。单文件超预算 → 自成 `oversize_file:true` 的 over-budget unit，**不把正文塞进 reviewer context**，立即记录 `coverage_risk:high` / `skipped_reason:single-file-over-budget`，最终只能得到 `stopped-with-deferrals` + `coverage-incomplete`，除非操作者先拆小该文件或未来另立文件内分块设计。`unit_id = unit-NNN` + member 内容 digest（缓存键）。
   - **【P2 已落地 / FULFILLED】** "未来另立文件内分块设计"这一出口现由 Part 2 兑现：assemblePartitionPlan 在分桶后对可拆分的超预算文本文件调用 `splitOversizeFile`，展开为 `oversize_chunk:true` 子 unit（每块 `contextLineRange` 切片 ≤ 字节预算），逐块赚取覆盖；任一 chunk 未确证即整文件维持 blocker（不假 PASS）。仅当文件不可拆分（单行超预算 / 二进制，`computeOversizeChunks` 返回 null）时才退回本条原有的 `oversize_file:true` 诚实 blocker 路径。原决策文字保留不动，仅注记其预留路径已实现。
...
> **【P2 已落地 / FULFILLED】** §8 此处所述"另一个已批准设计补上文件内分块审查"已由 Part 2（确定性 line-window chunk-as-sub-unit review）兑现，将原"已接受残余风险"中**可拆分文本文件**的部分从残余降为已覆盖：可拆分超预算文本文件经按行窗口切块逐块赚取覆盖即可进入 earned PASS；任一 chunk 未确证则整文件保持 `coverage-incomplete`（仍是诚实非 PASS）；不可拆分文件（单行超预算 / 二进制）继续维持 `oversize_file:true` 诚实 blocker。本条历史框架保留不动，仅标注其预留出口已被 P2 实现。
...
## 12. v1 落地修正与 v2 待办（partitioned fix loop）

> 记录于 v1 落地后（commit `fade844`）。§6 Phase 3 原计划的"fix 集成"在 v1 **未按原样实现**；此处诚实记录实际边界与后续项，避免遗忘或回潮。

### 12.1 v1 实际边界：partitioned 为**只读 fix loop**（v1 历史）

> **注**：本节记述 v1 落地时的设计决策。v2 / Plan B（见 §12.2 item 2）已取代该只读边界，现已实现真正的 in-place 修复闭环（见下文）。本节保留为历史记录。

§6 / §189 原写"聚合后接入现有 triage / fix / diff-review / full-re-review，修后重审受影响 unit … 再 re-aggregate"。v1 **不**实现这个 in-place 修复闭环：

- **为何不按原样做**：一次内容修改必然移动 `projectReviewFingerprint`，使每个 per-unit coverage receipt 失效；§6 设想的"修后只重审受影响 unit 再 re-aggregate"增量链路**尚未接通**（`unitsToReReview` 已存在于 `lib/workflow/file-set-unit-review.js`，但未接进 `end-fix`）。
- **为何不降级 mode→read-only**（一度考虑过）：**前提坍塌** —— `lib/workflow-state.js` 的 `pass` 状态硬性要求 `Mode: review-and-fix`；把 manifest 降级为 read-only 会让干净的 over-cap 项目永远只能终结为 `read-only-clean`，**摧毁 aggregate-earns-PASS**（partitioned review 存在的全部理由）。
- **v1 采用的闸门（已移除，见 §12.2 item 2）**：保留 `Mode: review-and-fix`（PASS 路径不动），仅在 **fix loop 入口**拒绝 active partition plan：
  - `begin-fix` 检测到 active partition plan → 门口早失败（不拿锁 / 不建 baseline / 不动 fixer），见 `lib/workflow/file-set-fix.js` `runBeginFix`。
  - `aggregate-review` FAIL 的 nextAction、generated 流程 step 6、route-contract phase 3 均不再承诺 begin-fix，统一指引"narrow `scope=` 修复子集或手动修复 + re-review"。
  - `end-fix` 的 freshness gate 保留为 inactive/stale plan 的 backstop。

### 12.2 v2 待办 / 已实现

1. **（作废）`record-triage` 入口早守卫**：v1 为强化只读边界设想的打磨项。因 Plan B 已移除只读闸门，本项不再需要。v1 设计曾考虑在 `record-triage` 入口加同样的 `readActivePartitionedPlan` 只读守卫，但如今 fix loop 已支持 in-place 修复，该只读边界已历史化。

2. **✅ 已实现：（Plan B，主要）partitioned 自动修复闭环**

   **实现内容**：v1 缺失的 in-place fix loop 已完整实现（Tasks 1-8 已合并至当前分支 feat/partitioned-fix-loop-v2）：
   ```
   aggregate FAIL → triage → begin-fix → fix → end-fix 增量 → 仅受影响 unit + 全部 backstop 的 bounded re-review → re-aggregate → 挣得 PASS
   ```

   **移除的 v1 只读闸门**：
   - v1 的 ②′ `begin-fix` 只读守卫（门口检测 active partition plan 早失败）已移除。
   - v1 的 `end-fix` freshness gate 只读 backstop 已移除。
   - v1 的 "narrow `scope=` 修复子集" 旁路指引已取代为真正的闭环自动修复。

   **实现机制**：`end-fix` 后重算 inventory / `projectReviewFingerprint`，更新/重写 `project-review/units.json`，用 `unitsToReReView`（changed ∪ suggestedRefs-hit ∪ extraReads-hit）标记受影响 unit / backstop 为待重审，bounded re-review 后再 re-aggregate，修复后的 partitioned run 挣得真正的 PASS 状态。

   **仍受阻的越界防护**（保留为 block + reset，不做 fake PASS）：
   - membership / 分桶(bucket) / refs 拓扑漂移仍然检测与阻止；超越边界的修改不进 in-place loop，转而 reset 回 aggregate 让用户重新界定 scope。

3. ~~**（Part 2 待办）oversize 单文件分块审查**：见 §8 预留，仍是 Part 2 版本的后续项（not in scope for Task 9）。~~ **【P2 已落地 / FULFILLED】** 已由 Part 2 确定性 line-window chunk-as-sub-unit review 实现（见上文 §3 决策与 §8 注记）：可拆分超预算文本文件逐块赚取覆盖进入 earned PASS，任一 chunk 未确证则整文件保持 `coverage-incomplete`，不可拆分文件仍维持 `oversize_file:true` 诚实 blocker。
